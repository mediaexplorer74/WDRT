...
namespace Microsoft.WindowsPhone.CompDB
{
	...
	public class BuildCompDB
	{
		...
		public List<CompDBFeature> Features { get; set; }
        
		public List<CompDBPackageInfo> Packages { get; set; }
		...
		public void FilterDB(OwnerType filterOnOwnerType, 
			ReleaseType filterOnReleaseType, bool filterOutSkipForPublishing)
		{
			...
			if (filterOnOwnerType != OwnerType.Invalid)
			{
				...
				if (this.Features != null)
				{
					...
					where feat.Group.Equals(filterOnOwnerType.ToString(), 
			...
			if (this.Features != null)
			{
				//BuildCompDB.f_DisplayClass46_1 f_locals2 = 
				var f_locals2 =

					new BuildCompDB().FindPackage;//.f_DisplayClass46_1();

				f_locals2.filteredPackageIDs = (from pkg1 in this.Packages
				select pkg1.ID).ToList<string>();
				...
				if (this.Type == BuildCompDB.CompDBType.Update)
				{
						...
						this.Features.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							if ((predicate = f_locals2.f9__5) == null)
							{
								BuildCompDB.f_DisplayClass46_1 f_locals3 = f_locals2;
								
								predicate = (f_locals3.f9__5 = 
									((CompDBFeaturePackage pkg) => 
							   f_locals3.filteredPackageIDs.Contains(pkg.ID, 
							...
							if ((predicate2 = f8__locals2.<>9__6) == null)
							{
								BuildCompDB.<>c__DisplayClass46_1 f8__locals4 = f8__locals2;
								predicate2 = (f8__locals4.<>9__6 = ((CompDBFeaturePackage pkg) => !f8__locals4.filteredPackageIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase) && pkg.UpdateType != CompDBFeaturePackage.UpdateTypes.Canonical));
							}
				...
				foreach (CompDBFeature compDBFeature2 in this.Features)
				{
					...
					if ((predicate3 = f8__locals2.<>9__7) == null)
					{
						BuildCompDB.<>c__DisplayClass46_1 CS$<>8__locals5 = f8__locals2;
				...
				IL_270:
				this.Features = list;
				f8__locals2.featurePkgIDs = (from pkg in this.Features.SelectMany((CompDBFeature feat) => feat.Packages)
				select pkg.ID).ToList<string>();
				this.Packages = (from pkg in this.Packages
				where f8__locals2.featurePkgIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase)
				select pkg).ToList<CompDBPackageInfo>();
