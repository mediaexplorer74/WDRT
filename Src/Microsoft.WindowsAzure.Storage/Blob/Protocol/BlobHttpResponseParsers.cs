...
namespace Microsoft.WindowsAzure.Storage.Blob.Protocol
{
	...
	public static class BlobHttpResponseParsers
	{
		...
		public static BlobProperties GetProperties(HttpWebResponse response)
		{
			...
			string text = response.Headers["x-ms-blob-type"];
			if (!string.IsNullOrEmpty(text))
			{
				blobProperties.BlobType = (BlobType)Enum.Parse(typeof(BlobType), text, true);
			}
			...
			string text2 = response.Headers[HttpResponseHeader.ContentRange];
			string text3 = response.Headers["Content-Length"];
			string text4 = response.Headers["x-ms-blob-content-length"];
			if (!string.IsNullOrEmpty(text2))
			{
				blobProperties.Length = long.Parse(text2.Split(new char[] { '/' })[1], CultureInfo.InvariantCulture);
			}
			else if (!string.IsNullOrEmpty(text4))
			{
				blobProperties.Length = long.Parse(text4, CultureInfo.InvariantCulture);
			}
			...
			string text5 = response.Headers["x-ms-blob-sequence-number"];
			if (!string.IsNullOrEmpty(text5))
			{
				blobProperties.PageBlobSequenceNumber = new long?(long.Parse(text5, CultureInfo.InvariantCulture));
			}
			string text6 = response.Headers["x-ms-blob-committed-block-count"];
			if (!string.IsNullOrEmpty(text6))
			{
				blobProperties.AppendBlobCommittedBlockCount = new int?(int.Parse(text6, CultureInfo.InvariantCulture));
			}
		...
		{
			CommonUtility.AssertNotNull("response", response);
			string text = response.Headers["x-ms-lease-status"];
			return BlobHttpResponseParsers.GetLeaseStatus(text);
		}
		...
		{
			CommonUtility.AssertNotNull("response", response);
			string text = response.Headers["x-ms-lease-state"];
			return BlobHttpResponseParsers.GetLeaseState(text);
		}
		...
		{
			CommonUtility.AssertNotNull("response", response);
			string text = response.Headers["x-ms-lease-duration"];
			return BlobHttpResponseParsers.GetLeaseDuration(text);
		}
		...
		public static int? GetRemainingLeaseTime(HttpWebResponse response)
		{
			CommonUtility.AssertNotNull("response", response);
			int num;
			if (int.TryParse(response.Headers["x-ms-lease-time"], out num))
			{
				return new int?(num);
			}
			return null;
		...
		internal static LeaseStatus GetLeaseStatus(string leaseStatus)
		{
			if (!string.IsNullOrEmpty(leaseStatus))
			{
				...
				throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid lease status in response: '{0}'", new object[] { leaseStatus }), "leaseStatus");
			}
		...
		internal static LeaseState GetLeaseState(string leaseState)
		{
			if (!string.IsNullOrEmpty(leaseState))
			{
				...
				throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid lease state in response: '{0}'", new object[] { leaseState }), "leaseState");
			}
		...
		internal static LeaseDuration GetLeaseDuration(string leaseDuration)
		{
			if (!string.IsNullOrEmpty(leaseDuration))
			{
				...
				throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid lease duration in response: '{0}'", new object[] { leaseDuration }), "leaseDuration");
			}
