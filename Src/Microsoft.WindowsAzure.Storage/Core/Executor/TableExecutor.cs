...
namespace Microsoft.WindowsAzure.Storage.Core.Executor
{
	...
	internal class TableExecutor : ExecutorBase
	{
		...
		public static void InitRequest<T, INTERMEDIATE_TYPE>(ExecutionState<T> executionState)
		{
				...
				else
				{
					...
					try
					{
						...
						else
						{
							...
							Logger.LogInformational(executionState.OperationContext, "Starting asynchronous request to {0}.", new object[] { tableCommandRef.Context.BaseUri });
							tableCommandRef.Begin(delegate(IAsyncResult res)
							{
											...
											else
											{
												...
												using (IEnumerator<OperationResponse> enumerator = dataServiceResponse.GetEnumerator())
												{
													while (enumerator.MoveNext())
													{
														...
														if (executionState.Req != null)
														{
															TableExecutor.SetStorageCmdRequestResults(executionState.Cmd.RequestResults.ElementAt(num), operationResponse);
															Logger.LogInformational(executionState.OperationContext, "Response received. Status code = {0}, Request ID = {1}, Content-MD5 = {2}, ETag = {3}.", new object[]
															{
								...
								{
									Logger.LogWarning(executionState.OperationContext, "Exception thrown during the operation: {0}.", new object[] { ex2.Message });
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown during the operation: {0}.", new object[] { ex.Message });
				if (executionState.ExceptionRef == null || !(executionState.ExceptionRef is StorageException))
				{
		...
		private static void EndOperation<T, INTERMEDIATE_TYPE>(ExecutionState<T> executionState)
		{
			...
			try
			{
				...
				{
					Logger.LogError(executionState.OperationContext, flag2 ? "Operation cannot be retried because the maximum execution time has been reached. Failing with {0}." : "Retry policy did not allow for a retry. Failing with {0}.", new object[] { executionState.ExceptionRef.Message });
					executionState.OnComplete();
				}
				else
				{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while retrying operation: {0}.", new object[] { ex2.Message });
			...
			try
			{
				for (;;)
				{
					...
					try
					{
						...
						try
						{
							Logger.LogInformational(executionState.OperationContext, "Starting synchronous request to {0}.", new object[] { cmd.Context.BaseUri });
		...
		internal static HttpWebResponse GetResponseForRequest<T>(ExecutionState<T> executionState)
		{
			...
			try
			{
				httpWebResponse = (HttpWebResponse)executionState.Req.GetResponse();
			}
