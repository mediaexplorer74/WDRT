...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	...
	public static class LongPathDirectory
	{
		...
		public static string[] GetDirectories(string path, string searchPattern, SearchOption searchOptions)
		{
			...
			bool flag = searchOptions == SearchOption.AllDirectories;
			IntPtr zero = IntPtr.Zero;
			int num = 0;
			string text = Path.Combine(path, Path.GetDirectoryName(searchPattern));
			string fileName = Path.GetFileName(searchPattern);
			int num2 = NativeMethods.IU_GetAllDirectories(text, fileName, flag, out zero, out num);
			if (num2 != 0)
			{
			...
			try
			{
				array = LongPathCommon.ConvertPtrArrayToStringArray(zero, num);
			}
			finally
			{
		...
		public static string[] GetFiles(string path, string searchPattern, SearchOption searchOptions)
		{
			...
			string text = Path.Combine(path, Path.GetDirectoryName(searchPattern));
			string fileName = Path.GetFileName(searchPattern);
			int num2 = NativeMethods.IU_GetAllFiles(text, fileName, flag, out zero, out num);
			if (num2 != 0)
			{
				throw LongPathCommon.GetExceptionFromWin32Error(num2);
			}
