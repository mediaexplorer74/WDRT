...
namespace Microsoft.WindowsAzure.Storage.Table.Queryable
{
	...
	public static class TableQueryableExtensions
	{
		...
		static TableQueryableExtensions()
		{
			...
			TableQueryableExtensions.WithOptionsMethodInfo = methods.Where((MethodInfo m) => m.Name == "WithOptions").FirstOrDefault<MethodInfo>();
			TableQueryableExtensions.WithContextMethodInfo = methods.Where((MethodInfo m) => m.Name == "WithContext").FirstOrDefault<MethodInfo>();
			TableQueryableExtensions.ResolveMethodInfo = methods.Where((MethodInfo m) => m.Name == "Resolve").FirstOrDefault<MethodInfo>();
		}
		...
		public static TableQuery<TElement> WithOptions<TElement>(this IQueryable<TElement> query, TableRequestOptions options)
		{
			...
			return (TableQuery<TElement>)query.Provider.CreateQuery<TElement>(Expression.Call(null, TableQueryableExtensions.WithOptionsMethodInfo.MakeGenericMethod(new Type[] { typeof(TElement) }), new Expression[]
			{
				query.Expression,
		...
		public static TableQuery<TElement> WithContext<TElement>(this IQueryable<TElement> query, OperationContext operationContext)
		{
			...
			return (TableQuery<TElement>)query.Provider.CreateQuery<TElement>(Expression.Call(null, TableQueryableExtensions.WithContextMethodInfo.MakeGenericMethod(new Type[] { typeof(TElement) }), new Expression[]
			{
				query.Expression,
