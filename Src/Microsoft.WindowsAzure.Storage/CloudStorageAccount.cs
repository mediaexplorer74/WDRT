...
namespace Microsoft.WindowsAzure.Storage
{
	...
	public sealed class CloudStorageAccount
	{
		...
		public CloudStorageAccount(StorageCredentials storageCredentials, string endpointSuffix, bool useHttps)
		{
			...
			string text = (useHttps ? "https" : "http");
			this.BlobStorageUri = CloudStorageAccount.ConstructBlobEndpoint(text, storageCredentials.AccountName, endpointSuffix);
			this.QueueStorageUri = CloudStorageAccount.ConstructQueueEndpoint(text, storageCredentials.AccountName, endpointSuffix);
			this.TableStorageUri = CloudStorageAccount.ConstructTableEndpoint(text, storageCredentials.AccountName, endpointSuffix);
			this.FileStorageUri = CloudStorageAccount.ConstructFileEndpoint(text, storageCredentials.AccountName, endpointSuffix);
			this.Credentials = storageCredentials;
		...
		public static CloudStorageAccount Parse(string connectionString)
		{
			...
			if (CloudStorageAccount.ParseImpl(connectionString, out cloudStorageAccount, delegate(string err)
			{
		...
		public static bool TryParse(string connectionString, out CloudStorageAccount account)
		{
			...
			try
			{
				flag = CloudStorageAccount.ParseImpl(connectionString, out account, delegate(string err)
				{
				});
			}
			catch (Exception)
		...
		public string ToString(bool exportSecrets)
		{
			...
			List<string> list = this.Settings.Select((KeyValuePair<string, string> pair) => string.Format(CultureInfo.InvariantCulture, "{0}={1}", new object[] { pair.Key, pair.Value })).ToList<string>();
		...
		internal static bool ParseImpl(string connectionString, out CloudStorageAccount accountInformation, Action<string> error)
		{
			...
			{
				CloudStorageAccount.AllRequired(new KeyValuePair<string, Func<string, bool>>[] { CloudStorageAccount.UseDevelopmentStorageSetting }),
				CloudStorageAccount.Optional(new KeyValuePair<string, Func<string, bool>>[] { CloudStorageAccount.DevelopmentStorageProxyUriSetting })
			}))
			{
				string text = null;
				if (dictionary.TryGetValue("DevelopmentStorageProxyUri", out text))
				{
					accountInformation = CloudStorageAccount.GetDevelopmentStorageAccount(new Uri(text));
				}
				else
				{
			...
			{
				string text2 = null;
				dictionary.TryGetValue("BlobEndpoint", out text2);
				string text3 = null;
				dictionary.TryGetValue("QueueEndpoint", out text3);
				string text4 = null;
				dictionary.TryGetValue("TableEndpoint", out text4);
				string text5 = null;
				dictionary.TryGetValue("FileEndpoint", out text5);
				accountInformation = new CloudStorageAccount(CloudStorageAccount.GetCredentials(dictionary), (text2 != null) ? new StorageUri(new Uri(text2)) : CloudStorageAccount.ConstructBlobEndpoint(dictionary), (text3 != null) ? new StorageUri(new Uri(text3)) : CloudStorageAccount.ConstructQueueEndpoint(dictionary), (text4 != null) ? new StorageUri(new Uri(text4)) : CloudStorageAccount.ConstructTableEndpoint(dictionary), (text5 != null) ? new StorageUri(new Uri(text5)) : CloudStorageAccount.ConstructFileEndpoint(dictionary));
				string text6 = null;
				if (dictionary.TryGetValue("EndpointSuffix", out text6))
				{
					accountInformation.EndpointSuffix = text6;
				}
				accountInformation.Settings = CloudStorageAccount.ValidCredentials(dictionary);
				return true;
			}
			...
			{
				Uri uri = ((!dictionary.ContainsKey("BlobEndpoint") || dictionary["BlobEndpoint"] == null) ? null : new Uri(dictionary["BlobEndpoint"]));
				Uri uri2 = ((!dictionary.ContainsKey("QueueEndpoint") || dictionary["QueueEndpoint"] == null) ? null : new Uri(dictionary["QueueEndpoint"]));
				Uri uri3 = ((!dictionary.ContainsKey("TableEndpoint") || dictionary["TableEndpoint"] == null) ? null : new Uri(dictionary["TableEndpoint"]));
				Uri uri4 = ((!dictionary.ContainsKey("FileEndpoint") || dictionary["FileEndpoint"] == null) ? null : new Uri(dictionary["FileEndpoint"]));
				accountInformation = new CloudStorageAccount(CloudStorageAccount.GetCredentials(dictionary), uri, uri2, uri3, uri4);
				accountInformation.Settings = CloudStorageAccount.ValidCredentials(dictionary);
				return true;
			}
			accountInformation = null;
		...
		private static IDictionary<string, string> ParseStringIntoSettings(string connectionString, Action<string> error)
		{
			...
			string[] array = connectionString.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
			string[] array2 = array;
			int i = 0;
			while (i < array2.Length)
			{
				string text = array2[i];
				string[] array3 = text.Split(new char[] { '=' }, 2);
				...
				else
				{
					...
					error(string.Format(CultureInfo.InvariantCulture, "Duplicate setting '{0}' found.", new object[] { array3[0] }));
					dictionary2 = null;
				}
				return dictionary2;
			}
		...
		private static Func<IDictionary<string, string>, IDictionary<string, string>> AllRequired(params KeyValuePair<string, Func<string, bool>>[] requiredSettings)
		{
			return delegate(IDictionary<string, string> settings)
			{
				...
				{
					string text;
					if (!dictionary.TryGetValue(keyValuePair.Key, out text) || !keyValuePair.Value(text))
					{
						return null;
					}
		...
		private static StorageUri ConstructBlobEndpoint(string scheme, string accountName, string endpointSuffix)
		{
			...
			string text = string.Format(CultureInfo.InvariantCulture, "{0}://{1}.{2}.{3}/", new object[] { scheme, accountName, "blob", endpointSuffix });
			string text2 = string.Format(CultureInfo.InvariantCulture, "{0}://{1}{2}.{3}.{4}", new object[] { scheme, accountName, "-secondary", "blob", endpointSuffix });
			return new StorageUri(new Uri(text), new Uri(text2));
		}
		...
		private static StorageUri ConstructFileEndpoint(string scheme, string accountName, string endpointSuffix)
		{
			...
			string text = string.Format(CultureInfo.InvariantCulture, "{0}://{1}.{2}.{3}/", new object[] { scheme, accountName, "file", endpointSuffix });
			string text2 = string.Format(CultureInfo.InvariantCulture, "{0}://{1}{2}.{3}.{4}", new object[] { scheme, accountName, "-secondary", "file", endpointSuffix });
			return new StorageUri(new Uri(text), new Uri(text2));
		}
		...
		private static StorageUri ConstructQueueEndpoint(string scheme, string accountName, string endpointSuffix)
		{
			...
			string text = string.Format(CultureInfo.InvariantCulture, "{0}://{1}.{2}.{3}/", new object[] { scheme, accountName, "queue", endpointSuffix });
			string text2 = string.Format(CultureInfo.InvariantCulture, "{0}://{1}{2}.{3}.{4}", new object[] { scheme, accountName, "-secondary", "queue", endpointSuffix });
			return new StorageUri(new Uri(text), new Uri(text2));
		}
		...
		private static StorageUri ConstructTableEndpoint(string scheme, string accountName, string endpointSuffix)
		{
			...
			string text = string.Format(CultureInfo.InvariantCulture, "{0}://{1}.{2}.{3}/", new object[] { scheme, accountName, "table", endpointSuffix });
			string text2 = string.Format(CultureInfo.InvariantCulture, "{0}://{1}{2}.{3}.{4}", new object[] { scheme, accountName, "-secondary", "table", endpointSuffix });
			return new StorageUri(new Uri(text), new Uri(text2));
		}
		...
		private static readonly KeyValuePair<string, Func<string, bool>> UseDevelopmentStorageSetting = CloudStorageAccount.Setting("UseDevelopmentStorage", new string[] { "true" });
		...
		private static readonly KeyValuePair<string, Func<string, bool>> DefaultEndpointsProtocolSetting = CloudStorageAccount.Setting("DefaultEndpointsProtocol", new string[] { "http", "https" });
