...
namespace Microsoft.WindowsAzure.Storage.Blob.Protocol
{
	...
	public sealed class ListBlobsResponse : ResponseParsingBase<IListBlobEntry>
	{
		...
		private IListBlobEntry ParseBlobEntry(Uri baseUri)
		{
							...
							while (this.reader.IsStartElement())
							{
								...
								else
								{
									...
									case "Last-Modified":
										...
										continue;
									case "Etag":
										blobAttributes.Properties.ETag = string.Format(CultureInfo.InvariantCulture, "\"{0}\"", new object[] { this.reader.ReadElementContentAsString() });
										continue;
									case "Content-Length":
									...
									case "BlobType":
									{
										string text8 = this.reader.ReadElementContentAsString();
										string text9;
										if ((text9 = text8) == null)
										{
											continue;
									...
									case "CopyCompletionTime":
										text4 = this.reader.ReadElementContentAsString();
										continue;
									case "CopyStatus":
										text3 = this.reader.ReadElementContentAsString();
										continue;
									case "CopyProgress":
										text5 = this.reader.ReadElementContentAsString();
										continue;
									case "CopySource":
										text6 = this.reader.ReadElementContentAsString();
										continue;
									case "CopyStatusDescription":
										text7 = this.reader.ReadElementContentAsString();
										continue;
									}
									this.reader.Skip();
								}
			...
			{
				blobAttributes.CopyState = BlobHttpResponseParsers.GetCopyAttributes(text3, text2, text6, text5, text4, text7);
			}
