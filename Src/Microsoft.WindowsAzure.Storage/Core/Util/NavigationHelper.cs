...
namespace Microsoft.WindowsAzure.Storage.Core.Util
{
	...
	internal static class NavigationHelper
	{
		...
		internal static string GetContainerName(Uri blobAddress, bool? usePathStyleUris)
		{
			string text;
			string text2;
			NavigationHelper.GetContainerNameAndBlobName(blobAddress, usePathStyleUris, out text, out text2);
			return text;
		}
		...
		internal static string GetBlobName(Uri blobAddress, bool? usePathStyleUris)
		{
			...
			string text2;
			NavigationHelper.GetContainerNameAndBlobName(blobAddress, usePathStyleUris, out text, out text2);
			return Uri.UnescapeDataString(text2);
		}
		...
		internal static string GetShareName(Uri fileAddress, bool? usePathStyleUris)
		{
			string text;
			string text2;
			NavigationHelper.GetShareNameAndFileName(fileAddress, usePathStyleUris, out text, out text2);
			return text;
		}
		...
		internal static string GetFileAndDirectoryName(Uri fileAddress, bool? usePathStyleUris)
		{
			...
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Invalid file address '{0}', missing share information", new object[] { fileAddress });
				throw new ArgumentException(text, "fileAddress");
			}
			if (segments.Length - 1 == num)
			{
				return string.Empty;
		...
		internal static Uri GetServiceClientBaseAddress(Uri addressUri, bool? usePathStyleUris)
		{
			...
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Missing account name information inside path style uri. Path style uris should be of the form http://<IPAddressPlusPort>/<accountName>", new object[0]);
				throw new ArgumentException("address", text);
			}
		...
		internal static Uri AppendPathToSingleUri(Uri uri, string relativeUri, string sep)
		{
			...
			{
				text = sep + relativeUri;
			}
			UriBuilder uriBuilder2 = uriBuilder;
			uriBuilder2.Path += text;
		...
		internal static string GetContainerNameFromContainerAddress(Uri uri, bool? usePathStyleUris)
		{
			...
			if (array.Length < 3)
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Cannot find account information inside Uri '{0}'", new object[] { uri.AbsoluteUri });
		...
		private static bool GetContainerNameAndBlobName(Uri blobAddress, bool? usePathStyleUris, out string containerName, out string blobName)
		{
			...
			if (segments.Length - 1 < num)
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Invalid blob address '{0}', missing container information", new object[] { blobAddress });
				throw new ArgumentException(text, "blobAddress");
			}
			if (segments.Length - 1 == num)
			{
				string text2 = segments[num];
				if (text2[text2.Length - 1] == '/')
				{
					containerName = text2.Trim(new char[] { '/' });
		...
		internal static DateTimeOffset ParseSnapshotTime(string snapshot)
		{
			...
			if (!DateTimeOffset.TryParse(snapshot, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out dateTimeOffset))
			{
		...
		private static Uri ParseBlobQueryAndVerify(Uri address, out StorageCredentials parsedCredentials, out DateTimeOffset? parsedSnapshot)
		{
			...
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Address '{0}' is a relative address. Only absolute addresses are permitted.", new object[] { address.ToString() });
				throw new ArgumentException(text, "address");
			}
			IDictionary<string, string> dictionary = HttpWebUtility.ParseQueryString(address.Query);
			string text2;
			if (dictionary.TryGetValue("snapshot", out text2) && !string.IsNullOrEmpty(text2))
			{
				parsedSnapshot = new DateTimeOffset?(NavigationHelper.ParseSnapshotTime(text2));
			}
			parsedCredentials = SharedAccessSignatureHelper.ParseQuery(dictionary);
			if (parsedCredentials != null)
			{
				string text3;
				dictionary.TryGetValue("sr", out text3);
				if (string.IsNullOrEmpty(text3))
				{
					string text4 = string.Format(CultureInfo.CurrentCulture, "Missing mandatory parameters for valid Shared Access Signature", new object[0]);
					throw new ArgumentException(text4);
				}
			}
		...
		private static Uri ParseFileQueryAndVerify(Uri address, out StorageCredentials parsedCredentials)
		{
			...
			{
				string text2;
				dictionary.TryGetValue("sr", out text2);
				if (string.IsNullOrEmpty(text2))
				{
					string text3 = string.Format(CultureInfo.CurrentCulture, "Missing mandatory parameters for valid Shared Access Signature", new object[0]);
					throw new ArgumentException(text3);
				}
			}
			return new Uri(address.GetComponents(UriComponents.Scheme | UriComponents.Host | UriComponents.Port | UriComponents.Path, UriFormat.UriEscaped));
		}
		...
		private static Uri ParseQueueTableQueryAndVerify(Uri address, out StorageCredentials parsedCredentials)
		{
			...
			if (!address.IsAbsoluteUri)
			{
				string text = string.Format(CultureInfo.InvariantCulture, "Address '{0}' is a relative address. Only absolute addresses are permitted.", new object[] { address.ToString() });
				throw new ArgumentException(text, "address");
			}
			IDictionary<string, string> dictionary = HttpWebUtility.ParseQueryString(address.Query);
			parsedCredentials = SharedAccessSignatureHelper.ParseQuery(dictionary);
		...
		internal static string GetAccountNameFromUri(Uri clientUri, bool? usePathStyleUris)
		{
			...
			if (usePathStyleUris.Value)
			{
				if (array.Length < 3)
				{
					string text = string.Format(CultureInfo.InvariantCulture, "Cannot find account information inside Uri '{0}'", new object[] { clientUri.AbsoluteUri });
					throw new InvalidOperationException(text);
				}
				return array[2];
			}
			else
			{
				if (array.Length < 2)
				{
					string text2 = string.Format(CultureInfo.InvariantCulture, "Cannot find account information inside Uri '{0}'", new object[] { clientUri.AbsoluteUri });
					throw new InvalidOperationException(text2);
				}
				int num = array[1].IndexOf(".", StringComparison.Ordinal);
				return array[1].Substring(0, num);
			}
		}
