...
namespace ComponentAce.Encryption
{
	...
	internal class TCipher_Blowfish : Cipher
	{
		...
		public TCipher_Blowfish(byte[] Key)
			: base(Key)
		{
		}
		...
		public override void Init(byte[] Key, int Size, byte[] IVector)
		{
			...
			for (int j = 0; j <= 17; j++)
			{
				...
				int num2 = BitConverter.ToInt32(base.User, length * 4 + j * 4) ^ num;
				BitConverter.GetBytes(num2).CopyTo(base.User, length * 4 + j * 4);
				i = (i + 4) % Size;
			}
			for (int j = 0; j <= 8; j++)
			{
				this.Encode(array, 0);
				int num3 = DecUtil.SwapInteger(BitConverter.ToInt32(array, 0));
				BitConverter.GetBytes(num3).CopyTo(base.User, (length + j * 2) * 4);
				num3 = DecUtil.SwapInteger(BitConverter.ToInt32(array, 4));
				BitConverter.GetBytes(num3).CopyTo(base.User, (length + j * 2 + 1) * 4);
			}
			for (int j = 0; j <= 3; j++)
			{
				for (i = 0; i <= 127; i++)
				{
					this.Encode(array, 0);
					int num3 = DecUtil.SwapInteger(BitConverter.ToInt32(array, 0));
					BitConverter.GetBytes(num3).CopyTo(base.User, (j * 256 + i * 2) * 4);
					num3 = DecUtil.SwapInteger(BitConverter.ToInt32(array, 4));
					BitConverter.GetBytes(num3).CopyTo(base.User, (j * 256 + i * 2 + 1) * 4);
				}
			}
			array.Initialize();
