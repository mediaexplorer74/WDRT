...
namespace Microsoft.WindowsAzure.Storage.Shared.Protocol
{
	...
	internal static class Request
	{
		...
		internal static void WriteSharedAccessIdentifiers<T>(IDictionary<string, T> sharedAccessPolicies, Stream outputStream, Action<T, XmlWriter> writePolicyXml)
		{
			...
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Too many '{0}' shared access policy identifiers provided. Server does not support setting more than '{1}' on a single container, queue, table, or share.", new object[] { sharedAccessPolicies.Count, 5 });
				throw new ArgumentOutOfRangeException("sharedAccessPolicies", text);
			}
			...
			}))
			{
				...
				foreach (string text2 in sharedAccessPolicies.Keys)
				{
					xmlWriter.WriteStartElement("SignedIdentifier");
					xmlWriter.WriteElementString("Id", text2);
					xmlWriter.WriteStartElement("AccessPolicy");
					T t = sharedAccessPolicies[text2];
					writePolicyXml(t, xmlWriter);
