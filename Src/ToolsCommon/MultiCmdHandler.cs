...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	...
	public class MultiCmdHandler
	{
		...
		{
			LogUtil.Message("Usage: {0} <command> <parameters>", new object[] { this.appName });
			...
			LogUtil.Message("\t Run {0} <command> /? to check command line parameters for each command", new object[] { this.appName });
		}
		...
		public int Run(string[] args)
		{
			...
			int num = -1;
			string text = ((args.Length > 1) ? string.Join(" ", args.Skip(1)) : string.Empty);
			...
			{
				num = cmdHandler.Execute(text, this.appName);
			}
			return num;
		}
