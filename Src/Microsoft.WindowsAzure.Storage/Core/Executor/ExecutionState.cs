	...
	internal class ExecutionState<T> : StorageCommandAsyncResult
	{
		...
		private void CheckDisposeAction()
		{
			...
			if (restCMD != null && restCMD.DisposeAction != null)
			{
				...
				{
					Logger.LogWarning(this.OperationContext, "Disposing action threw an exception : {0}.", new object[] { ex.Message });
				}
			}
		}
		...
		private void InitializeLocation()
		{
			...
			if (restCMD != null)
			{
				...
				Logger.LogInformational(this.OperationContext, "Starting operation with location {0} per location mode {1}.", new object[] { this.CurrentLocation, restCMD.LocationMode });
				return;
			}
			this.CurrentLocation = StorageLocation.Primary;
		}
