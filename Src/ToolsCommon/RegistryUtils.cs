...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	...
	public static class RegistryUtils
	{
		...
		public static void ConvertSystemHiveToRegFile(DriveInfo systemDrive, SystemRegistryHiveFiles hive, string outputRegFile)
		{
			...
			string text = Path.Combine(systemDrive.RootDirectory.FullName, "windows\\system32\\config");
			string text2 = Path.Combine(text, Enum.GetName(typeof(SystemRegistryHiveFiles), hive));
			RegistryUtils.ConvertHiveToRegFile(text2, RegistryUtils.MapHiveToMountPoint(hive), outputRegFile);
		}
		...
		{
			string text = string.Format("LOAD {0} {1}", mountpoint, inputhive);
			string text2 = Environment.ExpandEnvironmentVariables("%windir%\\System32\\REG.EXE");
			int num = CommonUtils.RunProcess(Environment.CurrentDirectory, text2, text, true);
			if (0 < num)
			{
		...
		{
			string text = string.Format("EXPORT {0} {1}", mountpoint, outputfile);
			string text2 = Environment.ExpandEnvironmentVariables("%windir%\\System32\\REG.EXE");
			int num = CommonUtils.RunProcess(Environment.CurrentDirectory, text2, text, true);
			if (0 < num)
			{
		...
		{
			string text = string.Format("UNLOAD {0}", mountpoint);
			string text2 = Environment.ExpandEnvironmentVariables("%windir%\\System32\\REG.EXE");
			int num = CommonUtils.RunProcess(Environment.CurrentDirectory, text2, text, true);
			if (0 < num)
			{
		...
		public static string MapHiveFileToMountPoint(string hiveFile)
		{
			...
			SystemRegistryHiveFiles systemRegistryHiveFiles;
			if (!RegistryUtils.hiveMap.TryGetValue(Path.GetFileName(hiveFile), out systemRegistryHiveFiles))
			{
				return "";
			}
			return RegistryUtils.MapHiveToMountPoint(systemRegistryHiveFiles);
		}
