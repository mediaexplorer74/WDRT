...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public class TableEntity : ITableEntity
	{
		...
		private static void ReflectionRead(object entity, IDictionary<string, EntityProperty> properties, OperationContext operationContext)
		{
			...
			foreach (PropertyInfo propertyInfo in properties2)
			{
				if (!TableEntity.ShouldSkipProperty(propertyInfo, operationContext))
				{
					...
					{
						Logger.LogInformational(operationContext, "Omitting property '{0}' from de-serialization because there is no corresponding entry in the dictionary provided.", new object[] { propertyInfo.Name });
					}
					else
					{
		...
		internal static bool ShouldSkipProperty(PropertyInfo property, OperationContext operationContext)
		{
			...
			{
				Logger.LogInformational(operationContext, "Omitting property '{0}' from serialization/de-serialization because the property's getter/setter are not public.", new object[] { property.Name });
				return true;
			}
			...
			{
				Logger.LogInformational(operationContext, "Omitting property '{0}' from serialization/de-serialization because IgnoreAttribute has been set on that property.", new object[] { property.Name });
				return true;
			}
			return false;
		}
		...
		private static Action<object, OperationContext, IDictionary<string, EntityProperty>> CompileReadAction(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties.Where((PropertyInfo p) => !TableEntity.ShouldSkipProperty(p, null)))
			{
				...
				if (expression != null)
				{
					...
					list.Add(Expression.IfThen(Expression.NotEqual(parameterExpression4, Expression.Constant(null)), Expression.IfThenElse(Expression.Call(parameterExpression4, TableEntity.EntityPropertyIsNullInfo), Expression.Call(Expression.Convert(parameterExpression, type), propertyInfo.FindSetProp(), new Expression[] { Expression.Convert(Expression.Constant(null), propertyInfo.PropertyType) }), expression)));
				}
			}
			if (list.Count == 0)
			{
			...
			Expression<Action<object, OperationContext, IDictionary<string, EntityProperty>>> expression2 = Expression.Lambda<Action<object, OperationContext, IDictionary<string, EntityProperty>>>(Expression.Block(new ParameterExpression[] { parameterExpression4, parameterExpression5 }, list), new ParameterExpression[] { parameterExpression, parameterExpression2, parameterExpression3 });
		...
		private static Func<object, OperationContext, IDictionary<string, EntityProperty>> CompileWriteFunc(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties.Where((PropertyInfo p) => !TableEntity.ShouldSkipProperty(p, null)))
			{
				...
				if (Attribute.IsDefined(propertyInfo, typeof(EncryptPropertyAttribute)))
				{
					PropertyInfo propertyInfo2 = typeof(EntityProperty).FindProperty("IsEncrypted");
					list.Add(Expression.Assign(Expression.Property(parameterExpression3, propertyInfo2), Expression.Constant(true)));
				}
			...
			Expression<Func<object, OperationContext, Dictionary<string, EntityProperty>>> expression = Expression.Lambda<Func<object, OperationContext, Dictionary<string, EntityProperty>>>(Expression.Block(new ParameterExpression[] { parameterExpression5, parameterExpression3, parameterExpression4 }, list), new ParameterExpression[] { parameterExpression, parameterExpression2 });
		...
		private static Expression GeneratePropertyReadExpressionByType(Type type, PropertyInfo property, Expression instanceParam, Expression currentEntityProperty)
		{
			...
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.String)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_StringPI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(byte[]))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Binary)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_BinaryPI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(bool?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Boolean)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_BoolPI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(bool))
			{
				MethodInfo methodInfo = typeof(bool?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo2 = typeof(bool?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Boolean)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_BoolPI.FindGetProp()), methodInfo)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_BoolPI.FindGetProp()), methodInfo2) })));
			}
			if (property.PropertyType == typeof(DateTime?))
			{
				MethodInfo methodInfo3 = typeof(DateTimeOffset?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo4 = typeof(DateTimeOffset?).FindProperty("Value").FindGetProp();
				MethodInfo methodInfo5 = typeof(DateTimeOffset).FindProperty("UtcDateTime").FindGetProp();
				ParameterExpression parameterExpression = Expression.Variable(typeof(DateTime?), "tempVal");
				ConditionalExpression conditionalExpression = Expression.IfThenElse(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()), methodInfo3), Expression.Assign(parameterExpression, Expression.TypeAs(Expression.Call(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()), methodInfo4), methodInfo5), typeof(DateTime?))), Expression.Assign(parameterExpression, Expression.TypeAs(Expression.Constant(null), typeof(DateTime?))));
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.DateTime)), Expression.Block(new ParameterExpression[] { parameterExpression }, new Expression[]
				{
					conditionalExpression,
					Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { parameterExpression })
				}));
			}
			if (property.PropertyType == typeof(DateTime))
			{
				MethodInfo methodInfo6 = typeof(DateTimeOffset?).FindProperty("Value").FindGetProp();
				MethodInfo methodInfo7 = typeof(DateTimeOffset).FindProperty("UtcDateTime").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.DateTime)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()), methodInfo6), methodInfo7) }));
			}
			if (property.PropertyType == typeof(DateTimeOffset?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.DateTime)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(DateTimeOffset))
			{
				MethodInfo methodInfo8 = typeof(DateTimeOffset?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo9 = typeof(DateTimeOffset?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.DateTime)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()), methodInfo8)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DateTimeOffsetPI.FindGetProp()), methodInfo9) })));
			}
			if (property.PropertyType == typeof(double?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Double)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DoublePI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(double))
			{
				MethodInfo methodInfo10 = typeof(double?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo11 = typeof(double?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Double)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DoublePI.FindGetProp()), methodInfo10)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_DoublePI.FindGetProp()), methodInfo11) })));
			}
			if (property.PropertyType == typeof(Guid?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Guid)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_GuidPI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(Guid))
			{
				MethodInfo methodInfo12 = typeof(Guid?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo13 = typeof(Guid?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Guid)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_GuidPI.FindGetProp()), methodInfo12)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_GuidPI.FindGetProp()), methodInfo13) })));
			}
			if (property.PropertyType == typeof(int?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Int32)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int32PI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(int))
			{
				MethodInfo methodInfo14 = typeof(int?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo15 = typeof(int?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Int32)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int32PI.FindGetProp()), methodInfo14)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int32PI.FindGetProp()), methodInfo15) })));
			}
			if (property.PropertyType == typeof(long?))
			{
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Int64)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int64PI.FindGetProp()) }));
			}
			if (property.PropertyType == typeof(long))
			{
				MethodInfo methodInfo16 = typeof(long?).FindProperty("HasValue").FindGetProp();
				MethodInfo methodInfo17 = typeof(long?).FindProperty("Value").FindGetProp();
				return Expression.IfThen(Expression.Equal(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_PropTypeGetter), Expression.Constant(EdmType.Int64)), Expression.IfThen(Expression.IsTrue(Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int64PI.FindGetProp()), methodInfo16)), Expression.Call(Expression.Convert(instanceParam, type), property.FindSetProp(), new Expression[] { Expression.Call(Expression.Call(currentEntityProperty, TableEntity.EntityProperty_Int64PI.FindGetProp()), methodInfo17) })));
			}
			return null;
		}
		...
		private static EntityProperty GetValueByKeyFromDictionary(string key, IDictionary<string, EntityProperty> dict, OperationContext operationContext)
		{
			...
			if (entityProperty == null)
			{
				Logger.LogInformational(operationContext, "Omitting property '{0}' from de-serialization because there is no corresponding entry in the dictionary provided.", new object[] { key });
			}
