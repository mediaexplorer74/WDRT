...
namespace Microsoft.WindowsAzure.Storage.Table.Queryable
{
	...
	internal class ExpressionWriter : DataServiceALinqExpressionVisitor
	{
		...
		internal static string ExpressionToString(Expression e)
		{
			...
			{
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression {0} is not supported.", new object[] { e.ToString() }));
			}
			return text;
		}
		...
		internal override Expression VisitInputReferenceExpression(InputReferenceExpression ire)
		{
			if (this.parent == null || this.parent.NodeType != ExpressionType.MemberAccess)
			{
				...
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression {0} is not supported.", new object[] { text }));
			}
			return ire;
		}
		...
		internal override Expression VisitMemberAccess(MemberExpression m)
		{
			...
			{
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Referencing public field '{0}' not supported in query option expression.  Use public property instead.", new object[] { m.Member.Name }));
			}
		...
		internal override Expression VisitConstant(ConstantExpression c)
		{
			...
			if (!ClientConvert.TryKeyPrimitiveToString(c.Value, out text))
			{
				throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Could not convert constant {0} expression to string.", new object[] { c.Value }));
			}
			this.builder.Append(text);
			return c;
		}
		...
		private string TypeNameForUri(Type type)
		{
			...
			throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Can't cast to unsupported type '{0}'", new object[] { type.Name }));
		}
