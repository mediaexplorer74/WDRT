	...
	public sealed class QueueContinuationToken : IContinuationToken, IXmlSerializable
	{
		...
		private string Version
		{
			...
			set
			{
				...
				if (this.version != "2.0")
				{
					throw new XmlException(string.Format(CultureInfo.InvariantCulture, "Unexpected Element '{0}'", new object[] { this.version }));
				}
			}
		}
			...
			while (reader.IsStartElement())
			{
				...
				if ((name = reader.Name) != null)
				{
					...
					else
					{
						...
						if (Enum.TryParse<StorageLocation>(text, out storageLocation))
						{
							this.TargetLocation = new StorageLocation?(storageLocation);
							continue;
						}
						throw new XmlException(string.Format(CultureInfo.InvariantCulture, "Unexpected Location '{0}'", new object[] { text }));
					}
				}
				throw new XmlException(string.Format(CultureInfo.InvariantCulture, "Unexpected Element '{0}'", new object[] { reader.Name }));
			}
		}
