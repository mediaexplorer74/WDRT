...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public class TableEncryptionPolicy
	{
				...
				using (SHA256CryptoServiceProvider sha256CryptoServiceProvider = new SHA256CryptoServiceProvider())
				{
					...
					foreach (KeyValuePair<string, EntityProperty> keyValuePair in properties)
					{
						...
						if (keyValuePair.Value != null && keyValuePair.Value.IsEncrypted)
						{
							...
							byte[] array = sha256CryptoServiceProvider.ComputeHash(CommonUtility.BinaryAppend(encryptionData.ContentEncryptionIV, Encoding.UTF8.GetBytes(string.Join(partitionKey, new string[] { rowKey, keyValuePair.Key }))));
							...
							using (ICryptoTransform cryptoTransform = aesCryptoServiceProvider.CreateEncryptor())
							{
								...
								dictionary.Add(keyValuePair.Key, new EntityProperty(array2));
					...
					byte[] array3 = sha256CryptoServiceProvider.ComputeHash(CommonUtility.BinaryAppend(encryptionData.ContentEncryptionIV, Encoding.UTF8.GetBytes(string.Join(partitionKey, new string[] { rowKey, "_ClientEncryptionMetadata2" }))));
					Array.Resize<byte>(ref array3, 16);
					...
					using (ICryptoTransform cryptoTransform2 = aesCryptoServiceProvider.CreateEncryptor())
					{
						...
						byte[] array4 = cryptoTransform2.TransformFinalBlock(bytes2, 0, bytes2.Length);
						dictionary.Add("_ClientEncryptionMetadata2", new EntityProperty(array4));
					}
				}
			}
		...
		internal byte[] DecryptMetadataAndReturnCEK(string partitionKey, string rowKey, EntityProperty encryptionKeyProperty, EntityProperty propertyDetailsProperty, out EncryptionData encryptionData)
		{
			...
			try
			{
				...
				using (AesCryptoServiceProvider aesCryptoServiceProvider = new AesCryptoServiceProvider())
				{
					using (SHA256CryptoServiceProvider sha256CryptoServiceProvider = new SHA256CryptoServiceProvider())
					{
						byte[] array2 = sha256CryptoServiceProvider.ComputeHash(CommonUtility.BinaryAppend(encryptionData.ContentEncryptionIV, Encoding.UTF8.GetBytes(string.Join(partitionKey, new string[] { rowKey, "_ClientEncryptionMetadata2" }))));
			...
			{
				throw new StorageException("Error while de-serializing the encryption metadata string from the wire.", ex)
				{
					IsRetryable = false
				};
			}
			...
			{
				throw new StorageException("Decryption logic threw error. Please check the inner exception for more details.", ex2)
				{
					IsRetryable = false
				};
			}
		...
		{
			Dictionary<string, EntityProperty> dictionary2;
			try
			{
				Dictionary<string, EntityProperty> dictionary = new Dictionary<string, EntityProperty>();
						...
						foreach (KeyValuePair<string, EntityProperty> keyValuePair in properties)
						{
							if (!(keyValuePair.Key == "_ClientEncryptionMetadata1") && !(keyValuePair.Key == "_ClientEncryptionMetadata2"))
							{
								if (encryptedPropertyDetailsSet.Contains(keyValuePair.Key))
								{
									...
									using (ICryptoTransform cryptoTransform = aesCryptoServiceProvider.CreateDecryptor())
									{
										byte[] array2 = cryptoTransform.TransformFinalBlock(binaryValue, 0, binaryValue.Length);
