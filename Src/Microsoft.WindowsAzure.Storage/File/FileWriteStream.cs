...
namespace Microsoft.WindowsAzure.Storage.File
{
	...
	internal sealed class FileWriteStream : FileWriteStreamBase
	{
		...
		private void CommitFlushCallback(IAsyncResult ar)
		{
			...
			lock (storageAsyncResult.CancellationLockerObject)
			{
				...
				try
				{
					...
					{
						this.file.Properties.ContentMD5 = this.fileMD5.ComputeHash();
						ICancellableAsyncResult cancellableAsyncResult = this.file.BeginSetProperties(this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.SetPropertiesCallback), storageAsyncResult);
						storageAsyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
					}
					else
					{
		...
		private void DispatchWrite(StorageAsyncResult<NullType> asyncResult)
		{
			...
			if (this.rangeMD5 != null)
			{
				text = this.rangeMD5.ComputeHash();
			...
			this.WriteRange(internalBuffer, currentFileOffset, text, asyncResult);
		}
		...
		private void WriteRange(Stream rangeData, long offset, string contentMD5, StorageAsyncResult<NullType> asyncResult)
		{
			...
			this.parallelOperationSemaphore.WaitAsync(delegate(bool calledSynchronously)
			{
				try
				{
					ICancellableAsyncResult cancellableAsyncResult = this.file.BeginWriteRange(rangeData, offset, contentMD5, this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.WriteRangeCallback), null);
					if (asyncResult != null)
					{
