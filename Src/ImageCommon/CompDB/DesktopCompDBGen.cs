...
namespace Microsoft.WindowsPhone.CompDB
{
	...
	public class DesktopCompDBGen
	{
		...
		public static BuildCompDB GenerateCompDB(FMCollection fmCollection, 
			string fmDirectory, string msPackageRoot, string language, 
			string buildType, CpuId buildArch, string buildInfo, 
			IULogger logger, List<DesktopCompDBGen.DesktopFilters> filters, List<string> featureIDs, 
			bool generateHashes)
		{

            FMCollections f8_locals1 =
				new DesktopCompDBGen.FMCollections();//.f__DisplayClass57_0();

            //RnD
            f8_locals1.featureIDs = featureIDs;
			DesktopCompDBGen._generateHashes = generateHashes;
			if (!string.IsNullOrEmpty(fmCollection.Manifest.ChunkMappingsFile))
			{
				string chunkMappingFile = fmCollection.Manifest.GetChunkMappingFile
					(fmDirectory);
				if (LongPathFile.Exists(chunkMappingFile))
				{
					BuildCompDB.InitializeChunkMapping
						(chunkMappingFile, fmCollection.Manifest.SupportedLanguages, logger);
				}
			}
			
			//RnD
			f8_locals1.newCompDB = new BuildCompDB(logger);
			...
			//RnD
			f8_locals1.newCompDB.Product = fmCollection.Manifest.Product;
			f8_locals1.newCompDB.BuildArch = buildArch.ToString();
			List<string> list = new List<string>();
			List<FMCollectionItem> list2 = new List<FMCollectionItem>();
			if (string.IsNullOrEmpty(language))
			{
				list2.AddRange(from fm in fmCollection.Manifest.FMs
				where fm.ID.Equals(DesktopCompDBGen.c_MicrosoftDesktopEditionsFM_ID, 
				StringComparison.OrdinalIgnoreCase) 
				|| fm.ID.Equals(DesktopCompDBGen.c_MicrosoftDesktopToolsFM_ID, 
				StringComparison.OrdinalIgnoreCase)
				select fm);
			}
			...
			{
				list2.AddRange(from fm in fmCollection.Manifest.FMs
				where fm.ID.Equals(DesktopCompDBGen.c_MicrosoftDesktopNeutralFM_ID, 
				StringComparison.OrdinalIgnoreCase) || fm.ID.Equals(
					DesktopCompDBGen.c_MicorosftDesktopConditionalFeaturesFMName, 
					StringComparison.OrdinalIgnoreCase)
				select fm);
			}
			else
			{
				...
				list2.AddRange(from fm in fmCollection.Manifest.FMs
				where fm.ID.Equals(DesktopCompDBGen.c_MicrosoftDesktopLangsFM_ID, 
				StringComparison.OrdinalIgnoreCase)
				select fm);
			}
			...
			foreach (FMCollectionItem fmcollectionItem in list2)
			{
				if (buildArch == fmcollectionItem.CPUType || fmcollectionItem.CPUType == CpuId.Invalid)
				{
					...
					if (featureManifest.OwnerType == OwnerType.Microsoft 
						&& f8_locals1.newCompDB.BuildID == Guid.Empty 
						&& !string.IsNullOrEmpty(featureManifest.BuildID))
					{
						f8_locals1.newCompDB.BuildID = new Guid(featureManifest.BuildID);
                        f8_locals1.newCompDB.BuildInfo = featureManifest.BuildInfo;
                        f8_locals1.newCompDB.OSVersion = featureManifest.OSVersion;
					}
					List<FeatureManifest.FMPkgInfo> allPackageByGroups 
						= featureManifest.GetAllPackageByGroups(list, 
						fmCollection.Manifest.SupportedLocales, 
						fmCollection.Manifest.SupportedResolutions, buildType, 
						DesktopCompDBGen._buildArch, msPackageRoot);
					if (list.Any<string>())
					{
						foreach (FeatureManifest.FMPkgInfo fmpkgInfo in allPackageByGroups)
						{
							...
							{
								fmpkgInfo.PackagePath = fmpkgInfo.PackagePath.Replace(
									PkgFile.DefaultLanguagePattern + fmpkgInfo.Language, "", 
									StringComparison.OrdinalIgnoreCase);
								fmpkgInfo.ID = fmpkgInfo.ID.Replace(
									PkgFile.DefaultLanguagePattern + fmpkgInfo.Language, "", 
									StringComparison.OrdinalIgnoreCase);
							}
						}
					...
					if (filters != null && filters.Contains(
						DesktopCompDBGen.DesktopFilters.IncludeByFeatureIDs))
					{
						IEnumerable<string> source = list3;
						Func<string, bool> predicate;
						if ((predicate = f8_locals1.f9__4) == null)
						{
                            FMCollections f8_locals2 = f8_locals1;
							predicate = (f8_locals2.f9__4 = 
								((string feat) => f8_locals2.featureIDs.Contains(
									FMFeatures.GetFeatureIDWithoutPrefix(feat), 
					...
					using (List<string>.Enumerator enumerator3 = list3.GetEnumerator())
					{
						while (enumerator3.MoveNext())
						{
							...
							CompDBFeature compDBFeature = new CompDBFeature(feature, 
								fmcollectionItem.ID, CompDBFeature.CompDBFeatureTypes.MobileFeature, 
								(fmcollectionItem.ownerType == OwnerType.Microsoft) ? 
								fmcollectionItem.ownerType.ToString() : 
							...
							using (List<FeatureManifest.FMPkgInfo>.Enumerator enumerator2 = list4.GetEnumerator())
							{
								while (enumerator2.MoveNext())
								{
									...
									if (!f8_locals1.newCompDB.Packages.Any((CompDBPackageInfo pkg2) 
										=> pkg2.ID.Equals(pkg.ID, StringComparison.OrdinalIgnoreCase)))
									{
										...
										CompDBPackageInfo compDBPackageInfo =
											DesktopCompDBGen.GenerateFeaturePackage(compDBFeature, 
											pkg.ID, assemblyIdentity, pkg.PackagePath, language, msPackageRoot);
										if ((string.IsNullOrEmpty(compDBPackageInfo.VersionStr) 
											|| compDBPackageInfo.VersionStr.Equals("0.0.0.0")) 
											&& !string.IsNullOrEmpty(assemblyIdentity.version))
										{
											compDBPackageInfo.VersionStr = assemblyIdentity.version;
										}
										f8_locals1.newCompDB.Packages.Add(compDBPackageInfo);
									}
									else
									{
										CompDBFeaturePackage compDBFeaturePackage = 
											f8_locals1.newCompDB.Features.SelectMany(
												(CompDBFeature feat) => feat.Packages).FirstOrDefault
												((CompDBFeaturePackage featPkg) => 
												featPkg.ID.Equals(pkg.ID, StringComparison.OrdinalIgnoreCase));
										if (compDBFeaturePackage == null)
										{
											flag = true;
											stringBuilder.AppendLine("Package '" + pkg.ID + 
												"' found in Packages but not in any features.");
										}
										else
										{
							...
							compDBFeature.Type = DesktopCompDBGen.GetFeatureTypeFromGrouping(
								featureManifest.Features.MSFeatureGroups, compDBFeature.FeatureID);
							f8_locals1.newCompDB.Features.Add(compDBFeature);
						}
					}
					if (featureManifest.Features != null && featureManifest.Features.MSConditionalFeatures != null 
						&& featureManifest.Features.MSConditionalFeatures.Count<FMConditionalFeature>() != 0)
					{
						if (f8_locals1.newCompDB.MSConditionalFeatures == null)
						{
							f8_locals1.newCompDB.MSConditionalFeatures = new List<FMConditionalFeature>();
						}

						f8_locals1.newCompDB.MSConditionalFeatures.AddRange(
							featureManifest.Features.MSConditionalFeatures);
					}
				}
			}
			if (flag)
			{
				throw new ImageCommonException("ImageCommon::DesktopCompDBGen!GenerateCompDB: " +
					"Errors processing FM File(s):\n" + stringBuilder.ToString());
			}
			
			f8_locals1.newCompDB.Packages = f8_locals1.newCompDB.Packages.Distinct(
				CompDBPackageInfoComparer.Standard).ToList<CompDBPackageInfo>();

			f8_locals1.newCompDB.Packages = (from pkg in f8_locals1.newCompDB.Packages
			select pkg.SetParentDB(f8_locals1.newCompDB)).ToList<CompDBPackageInfo>();
			if (f8_locals1.newCompDB.BuildID == Guid.Empty)
			{
				f8_locals1.newCompDB.BuildID = Guid.NewGuid();
				f8_locals1.newCompDB.BuildInfo = buildInfo;
			}
			
			return f8_locals1.newCompDB;
		}


		// Token: 0x06000096 RID: 150 RVA: 0x0000647C File Offset: 0x0000467C
		private static CompDBFeature.CompDBFeatureTypes GetFeatureTypeFromGrouping
			(List<FMFeatureGrouping> groupings, string feature)
		{
			...
			{
				FMFeatureGrouping fmfeatureGrouping = groupings.FirstOrDefault(
					(FMFeatureGrouping grp) => grp.FeatureIDs.Contains(
						feature, StringComparer.OrdinalIgnoreCase) 
					&& !string.IsNullOrEmpty(grp.GroupingType));

				if (fmfeatureGrouping != null && 
					!Enum.TryParse<CompDBFeature.CompDBFeatureTypes>(fmfeatureGrouping.GroupingType, out result))
				{
		...
		{
            LangPacks f8__locals1 = new DesktopCompDBGen.LangPacks();//<>c__DisplayClass59_0();
			...
			{
				fmcollectionManifest.ChunkMappingsFile = FMCollection.c_FMDirectoryVariable + 
					"\\" + DesktopCompDBGen.c_MicorosftDesktopChunksMappingFileName;
			}
			string text = null;
			...
			if (!string.IsNullOrEmpty(conditionalFeaturesFMFile) && LongPathFile.Exists(conditionalFeaturesFMFile))
			{
				...
				FeatureManifest.ValidateAndLoad(ref featureManifest, conditionalFeaturesFMFile,
					DesktopCompDBGen._logger);
			...
			DesktopCompDBGen.InitializeFMItem(ref fmcollectionItem, ref featureManifest2, 
				DesktopCompDBGen.c_MicrosoftDesktopNeutralFM_ID, DesktopCompDBGen.c_MicrosoftDesktopNeutralFM, cpuId, fmDirectory, buildID.ToString(), buildInfo);
			fmcollectionManifest.FMs.Add(fmcollectionItem);
			fmcollectionItem = new FMCollectionItem();
			FeatureManifest featureManifest3 = new FeatureManifest();
			DesktopCompDBGen.InitializeFMItem(ref fmcollectionItem, ref featureManifest3, 
				DesktopCompDBGen.c_MicrosoftDesktopLangsFM_ID, DesktopCompDBGen.c_MicrosoftDesktopLangsFM, 
				cpuId, fmDirectory, buildID.ToString(), buildInfo);
			fmcollectionManifest.FMs.Add(fmcollectionItem);
			fmcollectionItem = new FMCollectionItem();
			FeatureManifest featureManifest4 = new FeatureManifest();
			DesktopCompDBGen.InitializeFMItem(ref fmcollectionItem, ref featureManifest4, 
				DesktopCompDBGen.c_MicrosoftDesktopEditionsFM_ID, 
				DesktopCompDBGen.c_MicrosoftDesktopEditionsFM, cpuId, fmDirectory, 
				buildID.ToString(), buildInfo);
			fmcollectionManifest.FMs.Add(fmcollectionItem);
			fmcollectionItem = new FMCollectionItem();
			FeatureManifest featureManifest5 = new FeatureManifest();
			DesktopCompDBGen.InitializeFMItem(ref fmcollectionItem, ref featureManifest5, 
				DesktopCompDBGen.c_MicrosoftDesktopToolsFM_ID, 
				DesktopCompDBGen.c_MicrosoftDesktopToolsFM, cpuId, fmDirectory, 
				buildID.ToString(), buildInfo);
			fmcollectionManifest.FMs.Add(fmcollectionItem);
			...
			foreach (string lang in langs)
			{
				...
				DesktopCompDBGen._pkgMap = new Dictionary<string, 
					CompDBPackageInfo>(StringComparer.OrdinalIgnoreCase);
				DesktopCompDBGen._featurePkgMap = 
					new Dictionary<CompDBFeature, List<CompDBFeaturePackage>>();
				DesktopCompDBGen._clientEditionsFeatures = 
					new Dictionary<DesktopCompDBGen.ClientTypes, List<CompDBFeature>>();
			}
			BuildCompDB srcDB = DesktopCompDBGen.GenerateCompDBForOptionalPkgs
				(DesktopCompDBGen.c_NeutralLanguage, buildID, buildInfo);
			...
			{
				featureManifest.OSVersion = 
					DesktopCompDBGen._editionVersionLookup[DesktopCompDBGen.ClientTypes.Client];
			...
			f8__locals1.langPkgIDs = 
				(from pkg4 in list.SelectMany((CompDBFeature feat) => feat.Packages)
			select pkg4.ID).Distinct(StringComparer.OrdinalIgnoreCase).ToList<string>();
			
			(from pkg4 in buildCompDB.Packages
			where f8__locals1.langPkgIDs.Contains(pkg4.ID, 
			StringComparer.OrdinalIgnoreCase)
			select pkg4).ToList<CompDBPackageInfo>();
			...
			List<CompDBPackageInfo> source = 
			(
			from pkg2 in buildCompDB.Packages
			where pkg2.SatelliteType == CompDBPackageInfo.SatelliteTypes.Language 
			&& !f8__locals1.langPkgIDs.Contains(pkg2.ID, StringComparer.OrdinalIgnoreCase)
			select pkg2
			).ToList<CompDBPackageInfo>();
			...
			if (source.Any<CompDBPackageInfo>())
			{
				...
				compDBFeature.Packages = (from pkg3 in source
				select new CompDBFeaturePackage(pkg3.ID, false))
				.ToList<CompDBFeaturePackage>();

				list.Add(compDBFeature);
				
				f8__locals1.langPkgIDs.AddRange(from pkg3 in compDBFeature.Packages
				select pkg3.ID);
			}
			IEnumerable<CompDBPackageInfo> packages = buildCompDB.Packages;
			Func<CompDBPackageInfo, bool> predicate;
			if ((predicate = (Func<CompDBPackageInfo, bool>)f8__locals1.f9__8) == null)
			{
				var f8__locals2 = f8__locals1;
				predicate = ((Func<CompDBPackageInfo, bool>)(f8__locals2.f9__8 = 
					((CompDBPackageInfo pkg) => f8__locals2.langPkgIDs.Contains(
						pkg.ID, StringComparer.OrdinalIgnoreCase))));
			}
			...
			using (IEnumerator<CompDBPackageInfo> enumerator2 = packages.Where(predicate).GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					CompDBPackageInfo pkg1 = enumerator2.Current;
					Func<CompDBFeaturePackage, bool> f9__10 = default;
					List<string> featureIDs = 
						(from feat in list.Where(delegate(CompDBFeature feat)
					{
						IEnumerable<CompDBFeaturePackage> packages4 = feat.Packages;
						Func<CompDBFeaturePackage, bool> predicate4;
						if ((predicate4 = f9__10) == null)
						{
							predicate4 = (f9__10 = ((CompDBFeaturePackage featPkg) => 
							featPkg.ID.Equals(pkg1.ID, StringComparison.OrdinalIgnoreCase)));
						}
					...
					msoptionalPkgFile.Language = "(" + pkg1.SatelliteValue + ")";
					msoptionalPkgFile.NoBasePackage = true;
					msoptionalPkgFile.ID = pkg1.ID;
					string path = pkg1.FirstPayloadItem.Path;
					msoptionalPkgFile.Directory = "$(mspackageroot)\\" 
						+ LongPath.GetDirectoryName(path);
					msoptionalPkgFile.Name = LongPath.GetFileName(path);
					msoptionalPkgFile.Version = pkg1.VersionStr;
					featureManifest3.Features.Microsoft.Add(msoptionalPkgFile);
				}
			}
			
			string fileName2 = Path.Combine(fmDirectory, 
				DesktopCompDBGen.c_MicrosoftDesktopLangsFM);
			...
			select feat).ToList<CompDBFeature>();
			f8__locals1.neutralPkgIDs = (from pkg1 in list2.SelectMany(
			(CompDBFeature feat) => feat.Packages) 	
			select pkg1.ID).Distinct(StringComparer.OrdinalIgnoreCase).ToList<string>();
			
			(from pkg1 in buildCompDB.Packages
			where f8__locals1.neutralPkgIDs.Contains(pkg1.ID, 
				StringComparer.OrdinalIgnoreCase)
			select pkg1).ToList<CompDBPackageInfo>();
			...
			List<CompDBPackageInfo> source2 = (from pkg1 in buildCompDB.Packages
			where pkg1.SatelliteType != CompDBPackageInfo.SatelliteTypes.Language && 
			!f8__locals1.neutralPkgIDs.Contains(pkg1.ID, StringComparer.OrdinalIgnoreCase)
			select pkg1).Distinct<CompDBPackageInfo>().ToList<CompDBPackageInfo>();
			...
			if (source2.Any<CompDBPackageInfo>())
			{
				...
				select new CompDBFeaturePackage(pkg.ID, false))
				.ToList<CompDBFeaturePackage>();
				
				list2.Add(compDBFeature2);
				f8__locals1.neutralPkgIDs.AddRange(from pkg in compDBFeature2.Packages
				select pkg.ID);
			}
			IEnumerable<CompDBPackageInfo> packages2 = buildCompDB.Packages;
			Func<CompDBPackageInfo, bool> predicate2;
			if ((predicate2 = (Func<CompDBPackageInfo, bool>)f8__locals1.f9__20) == null)
			{
                LangPacks f8__locals4 = f8__locals1;

				predicate2 = 
					((Func<CompDBPackageInfo, bool>)(f8__locals4.f9__20 = 
					((CompDBPackageInfo pkg) =>
				f8__locals4.neutralPkgIDs.Contains(pkg.ID, 
				StringComparer.OrdinalIgnoreCase))));
			}
			...
			f8__locals1.editionPkgIDs = 
				(from pkg in list3.SelectMany((CompDBFeature feat) => feat.Packages)
			    select pkg.ID).Distinct(StringComparer.OrdinalIgnoreCase)
				.ToList<string>();
			...
			if ((predicate3 = f8__locals1.f9__28) == null)
			{
                LangPacks f8__locals6 = f8__locals1;

				predicate3 = (f8__locals6.f9__28 = ((CompDBPackageInfo pkg) =>
				f8__locals6.editionPkgIDs.Contains(pkg.ID, 
				StringComparer.OrdinalIgnoreCase)));
			}
			using (IEnumerator<CompDBPackageInfo> enumerator2 = packages3.Where(predicate3).GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					...
					Func<CompDBFeaturePackage, bool> f9__30 = default;
					
					List<string> featureIDs3 = ( from feat in 
					list3.Where(delegate(CompDBFeature feat )
					{
						IEnumerable<CompDBFeaturePackage> packages4 = feat.Packages;
						Func<CompDBFeaturePackage, bool> predicate4;
						if ((predicate4 = f9__30) == null)
						{
							predicate4 = (f9__30 = (
							(CompDBFeaturePackage featPkg) => 
							featPkg.ID.Equals(pkg.ID, 
							StringComparison.OrdinalIgnoreCase)));
						}
						return packages4.Any(predicate4);
					})
			...
			foreach (CompDBFeature compDBFeature3 in list3)
			{
				...
				msoptionalPkgFile4.Version = 
					DesktopCompDBGen._editionVersionLookup[DesktopCompDBGen.GetEditionFeatureClientType(
						compDBFeature3, buildCompDB.Packages)];
			...
			if (!string.IsNullOrEmpty(fmcollectionManifest.ChunkMappingsFile))
			{
				...
				CompDBChunkMapping compDBChunkMapping = CompDBChunkMapping.ValidateAndLoad(
					chunkMappingFile, fmcollectionManifest.SupportedLanguages, DesktopCompDBGen._logger);
				compDBChunkMapping.ChunkMappings.RemoveAll((CompDBChunkMapItem map) => 
				map.Type == DesktopCompDBGen.PackageTypes.ESD);
		...
		private static DesktopCompDBGen.ClientTypes GetEditionFeatureClientType(
			CompDBFeature editionFeature, List<CompDBPackageInfo> packages)
		{
			List<string> featurePkgIDs = (from pkg in editionFeature.Packages
			select pkg.ID).ToList<string>();
			if (packages.Any((CompDBPackageInfo pkg) => featurePkgIDs.Contains(pkg.ID, 
				StringComparer.OrdinalIgnoreCase) && pkg.FirstPayloadItem.Path.ToUpper(
					CultureInfo.InvariantCulture).Contains(
					DesktopCompDBGen.c_ClientNEditions.ToUpper(CultureInfo.InvariantCulture))))
			{
		...
		private static void InitializeFMItem(ref FMCollectionItem fmItem, 
			ref FeatureManifest fm, string FMID, string fmFile, 
			CpuId cpuType, string fmDirectory, string buildID, string buildInfo)
		{
		...
		public static BuildCompDB GenerateCompDBForOptionalPkgs(string lang, 
			Guid buildID, string buildInfo)
		{
			if (!DesktopCompDBGen._bInitialized)
			{
				throw new Exception("ImageCommon::DesktopCompDBGen!GenerateCompDB: " +
					"Function called before being initialized.");
			}
			...
			try
			{
				...
				DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
					"Processing FOD ({0} and {1})...", new object[]
				{
					DesktopCompDBGen.c_NeutralDirectory,
					lang
				});
				DesktopCompDBGen.GenerateCompDBForRemainingOptionalPkgs(
					Path.GetFullPath(Path.Combine(DesktopCompDBGen._pkgRoot, 
					DesktopCompDBGen.c_FODDirectory, DesktopCompDBGen.c_NeutralDirectory, 
					DesktopCompDBGen.c_FODCabs)), DesktopCompDBGen.PackageTypes.FOD, 
					lang, DesktopCompDBGen._pkgRoot);
		...
		public static BuildCompDB GenerateCompDB(string lang, Guid buildID, string buildInfo)
		{
			...
			try
			{
				string path = LongPath.Combine(DesktopCompDBGen._pkgRoot, 
					DesktopCompDBGen.c_DesktopUpdatesDirectory);
				string text = LongPath.Combine(path, DesktopCompDBGen.c_EditionPackagesDirectory);
				DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
					"Processing All Editions ({0} and {1})...", new object[]
				{
					DesktopCompDBGen.c_NeutralDirectory,
					lang
				});
				DesktopCompDBGen.CompDBForAllEditions(LongPath.Combine(text, 
					DesktopCompDBGen.c_NeutralDirectory), lang, DesktopCompDBGen._pkgRoot);
				...
				string editionsMap = LongPath.Combine(DesktopCompDBGen._sdxRoot,
					"data\\DistribData\\mc\\FeaturesOnDemand\\" + 
					DesktopCompDBGen.c_FODEditionsMapPattern.Replace(DesktopCompDBGen.c_BuildArchVar, 
					DesktopCompDBGen._buildArch, StringComparison.OrdinalIgnoreCase));
				DesktopCompDBGen.GenerateCompDB(Path.GetFullPath(path2), DesktopCompDBGen.PackageTypes.FOD,
					lang, DesktopCompDBGen._pkgRoot, editionsMap);
				string text2 = LongPath.Combine(path, DesktopCompDBGen.c_AppsDirectory);
				if (Directory.Exists(text2))
				{
					DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
						"Processing APPS...", new object[0]);
					editionsMap = LongPath.Combine(text2, DesktopCompDBGen.c_AppsEditionsMap);
					DesktopCompDBGen.GenerateCompDB(text2, DesktopCompDBGen.PackageTypes.APP, 
						lang, DesktopCompDBGen._pkgRoot, editionsMap);
				}
				buildCompDB.Features = DesktopCompDBGen.CompDBFeatures();
				buildCompDB.Packages = DesktopCompDBGen.CompDBPackages();
		...
		{
			return string.Format("{0}_{1}{2}", ai.processorArchitecture, ai.name, 
				ai.language.Equals("Neutral", StringComparison.OrdinalIgnoreCase) ? "" : ("_" + ai.language));
		}
		...
		private static List<CompDBFeature> CompDBFeatures()
		{
			...
			DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
				"Generated {0} DesktopCompDB Features", new object[]
			{
				list.Count
			});
			return list;
		...
		{
			DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
				"Generated {0} DesktopCompDB Packages", new object[]
			{
		...
		private static void CompDBForAllEditions(string pkgPath, string lang, string pkgRoot)
		{
			...
			{
				throw new DirectoryNotFoundException("ImageCommon::DesktopCompDBGen!CompDBForAllEditions: " +
					"Invalid package path:" + pkgPath);
			}
			...
			XDocument xdocument = XDocument.Load(LongPath.Combine(DesktopCompDBGen._sdxRoot, 
				DesktopCompDBGen.c_WindowsEditionsMap));
			XNamespace RNS = "urn:schemas-microsoft-com:windows:editions:v1";
			using (Dictionary<string, List<string>>.KeyCollection.Enumerator 
				enumerator = dictionary.Keys.GetEnumerator())
			{
				Func<XElement, string> f9__1 = default;
				while (enumerator.MoveNext())
				{
					string edition = enumerator.Current;
					IEnumerable<XElement> source = from e1 in xdocument.Root.Elements(RNS + "Edition")
					where e1.Element(RNS + "Name").Value.ToString().Equals(edition, 
					StringComparison.InvariantCultureIgnoreCase) && e1.Element(RNS + "Media") != null 
					&& (e1.Attribute(RNS + "legacy") == null || !e1.Attribute(RNS + "legacy").Value.Equals("true"))
					select e1;
					Func<XElement, string> selector;
					if ((selector = f9__1) == null)
					{
						selector = (f9__1 = ((XElement e1) => e1.Element(RNS + "Media").Value));
					}
					...
					if (Enum.TryParse<DesktopCompDBGen.ClientTypes>(text, out key))
					{
						...
						using (List<string>.Enumerator enumerator2 = dictionary[edition].GetEnumerator())
						{
							while (enumerator2.MoveNext())
							{
								...
								{
									AssemblyIdentity assemblyIdMum = 
										DesktopCompDBGen.GetAssemblyIdMum(
											Path.ChangeExtension(text2, PkgConstants.c_strMumExtension));
									DesktopCompDBGen._editionVersionLookup[key] = assemblyIdMum.version;
									CompDBFeature item = DesktopCompDBGen.GenerateFeaturePackageMapping(
										featureId, assemblyIdMum, text2, lang, pkgRoot, 
										DesktopCompDBGen.PackageTypes.FOD, null, false);
					...
					throw new ImageCommonException("ImageCommon::DesktopCompDBGen!CompDBForAllEditions: " +
						"Unknown Client Type: " + text);
				}
			}
		}
		...
		private static void CompDBForOtherPackages(string pkgPathRoot, 
			DesktopCompDBGen.PackageTypes pkgType, string lang, string pkgRoot)
		{
			List<string> list = new List<string>();
			...
			if (DesktopCompDBGen._generatingFMs && pkgType == 
				DesktopCompDBGen.PackageTypes.LANGPACK
				&& !lang.Equals("en-us", StringComparison.OrdinalIgnoreCase))
			{
			...
			foreach (string str in list)
			{
				foreach (string text in Directory.GetFiles(path, "*" + str))
				{
					...
					{
						DesktopCompDBGen.GetAssemblyIdAndFeatureIdFromCab(text3, 
							out assemblyIdentity, out text2);
					...
					DesktopCompDBGen.GenerateFeaturePackageMapping(DesktopCompDBGen.ClientTypes.Client 
						+ "_" + lang, assemblyIdentity, text3, lang, pkgRoot, pkgType, text2, false);
					DesktopCompDBGen.GenerateFeaturePackageMapping(DesktopCompDBGen.ClientTypes.ClientN
						+ "_" + lang, assemblyIdentity, text3, lang, pkgRoot, 
						DesktopCompDBGen.PackageTypes.FOD, null, false);
				}
			}
		}
		...
		private static void GenerateCompDB(string pkgPath, DesktopCompDBGen.PackageTypes pkgType,
			string lang, string pkgRoot, string editionsMap)
		{
			if (string.IsNullOrEmpty(editionsMap) || !LongPathFile.Exists(editionsMap))
			{
				throw new ImageCommonException("ImageCommon::DesktopCompDBGen!GenerateCompDB: " +
					"Invalid editions map specified: " + editionsMap);
			}
			...
			foreach (string text in dictionary.Keys)
			{
				foreach (string file in dictionary[text])
				{
					...
					if (LongPathFile.Exists(text2))
					{
						...
						{
							DesktopCompDBGen.GetAssemblyIdAndFeatureIdFromCab(text2, 
								out ai, out optionalFeatureId);
						}
						else
						{
							ai = DesktopCompDBGen.GetAssemblyIdFromPkg(text2);
						}
						DesktopCompDBGen.GenerateFeaturePackageMapping(text + "_" + lang, 
							ai, text2, lang, pkgRoot, pkgType, optionalFeatureId, false);
		...
		private static void GenerateCompDBForRemainingOptionalPkgs(string pkgPath, 
			DesktopCompDBGen.PackageTypes pkgType, string lang, string pkgRoot)
		{
			foreach (string pkgPath2 in Directory.GetFiles(pkgPath, "*" + DesktopCompDBGen.c_CabExtension))
			{
				if (!DesktopCompDBGen._processedOptionalPkgs.Contains(pkgPath))
				{
					...
					if (pkgType == DesktopCompDBGen.PackageTypes.FOD)
					{
						...
						{
							DesktopCompDBGen.GenerateFeaturePackageMapping(string.Empty, ai, 
								pkgPath2, lang, pkgRoot, pkgType, text, true);
						}
					}
				}
			}
		...
		private static Dictionary<string, List<string>> GenerateEditionsMap
			(string lang, string editionsPkgMap)
		{
            DesktopCompDBGen.EditionMaps f8__locals1 =
				new DesktopCompDBGen.EditionMaps();//<>c__DisplayClass71_0();

			f8__locals1.lang = lang;
			XDocument xdocument = XDocument.Load(editionsPkgMap);
			Dictionary<string, List<string>> dictionary = new Dictionary<string, 
				List<string>>(StringComparer.OrdinalIgnoreCase);
			Dictionary<string, HashSet<string>> dictionary2 =
				new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
			if (DesktopCompDBGen._clientEditionsFeatures.Count<KeyValuePair<DesktopCompDBGen.ClientTypes, 
				List<CompDBFeature>>>() != 0)
			{
				foreach (KeyValuePair<DesktopCompDBGen.ClientTypes, 
					List<CompDBFeature>> keyValuePair in DesktopCompDBGen._clientEditionsFeatures)
				{
					...
					{
						string text = 
							compDBFeature.FeatureID.Replace("_" + f8__locals1.lang, 
							"", StringComparison.OrdinalIgnoreCase);
			...
			XDocument xdocument2 = 
				XDocument.Load(LongPath.Combine(DesktopCompDBGen._sdxRoot, 
				DesktopCompDBGen.c_WindowsEditionsMap));
			
			f8__locals1.RNS = "urn:schemas-microsoft-com:windows:editions:v1";
			f8__locals1.media = (from e1 in xdocument2.Root.Elements(f8__locals1.RNS 
								 + "Edition")
			where e1.Element(f8__locals1.RNS + "Media") != null 
			&& (e1.Attribute(f8__locals1.RNS + "legacy") == null 
			|| !e1.Attribute(f8__locals1.RNS + "legacy").Value.Equals("true"))
			select e1.Element(f8__locals1.RNS + "Media").Value).Distinct<string>()
			.ToList<string>();
			
			foreach (XElement xelement in from p in xdocument.Root.Elements("Product")
			where f8__locals1.media.Contains(p.Attribute("name").Value.ToString())
			select p)
			{
				...
				foreach (XElement xelement2 in xelement.Elements("Edition"))
				{
					...
					foreach (XElement xelement3 in xelement2.Elements("Package"))
					{
						...
						if ((predicate = f8__locals1.f9__3) == null)
						{
							EditionMaps f8__locals2 = f8__locals1;
							predicate = (f8__locals2.f9__3 = 
								((XElement x) 
								=> x.Value.ToString().Equals(f8__locals2.lang) 
								|| x.Value.ToString().Equals("*")));
						}
					...
					if (list2.Count<string>() > 0)
					{
						...
						bool flag = false;
						if (xelement2.Attribute("ignoreAllSection") != null && 
							xelement2.Attribute("ignoreAllSection").Value.Equals(
						...
						else
						{
							if (dictionary.ContainsKey(value2))
							{
								...
								DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
									"Adding to editionsMap. Edition: {0}, Num of packages: {1}", new object[]
								{
									value2,
									list2.Count
								});
		...
		public static AssemblyIdentity GetAssemblyIdFromPkg(string pkgPath)
		{
			if (!DesktopCompDBGen._bInitialized)
			{
				throw new Exception("ImageCommon::DesktopCompDBGen!GetAssemblyIdFromPkg: " +
					"Function called before being initialized.");
			}
			if (Path.GetExtension(pkgPath).Equals(DesktopCompDBGen.c_WimExtenstion, 
				StringComparison.OrdinalIgnoreCase) || 
				Path.GetExtension(pkgPath).Equals(DesktopCompDBGen.c_EsdExtenstion, 
				StringComparison.OrdinalIgnoreCase))
			{
				return DesktopCompDBGen.GetAssemblyIdFromWim(pkgPath);
			}
			if (Path.GetExtension(pkgPath).Equals(DesktopCompDBGen.c_CabExtension, 
				StringComparison.OrdinalIgnoreCase))
			{
				return DesktopCompDBGen.GetAssemblyIdFromCab(pkgPath);
			}
			throw new ImageCommonException("ImageCommon::DesktopCompDBGen!GetAssemblyIdFromPkg: " +
				"Invalid package type:" + Path.GetExtension(pkgPath));
		}
		...
		private static AssemblyIdentity GetAssemblyIdFromWim(string pkgPath)
		{
			...
			process.StartInfo.FileName = LongPath.Combine(DesktopCompDBGen._sdxRoot, 
				"tools\\PostBuildScripts\\DistribData\\esd\\tools\\x64\\esdtool.exe");
			...
			DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
				"Calling ESDTool with arguments: " + process.StartInfo.Arguments, new object[0]);
			process.Start();
			...
			{
				DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
					"EsdTool_exit_code: {0}", new object[]
				{
		...
		private static void GetAssemblyIdAndFeatureIdFromCab(string pkgPath, 
			out AssemblyIdentity ai, out string featureId)
		{
		...
		private static string GetMumFileFromCab(string pkgPath)
		{
			...
			process.StartInfo.RedirectStandardOutput = true;
			DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
				"Calling Expand.exe with arguments: " + process.StartInfo.Arguments, new object[0]);
			process.Start();
			DesktopCompDBGen._logger.LogInfo(process.StandardOutput.ReadToEnd(), new object[0]);
			process.WaitForExit();
			DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
				"Expand.exe ExitCode: {0}", new object[]
			{
				process.ExitCode.ToString()
			});
			if (process.ExitCode != 0 || !LongPathFile.Exists(text))
			{
				throw new ImageCommonException("ImageCommon::DesktopCompDBGen!GetMumFileFromCab: " +
					"Unable to extract update.mum from :" + pkgPath);
			}
			return text;
		}
		...
		private static AssemblyIdentity GetAssemblyIdMum(string mumPath)
		{
			...
			XNamespace @namespace = xdocument.Root.Name.Namespace;
			return DesktopCompDBGen.GetAssemblyIdentity(
				xdocument.Root.Element(@namespace + "assemblyIdentity"));
		}
		...
		private static string GetFeatureIdFromMum(string mumPath)
		{
			...
			bool flag2 = xelement.Element(@namespace + "parent") != null 
				&& xelement.Element(@namespace + "parent").Element(@namespace + 
				"assemblyIdentity").Attribute("name").Value.ToString().Contains("NanoServer-Edition");
			
			if (flag && !flag2)
			{
				XElement xelement2 = xdocument.Root.Element(
					@namespace + "package").Element(@namespace + "declareCapability").Element(
					@namespace + "capability").Element(@namespace + "capabilityIdentity");
				...
				return value + "~" + (string.IsNullOrEmpty(text) ? "" : text) 
		...
		{
			return (AssemblyIdentity)new XmlSerializer(
				typeof(AssemblyIdentity)).Deserialize(assemblyIdentity.CreateReader());
		}


		// Token: 0x060000AC RID: 172 RVA: 0x00008934 File Offset: 0x00006B34
		public static CompDBPackageInfo GenerateFeaturePackage(
			CompDBFeature feature, AssemblyIdentity ai, string pkgPath, string lang, string pkgRoot)
		{
		...
		public static CompDBPackageInfo GenerateFeaturePackage(
			CompDBFeature feature, string id, AssemblyIdentity ai, string pkgPath, 
			string lang, string pkgRoot)
		{
			...
			if (feature.Packages.Any((CompDBFeaturePackage pkg) => 
			pkg.ID.Equals(featurePackage.ID, StringComparison.OrdinalIgnoreCase)))
			{
				return null;
			}
			if (!ai.processorArchitecture.Equals(DesktopCompDBGen._buildArch,
				StringComparison.OrdinalIgnoreCase))
			{
				compDBPackageInfo.BuildArchOverride = ai.processorArchitecture;
			}
			if (!string.IsNullOrEmpty(ai.language) && !ai.language.Equals(
				"neutral", StringComparison.OrdinalIgnoreCase))
			{
			...
			CompDBPayloadInfo compDBPayloadInfo = new CompDBPayloadInfo(pkgPath, pkgRoot, 
				compDBPackageInfo, false);
			...
			if (pkgPath.ToUpper(CultureInfo.InvariantCulture).Contains(
				DesktopCompDBGen.c_ESDRoot.ToUpper(CultureInfo.InvariantCulture)))
			{
			...
			foreach (CompDBPackageInfo compDBPackageInfo in srcCompDB.Packages)
			{
				foreach (CompDBPayloadInfo compDBPayloadInfo in compDBPackageInfo.Payload)
				{
					...
					if (!string.IsNullOrEmpty(compDBPayloadInfo.PayloadHash) && 
						!DesktopCompDBGen._pkgHashs.ContainsKey(key))
					{
						DesktopCompDBGen._pkgHashs.Add(key, compDBPayloadInfo.PayloadHash);
					}
		...
		private static CompDBFeature GenerateFeaturePackageMapping(string featureId, 
			AssemblyIdentity ai, string pkgPath, string lang, string pkgRoot, 
			DesktopCompDBGen.PackageTypes pkgType = DesktopCompDBGen.PackageTypes.FOD, 
			string optionalFeatureId = null, bool optionalFeatureOnly = false)
		{
			
			var f8__locals1 = 
				new DesktopCompDBGen.FPMappings();

            f8__locals1.featureId = featureId;
			f8__locals1.optionalFeatureId = optionalFeatureId;

			CompDBFeature compDBFeature = new CompDBFeature();

			CompDBPackageInfo compDBPackageInfo = 
				DesktopCompDBGen.GenerateFeaturePackage
				(compDBFeature, ai, pkgPath, lang, pkgRoot);

            f8__locals1.featurePackage = compDBFeature.FindPackage(compDBPackageInfo.ID);
			if (!optionalFeatureOnly)
			{
				DesktopCompDBGen.ClientTypes key;
				if (Enum.TryParse<DesktopCompDBGen.ClientTypes>
					(f8__locals1.featureId.Replace("_" + lang, "", 
					StringComparison.OrdinalIgnoreCase), out key))
				{
					if (DesktopCompDBGen._clientEditionsFeatures[key] == null)
					{
						throw new ImageCommonException
							("ImageCommon::DesktopCompDBGen!GenerateFeaturePackageMapping: The client '" 
							+ key.ToString() + "' was not found.  " +
							"Ensure all Editions are processed before doing feature mapping for FOD\\Apps.");
					}
					using (List<CompDBFeature>.Enumerator enumerator = 
						DesktopCompDBGen._clientEditionsFeatures[key].GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							if ((predicate = f8__locals1.f9__0) == null)
							{
								//DesktopCompDBGen.f_DisplayClass83_0 
							    var f8__locals2 = 
									f8__locals1;
								predicate = (f8__locals2.f9__0 = 
									((CompDBFeaturePackage pkg) => 
									pkg.ID.Equals(f8__locals2.featurePackage.ID,
									StringComparison.OrdinalIgnoreCase)));
							}
							if (!source.Any(predicate))
							{
								DesktopCompDBGen._featurePkgMap[key2].Add(
									f8__locals1.featurePackage);
							}
						}
						goto IL_1A9;
					}
				...
				CompDBFeature compDBFeature2 = 
					DesktopCompDBGen._featurePkgMap.Keys.FirstOrDefault(
						(CompDBFeature feat) => feat.FeatureID.Equals(
							f8__locals1.featureId, StringComparison.OrdinalIgnoreCase));
				if (compDBFeature2 == null)
				{
					CompDBFeature compDBFeature3 = new CompDBFeature
						(f8__locals1.featureId, null, CompDBFeature.CompDBFeatureTypes.DesktopMedia, 
						OwnerType.Microsoft.ToString());
					Dictionary<CompDBFeature, List<CompDBFeaturePackage>> featurePkgMap 
						= DesktopCompDBGen._featurePkgMap;
				...
				if (!DesktopCompDBGen._featurePkgMap[compDBFeature].Any((CompDBFeaturePackage pkg)
					=> pkg.ID.Equals(f8__locals1.featurePackage.ID, StringComparison.OrdinalIgnoreCase)))
				{
					DesktopCompDBGen._featurePkgMap[compDBFeature].Add(f8__locals1.featurePackage);
				}
			}
			IL_1A9:
			if (!string.IsNullOrEmpty(f8__locals1.optionalFeatureId) 
				&& DesktopCompDBGen._featurePkgMap.Keys.FirstOrDefault((CompDBFeature feat) 
				=> feat.FeatureID.Equals(f8__locals1.optionalFeatureId, StringComparison.OrdinalIgnoreCase)) == null)
			{
				CompDBFeature.CompDBFeatureTypes type = (pkgType == DesktopCompDBGen.PackageTypes.LANGPACK) 
					? CompDBFeature.CompDBFeatureTypes.LanguagePack
					: CompDBFeature.CompDBFeatureTypes.OnDemandFeature;
				
				CompDBFeature compDBFeature5 = new CompDBFeature(f8__locals1.optionalFeatureId, 
					null, type, OwnerType.Microsoft.ToString());
				
				compDBFeature5.Packages.Add(f8__locals1.featurePackage);
				
				Dictionary<CompDBFeature, List<CompDBFeaturePackage>> featurePkgMap2 
					= DesktopCompDBGen._featurePkgMap;
				CompDBFeature compDBFeature6 = compDBFeature5;
				featurePkgMap2.Add(compDBFeature6, compDBFeature6.Packages);
				DesktopCompDBGen._logger.LogInfo("ImageCommon::DesktopCompDBGen: " +
					"Generating feature package mapping. Feature: {0}, Package: {1}", new object[]
				{
					f8__locals1.optionalFeatureId,
					f8__locals1.featurePackage.ID
				});
			}
			...
			DesktopCompDBGen._logger.LogInfo
				("ImageCommon::DesktopCompDBGen: Generating feature package mapping. Feature: {0}, Package: {1}", 
				new object[]
			{
				f8__locals1.featureId,
				f8__locals1.featurePackage.ID
			});
		...
		private class EditionMaps
		{
            internal string lang;
            internal List<string> media;
            internal Func<XElement, bool> f9__3;

            public EditionMaps()
            {
            }

            public string RNS { get; internal set; }
        }

        private class LangPacks
        {
            internal List<string> langPkgIDs;
           
            internal List<string> neutralPkgIDs;
            ...
            //RnD
            internal object f9__8;
            internal object f9__20;
            internal List<string> editionPkgIDs;
            internal Func<CompDBPackageInfo, bool> f9__28;
        }

        private class FMCollections
        {
            internal List<string> featureIDs;
            internal BuildCompDB newCompDB;
            internal Func<string, bool> f9__4;

            public FMCollections()
            {
            }
        }

        private class FPMappings
        {
            internal string featureId;
            internal string optionalFeatureId;
            internal CompDBFeaturePackage featurePackage;
            internal Func<CompDBFeaturePackage, bool> f9__0;
