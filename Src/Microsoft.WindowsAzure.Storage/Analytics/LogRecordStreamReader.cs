...
namespace Microsoft.WindowsAzure.Storage.Analytics
{
	...
	internal class LogRecordStreamReader : IDisposable
	{
		...
		public bool? ReadBool()
		{
			...
			return new bool?(bool.Parse(text));
		}
		...
		public DateTimeOffset? ReadDateTimeOffset(string format)
		{
			...
			bool flag = DateTimeOffset.TryParseExact(text, format, null, DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal, out dateTimeOffset);
			if (flag)
			{
				return new DateTimeOffset?(dateTimeOffset);
			}
			throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "Error parsing log record: could not parse '{0}' using format: {1}", new object[] { text, format }));
		}
		...
		public Uri ReadUri()
		{
			...
			return new Uri(WebUtility.HtmlDecode(text));
		}
		...
		private void EnsureNotEndOfFile()
		{
			if (this.IsEndOfFile)
			{
				throw new EndOfStreamException(string.Format(CultureInfo.InvariantCulture, "Error parsing log record: unexpected end of stream at position '{0}'.", new object[] { this.Position }));
			}
		}
		...
		private string ReadField(bool isQuotedString)
		{
			...
			{
				text = stringBuilder.ToString(1, stringBuilder.Length - 2);
			}
			else
			{
