...
namespace Nokia.Mira.Strategies
{
	...
	public interface IDownloadStrategyFactory
	{
		...
		IDownloadStrategy Create(
			string temporaryFileName, 
			DownloadSettings downloadSettings, 
			IHttpWebRequestFactory httpWebRequestFactory, 
			IChunkInformationWriter chunkInformationWriter, 
			ChunkInformation[] informations, 
			IChunkInformationProvider provider, 
			IWebResponse initialResponse, 
			ChunkInformation initialChunkInformation, 
			CancellationToken token, 
			Action<DownloadProgressInfo> reportProgress);
	}
}
