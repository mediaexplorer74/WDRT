...
namespace Microsoft.WindowsAzure.Storage.Core
{
	...
	public class MultiBufferMemoryStream : Stream
	{
		...
		private void AddBlock()
		{
			...
			{
				throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "The IBufferManager provided an incorrect length buffer to the stream, Expected {0}, received {1}. Buffer length should equal the value returned by IBufferManager.GetDefaultBufferSize().", new object[] { this.bufferSize, array.Length }));
			}
		...
		private int ReadInternal(byte[] buffer, int offset, int count)
		{
			...
			{
				ArraySegment<byte> currentBlock = this.GetCurrentBlock();
				int num3 = Math.Min(num2, currentBlock.Count);
				Buffer.BlockCopy(currentBlock.Array, currentBlock.Offset, buffer, offset, num3);
				this.AdvancePosition(ref offset, ref num2, num3);
			}
			return num;
		}
		...
		private ArraySegment<byte> GetCurrentBlock()
		{
			...
			int num2 = (int)(this.position % (long)this.bufferSize);
			byte[] array = this.bufferBlocks[num];
			return new ArraySegment<byte>(array, num2, array.Length - num2);
		}
		...
		protected override void Dispose(bool disposing)
		{
			if (!this.disposed)
			{
				...
				if (disposing)
				{
					...
					{
						foreach (byte[] array in this.bufferBlocks)
						{
