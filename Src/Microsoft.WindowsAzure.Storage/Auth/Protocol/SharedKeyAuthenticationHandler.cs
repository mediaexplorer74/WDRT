...
namespace Microsoft.WindowsAzure.Storage.Auth.Protocol
{
	...
	public sealed class SharedKeyAuthenticationHandler : IAuthenticationHandler
	{
		...
		public void SignRequest(HttpWebRequest request, OperationContext operationContext)
		{
			...
			{
				string text = HttpWebUtility.ConvertDateTimeToHttpString(DateTime.UtcNow);
				request.Headers.Add("x-ms-date", text);
			}
			if (this.credentials.IsSharedKey)
			{
				...
				string text2 = this.canonicalizer.CanonicalizeHttpRequest(request, this.accountName);
				Logger.LogVerbose(operationContext, "StringToSign = {0}.", new object[] { text2 });
				string text3 = CryptoUtility.ComputeHmac256(key.KeyValue, text2);
