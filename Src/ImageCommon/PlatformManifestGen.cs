			...
			if (this._doSignInfo)
			{
				...
				foreach (IFileEntry fileEntry in package.Files)
				{
					if (fileEntry.SignInfoRequired)
					{
						...
						string fileHash = Convert.ToBase64String(array).Replace('/', '-').ToLower();
						if (fileHash == string.Empty)
						{
							this._logger.LogWarning("Warning: File '{0}' in package '{1}' requires signInfo but has empty fileHash!", new object[] { fileEntry.DevicePath, package.Name });
						}
						else
						{
							List<string> list = PlatformManifestGen._signInfoFiles.Where((string file) => file.Contains(fileHash)).ToList<string>();
							if (list.Count<string>() != 1)
							{
								if (list.Count<string>() == 0)
								{
									...
									{
										this.ErrorMessages.AppendLine(string.Concat(new string[] { "Error: File '", fileEntry.DevicePath, "' in package '", package.Name, "' failed to find any SignInfo files using the following pattern: ", fileHash, " (Filename found but using different hashes)" }));
										continue;
									}
									this.ErrorMessages.AppendLine(string.Concat(new string[] { "Error: File '", fileEntry.DevicePath, "' in package '", package.Name, "' failed to find any SignInfo files using the following pattern: ", fileHash }));
									continue;
								}
								else
								{
									string text = string.Concat(new string[] { "Error: File '", fileEntry.DevicePath, "' in package '", package.Name, "' failed as we found multiple SignInfo files using the following pattern: ", fileHash });
									...
									using (List<string>.Enumerator enumerator2 = list.GetEnumerator())
									{
										while (enumerator2.MoveNext())
										{
											...
											this._logger.LogError(string.Format("Error: \tMatching file: '{0}'", text2), new object[0]);
										}
										continue;
									}
								}
							}
							string text3 = Path.Combine(this._signinfoPath, list[0]);
							try
							{
								this._pmAPI.AddBinaryFromSignInfo(text3);
							}
							catch
							{
								this.ErrorMessages.AppendLine(string.Concat(new string[] { "Error: File '", fileEntry.DevicePath, "' in package '", package.Name, "' failed to be added to the Platform Manifest" }));
							}
						}
					}
				}
