...
namespace Microsoft.WindowsAzure.Storage.Blob
{
	...
	public class CloudBlob : IListBlobItem
	{
		...
		public Stream OpenRead(AccessCondition accessCondition = null, BlobRequestOptions options = null, OperationContext operationContext = null)
		{
			...
			BlobRequestOptions blobRequestOptions = BlobRequestOptions.ApplyDefaults(options, BlobType.Unspecified, this.ServiceClient, false);
			return new BlobReadStream(this, accessCondition2, blobRequestOptions, operationContext);
		}
		...
		public ICancellableAsyncResult BeginOpenRead(AccessCondition accessCondition, BlobRequestOptions options, OperationContext operationContext, AsyncCallback callback, object state)
		{
			...
			ICancellableAsyncResult cancellableAsyncResult = this.BeginFetchAttributes(accessCondition, options, operationContext, delegate(IAsyncResult ar)
			{
				try
				{
					...
					AccessCondition accessCondition2 = AccessCondition.CloneConditionWithETag(accessCondition, this.Properties.ETag);
					BlobRequestOptions blobRequestOptions = BlobRequestOptions.ApplyDefaults(options, BlobType.Unspecified, this.ServiceClient, false);
					storageAsyncResult.Result = new BlobReadStream(this, accessCondition2, blobRequestOptions, operationContext);
		...
		public ICancellableAsyncResult BeginDownloadRangeToByteArray(byte[] target, int index, long? blobOffset, long? length, AccessCondition accessCondition, BlobRequestOptions options, OperationContext operationContext, AsyncCallback callback, object state)
		{
			...
			ICancellableAsyncResult cancellableAsyncResult = this.BeginDownloadRangeToStream(syncMemoryStream, blobOffset, length, accessCondition, options, operationContext, new AsyncCallback(this.DownloadRangeToByteArrayCallback), storageAsyncResult);
			storageAsyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
			return storageAsyncResult;
		}
		...
		public bool DeleteIfExists(DeleteSnapshotsOption deleteSnapshotsOption = DeleteSnapshotsOption.None, AccessCondition accessCondition = null, BlobRequestOptions options = null, OperationContext operationContext = null)
		{
			...
			try
			{
				this.Delete(deleteSnapshotsOption, accessCondition, blobRequestOptions, operationContext);
				flag = true;
			}
		...
		{
			ICancellableAsyncResult cancellableAsyncResult = this.BeginExists(true, options, operationContext, delegate(IAsyncResult existsResult)
			{
				...
				lock (storageAsyncResult.CancellationLockerObject)
				{
					...
					try
					{
						...
						{
							ICancellableAsyncResult cancellableAsyncResult2 = this.BeginDelete(deleteSnapshotsOption, accessCondition, options, operationContext, delegate(IAsyncResult deleteResult)
							{
								...
								catch (StorageException ex2)
								{
									...
									{
										if (ex2.RequestInformation.ExtendedErrorInformation == null || ex2.RequestInformation.ExtendedErrorInformation.ErrorCode == BlobErrorCodeStrings.BlobNotFound)
										{
											storageAsyncResult.Result = false;
											storageAsyncResult.OnComplete();
										}
		...
		private RESTCommand<CloudBlob> SnapshotImpl(IDictionary<string, string> metadata, AccessCondition accessCondition, BlobRequestOptions options)
		{
			...
			restcommand.PreProcessResponse = delegate(RESTCommand<CloudBlob> cmd, HttpWebResponse resp, Exception ex, OperationContext ctx)
			{
				...
				DateTimeOffset dateTimeOffset = NavigationHelper.ParseSnapshotTime(BlobHttpResponseParsers.GetSnapshotTime(resp));
				CloudBlob cloudBlob = new CloudBlob(this.Name, new DateTimeOffset?(dateTimeOffset), this.Container);
		...
		public CloudBlobDirectory Parent
		{
			get
			{
				...
				if (this.parent == null && NavigationHelper.GetBlobParentNameAndAddress(this.StorageUri, this.ServiceClient.DefaultDelimiter, new bool?(this.ServiceClient.UsePathStyleUris), out text, out storageUri))
				{
					this.parent = new CloudBlobDirectory(storageUri, text, this.Container);
				}
				return this.parent;
			}
		}
		...
		public string GetSharedAccessSignature(SharedAccessBlobPolicy policy, SharedAccessBlobHeaders headers, string groupPolicyIdentifier, string sasVersion)
		{
			...
			string text2 = SharedAccessSignatureHelper.ValidateSASVersionString(sasVersion);
			string canonicalName = this.GetCanonicalName(true, text2);
			StorageAccountKey key = this.ServiceClient.Credentials.Key;
			string hash = SharedAccessSignatureHelper.GetHash(policy, headers, groupPolicyIdentifier, canonicalName, text2, key.KeyValue);
			UriQueryBuilder signature = SharedAccessSignatureHelper.GetSignature(policy, headers, groupPolicyIdentifier, "b", hash, key.KeyName, text2);
			return signature.ToString();
		}
		...
		private string GetCanonicalName(bool ignoreSnapshotTime, string sasVersion)
		{
			...
			string text2 = "/{0}/{1}/{2}/{3}";
			if (sasVersion == "2012-02-12" || sasVersion == "2013-08-15")
			{
				text2 = "/{1}/{2}/{3}";
			}
			string text3 = string.Format(CultureInfo.InvariantCulture, text2, new object[] { "blob", accountName, name, text });
			if (!ignoreSnapshotTime && this.SnapshotTime != null)
			{
				text3 = text3 + "?snapshot=" + Request.ConvertDateTimeToSnapshotString(this.SnapshotTime.Value);
			}
			return text3;
		}
		...
		private void ParseQueryAndVerify(StorageUri address, StorageCredentials credentials)
		{
			...
			{
				string text2 = string.Format(CultureInfo.CurrentCulture, "Multiple different snapshot times provided as part of query '{0}' and as constructor parameter '{1}'.", new object[] { dateTimeOffset, this.SnapshotTime });
