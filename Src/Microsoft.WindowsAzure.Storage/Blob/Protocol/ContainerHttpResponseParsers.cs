...
namespace Microsoft.WindowsAzure.Storage.Blob.Protocol
{
	...
	public static class ContainerHttpResponseParsers
	{
		...
		{
			CommonUtility.AssertNotNull("response", response);
			string text = response.Headers["x-ms-blob-public-access"];
			return ContainerHttpResponseParsers.GetContainerAcl(text);
		}
		...
		private static BlobContainerPublicAccessType GetContainerAcl(string acl)
		{
			...
			if (!string.IsNullOrEmpty(acl))
			{
				...
				string text2 = string.Format(CultureInfo.CurrentCulture, "Invalid acl public access type returned '{0}'. Expected blob or container.", new object[] { acl });
				throw new InvalidOperationException(text2);
			}
			return blobContainerPublicAccessType;
		}
	}
}
