	...
	internal class EventCounterGroup : IDisposable
	{
		...
		private static void EnsureEventSourceIndexAvailable(int eventSourceIndex)
		{
			checked
			{
				...
				{
					EventCounterGroup[] array = new EventCounterGroup[eventSourceIndex + 1];
					Array.Copy(EventCounterGroup.s_eventCounterGroups, array, EventCounterGroup.s_eventCounterGroups.Length);
		...
		private void EnableTimer(float pollingIntervalInSeconds)
		{
			...
			checked
			{
				...
				{
					this._pollingIntervalInMilliseconds = (int)(unchecked(pollingIntervalInSeconds * 1000f));
					if (this._pollingTimer != null)
					{
						this._pollingTimer.Dispose();
						this._pollingTimer = null;
