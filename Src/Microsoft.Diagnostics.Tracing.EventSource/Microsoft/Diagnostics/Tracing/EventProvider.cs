...
namespace Microsoft.Diagnostics.Tracing
{
	...
	internal class EventProvider : IDisposable
	{
					...
					if (controlCode == 1)
					{
						...
						using (List<Tuple<EventProvider.SessionInfo, bool>>.Enumerator enumerator = sessions.GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
								...
								this.OnControllerCommand(controllerCommand, dictionary, item ? sessionIdBit : (0 - sessionIdBit), etwSessionId);
							}
							goto IL_16C;
						}
					}
			...
			checked
			{
				if (registryKey != null)
				{
					foreach (string text2 in registryKey.GetValueNames())
					{
						...
						{
							string text3 = text2.Substring(23);
							int num;
							if (int.TryParse(text3, out num))
							{
								...
								if (array != null)
								{
									...
									int num2 = @string.IndexOf("EtwSessionKeyword");
									if (0 <= num2)
									{
										int num3 = num2 + 18;
										int num4 = @string.IndexOf('\0', num3);
										string text4 = @string.Substring(num3, num4 - num3);
										int num5;
										if (0 < num4 && int.TryParse(text4, out num5))
										{
											action(num, 1L << num5);
										}
									}
		...
		private unsafe bool GetDataFromController(int etwSessionId, UnsafeNativeMethods.ManifestEtw.EVENT_FILTER_DESCRIPTOR* filterData, out ControllerCommand command, out byte[] data, out int dataStart)
		{
			...
			string text2 = "ControllerData_Session_" + etwSessionId.ToString(CultureInfo.InvariantCulture);
			new RegistryPermission(RegistryPermissionAccess.Read, text).Assert();
			data = Registry.GetValue(text, text2, null) as byte[];
			if (data != null)
			{
		...
		private unsafe static object EncodeObject(ref object data, ref EventProvider.EventData* dataDescriptor, ref byte* dataBuffer, ref uint totalEventSize)
		{
			...
			*dataBuffer = array.Length;
			...
			Block_2:
			dataDescriptor.Size = checked((uint)sizeof(IntPtr));
			...
			*ptr12 = (double)data;
			...
			goto IL_43B;
			Block_16:
			dataDescriptor.Size = checked((uint)sizeof(Guid));
		...
		internal unsafe bool WriteEvent(ref EventDescriptor eventDescriptor, Guid* activityID, Guid* childActivityID, params object[] eventPayload)
		{
			...
			checked
			{
				if (this.IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
				{
					...
					unchecked
					{
						byte* ptr3 = stackalloc byte[(UIntPtr)(checked(32 * num2))];
						ptr4 = ptr3;
						flag = false;
					}
