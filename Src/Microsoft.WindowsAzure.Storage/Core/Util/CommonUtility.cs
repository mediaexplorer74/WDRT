...
namespace Microsoft.WindowsAzure.Storage.Core.Util
{
	...
	internal static class CommonUtility
	{
		...
		{
			RESTCommand<NullType> restcommand = new RESTCommand<NullType>(new StorageCredentials(), null);
			if (options != null)
			{
				options.ApplyToStorageCommand<NullType>(restcommand);
			}
			return new ExecutionState<NullType>(restcommand, (options != null) ? options.RetryPolicy : null, new OperationContext());
		}
		...
		{
			throw new ArgumentOutOfRangeException(paramName, string.Format(CultureInfo.InvariantCulture, "The argument is out of range. Value passed: {0}", new object[] { value }));
		}
		...
		internal static void AssertInBounds<T>(string paramName, T val, T min, T max) where T : IComparable
		{
			...
			{
				throw new ArgumentOutOfRangeException(paramName, string.Format(CultureInfo.InvariantCulture, "The argument '{0}' is smaller than minimum of '{1}'", new object[] { paramName, min }));
			}
			if (val.CompareTo(max) > 0)
			{
				throw new ArgumentOutOfRangeException(paramName, string.Format(CultureInfo.InvariantCulture, "The argument '{0}' is larger than maximum of '{1}'", new object[] { paramName, max }));
			}
		}
