				...
				while (i < CS$<>8__locals1.eventMessage.Length)
				{
					...
					else if (CS$<>8__locals1.eventMessage[i] == '{')
					{
						...
						if (i < CS$<>8__locals1.eventMessage.Length && CS$<>8__locals1.eventMessage[i] == '}')
						{
							...
							int num2 = this.TranslateIndexToManifestConvention(num, evtName);
							CS$<>8__locals1.stringBuilder.Append('%').Append(num2);
							if (i < CS$<>8__locals1.eventMessage.Length && CS$<>8__locals1.eventMessage[i] == '!')
							{
								i++;
								CS$<>8__locals1.stringBuilder.Append("%!");
						...
						{
							this.ManifestError(Microsoft.Diagnostics.Tracing.Internal.Environment.GetResourceString("EventSource_UnsupportedMessageProperty", new object[] { evtName, CS$<>8__locals1.eventMessage }), false);
						}
					}
					else if ((num3 = "&<>'\"\r\n\t".IndexOf(CS$<>8__locals1.eventMessage[i])) >= 0)
					{
						...
						action(CS$<>8__locals1.eventMessage[i], array[num3]);
					}
					else
					{
						i++;
