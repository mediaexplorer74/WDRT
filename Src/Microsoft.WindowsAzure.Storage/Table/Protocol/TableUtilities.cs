		...
		internal static StorageException TranslateDataServiceException(Exception e, RequestResult reqResult, Func<Stream, IDictionary<string, string>, StorageExtendedErrorInformation> parseError)
		{
			...
			try
			{
				...
				if (ex == null && ex2 == null)
				{
					...
					{
						ex4 = new StorageException(reqResult, e.Message, e)
						{
							IsRetryable = false
						};
					}
				...
				else if (ex != null)
				{
					...
					foreach (OperationResponse operationResponse in response)
					{
						...
						if (reqResult.HttpStatusCode >= 300)
						{
							...
							string text = ex.InnerException.ToString().Replace("System.Data.Services.Client.DataServiceClientException: ", string.Empty);
					...
					ex4 = new StorageException(reqResult, (reqResult.ExtendedErrorInformation != null) ? reqResult.ExtendedErrorInformation.ErrorCode : ex.Message, ex);
				}
				else
				{
					...
					ex4 = new StorageException(reqResult, (reqResult.ExtendedErrorInformation != null) ? reqResult.ExtendedErrorInformation.ErrorCode : ex2.Message, ex2);
				}
			}
			catch (Exception)
			{
