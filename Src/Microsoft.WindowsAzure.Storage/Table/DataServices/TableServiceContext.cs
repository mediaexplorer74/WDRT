...
namespace Microsoft.WindowsAzure.Storage.Table.DataServices
{
	...
	public class TableServiceContext : DataServiceContext, IDisposable
	{
			...
			: base(client.BaseUri, DataServiceProtocolVersion.V3)
		{
			...
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Address '{0}' is a relative address. Only absolute addresses are permitted.", new object[] { client.BaseUri.ToString() });
				throw new ArgumentException(text, "client");
			}
		...
		private void TableServiceContext_SendingRequest(object sender, SendingRequestEventArgs e)
		{
			...
			if (num > 0)
			{
				...
				string text = ((num2 > 0) ? httpWebRequest.RequestUri.Query.Substring(num, num2 - num) : httpWebRequest.RequestUri.Query.Substring(num));
				int num3 = -1;
				if (int.TryParse(text, out num3) && num3 > 0)
				{
			...
			{
				Uri uri = this.ServiceClient.Credentials.TransformUri(httpWebRequest.RequestUri);
				HttpWebRequest httpWebRequest2 = WebRequest.Create(uri) as HttpWebRequest;
		...
		public DataServiceResponse SaveChangesWithRetries(SaveChangesOptions options, TableRequestOptions requestOptions = null, OperationContext operationContext = null)
		{
			...
			TableCommand<DataServiceResponse, DataServiceResponse> tableCommand = this.GenerateSaveChangesCommand(options, requestOptions);
			return TableExecutor.ExecuteSync<DataServiceResponse, DataServiceResponse>(tableCommand, requestOptions.RetryPolicy, operationContext);
		}
		...
		public ICancellableAsyncResult BeginSaveChangesWithRetries(SaveChangesOptions options, TableRequestOptions requestOptions, OperationContext operationContext, AsyncCallback callback, object state)
		{
			...
			operationContext = operationContext ?? new OperationContext();
			TableCommand<DataServiceResponse, DataServiceResponse> tableCommand = this.GenerateSaveChangesCommand(options, requestOptions);
			return TableExecutor.BeginExecuteAsync<DataServiceResponse, DataServiceResponse>(tableCommand, requestOptions.RetryPolicy, operationContext, callback, state);
		}
