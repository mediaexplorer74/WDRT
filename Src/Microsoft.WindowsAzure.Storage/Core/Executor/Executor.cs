...
namespace Microsoft.WindowsAzure.Storage.Core.Executor
{
	...
	internal class Executor : ExecutorBase
	{
		...
		public static void InitRequest<T>(ExecutionState<T> executionState)
		{
			...
			{
				Logger.LogError(executionState.OperationContext, "Exception thrown while initializing request: {0}.", new object[] { ex.Message });
		...
		private static void BeginGetRequestStream<T>(ExecutionState<T> executionState)
		{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while preparing to write request data: {0}.", new object[] { ex.Message });
		...
		private static void EndGetRequestStream<T>(IAsyncResult getRequestStreamResult)
		{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while writing request data: {0}.", new object[] { ex.Message });
				executionState.ExceptionRef = ExecutorBase.TranslateExceptionBasedOnParseError(ex, executionState.Cmd.CurrentResult, executionState.Resp, executionState.Cmd.ParseError);
				Executor.EndOperation<T>(executionState);
			}
		}
		...
		private static void BeginGetResponse<T>(ExecutionState<T> executionState)
		{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while waiting for response: {0}.", new object[] { ex.Message });
				executionState.ExceptionRef = ExecutorBase.TranslateExceptionBasedOnParseError(ex, executionState.Cmd.CurrentResult, executionState.Resp, executionState.Cmd.ParseError);
				Executor.EndOperation<T>(executionState);
			}
		}
		...
		private static void EndGetResponse<T>(IAsyncResult getResponseResult)
		{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while processing response: {0}.", new object[] { ex3.Message });
		...
		private static void EndOperation<T>(ExecutionState<T> executionState)
		{
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while ending operation: {0}.", new object[] { ex.Message });
				executionState.ExceptionRef = ExecutorBase.TranslateExceptionBasedOnParseError(ex, executionState.Cmd.CurrentResult, executionState.Resp, executionState.Cmd.ParseError);
			}
			finally
			{
			...
			try
			{
				...
				if (ex2.IsRetryable && executionState.RetryPolicy != null)
				{
					...
					Logger.LogInformational(executionState.OperationContext, "The next location has been set to {0}, based on the location mode.", new object[] { executionState.CurrentLocation });
					...
					if (extendedRetryPolicy != null)
					{
						...
						if (retryInfo != null)
						{
							Logger.LogInformational(executionState.OperationContext, "The extended retry policy set the next location to {0} and updated the location mode to {1}.", new object[] { retryInfo.TargetLocation, retryInfo.UpdatedLocationMode });
							flag = true;
				...
				{
					Logger.LogError(executionState.OperationContext, flag ? "Operation cannot be retried because the maximum execution time has been reached. Failing with {0}." : "Retry policy did not allow for a retry. Failing with {0}.", new object[] { executionState.ExceptionRef.Message });
					executionState.OnComplete();
				}
				else
				{
					...
					{
						Logger.LogInformational(executionState.OperationContext, "Operation will be retried after {0}ms.", new object[] { (int)timeSpan.TotalMilliseconds });
			...
			{
				Logger.LogWarning(executionState.OperationContext, "Exception thrown while retrying operation: {0}.", new object[] { ex3.Message });
				executionState.ExceptionRef = ExecutorBase.TranslateExceptionBasedOnParseError(ex3, executionState.Cmd.CurrentResult, executionState.Resp, executionState.Cmd.ParseError);
				executionState.OnComplete();
			}
		}
		...
		private static void AbortRequest<T>(object state)
		{
			...
			{
				Logger.LogError(executionState.OperationContext, "Could not abort pending request because of {0}.", new object[] { ex.Message });
			}
		}
			...
			using (ExecutionState<T> executionState = new ExecutionState<T>(cmd, policy, operationContext))
			{
				...
				for (;;)
				{
					...
					try
					{
						...
						{
							Logger.LogWarning(executionState.OperationContext, "Exception thrown while waiting for response: {0}.", new object[] { ex3.Message });
					...
					{
						Logger.LogWarning(executionState.OperationContext, "Exception thrown during the operation: {0}.", new object[] { ex4.Message });
						ExecutorBase.FinishRequestAttempt<T>(executionState);
		...
		private static void ProcessStartOfRequest<T>(ExecutionState<T> executionState, string startLogMessage)
		{
			...
			Logger.LogInformational(executionState.OperationContext, startLogMessage, new object[] { uri2 });
			UriQueryBuilder uriQueryBuilder = new UriQueryBuilder(executionState.RestCMD.Builder);
			executionState.Req = executionState.RestCMD.BuildRequestDelegate(uri2, uriQueryBuilder, executionState.Cmd.ServerTimeoutInSeconds, true, executionState.OperationContext);
			...
			{
				long num = executionState.RestCMD.SendStreamLength ?? (executionState.RestCMD.SendStream.Length - executionState.RestCMD.SendStream.Position);
				CommonUtility.ApplyRequestOptimizations(executionState.Req, num);
			}
			else
			{
