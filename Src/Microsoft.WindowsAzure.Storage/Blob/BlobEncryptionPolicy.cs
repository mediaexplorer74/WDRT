...
namespace Microsoft.WindowsAzure.Storage.Blob
{
	...
	public sealed class BlobEncryptionPolicy
	{
			...
			try
			{
				if (text != null)
				{
					...
					if (this.KeyResolver != null)
					{
						...
						array = result.UnwrapKeyAsync(blobEncryptionData.WrappedContentKey.EncryptedKey, blobEncryptionData.WrappedContentKey.Algorithm, CancellationToken.None).Result;
					}
					else
					{
						...
						array = this.Key.UnwrapKeyAsync(blobEncryptionData.WrappedContentKey.EncryptedKey, blobEncryptionData.WrappedContentKey.Algorithm, CancellationToken.None).Result;
					}
		...
		internal static Stream WrapUserStreamWithDecryptStream(CloudBlob blob, Stream userProvidedStream, BlobRequestOptions options, BlobAttributes attributes, bool rangeRead, out ICryptoTransform transform, long? endOffset = null, long? userSpecifiedLength = null, int discardFirst = 0, bool bufferIV = false)
		{
			...
			bool flag = blob.BlobType == BlobType.PageBlob || (endOffset != null && endOffset.Value < attributes.Properties.Length - 16L);
			transform = null;
			return new BlobDecryptStream(userProvidedStream, attributes.Metadata, userSpecifiedLength, discardFirst, bufferIV, flag, options.EncryptionPolicy, options.RequireEncryption);
		}
		...
		internal ICryptoTransform CreateAndSetEncryptionContext(IDictionary<string, string> metadata, bool noPadding)
		{
			...
			using (AesCryptoServiceProvider aesCryptoServiceProvider = new AesCryptoServiceProvider())
			{
				...
				cryptoTransform = aesCryptoServiceProvider.CreateEncryptor();
			}
