...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public class TableStorageModel : EdmModel, IEdmModel, IEdmElement
	{
		...
		public TableStorageModel(string accountName)
		{
			...
			this.tableType.AddKeys(new IEdmStructuralProperty[] { this.tableType.AddStructuralProperty("TableName", EdmPrimitiveTypeKind.String) });
		...
		IEdmSchemaType IEdmModel.FindDeclaredType(string qualifiedName)
		{
			...
			if (edmSchemaType == null)
			{
				string text;
				string text2;
				TableStorageModel.SplitFullTypeName(qualifiedName, out text, out text2);
				edmSchemaType = TableStorageModel.CreateEntityType(text2, text);
