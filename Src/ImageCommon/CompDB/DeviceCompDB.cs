...
namespace Microsoft.WindowsPhone.CompDB
{
	...
	{
        public Guid BSPBuildID { get; private set; }

        // Token: 0x060000B4 RID: 180 RVA: 0x0000947D File Offset: 0x0000767D
        public bool ShouldSerializeConditionAnswers()
		{
			return this.ConditionAnswers != null 
				&& this.ConditionAnswers.GetAllConditions().Count<Condition>() > 0;
		}
		...
		public new static DeviceCompDB ValidateAndLoad(string xmlFile, IULogger logger)
		{
			...
			{
				throw new ImageCommonException("ImageCommon::DeviceCompDB!ValidateAndLoad: " +
					"XSD resource was not found: " + deviceCompDBSchema);
			}
			...
			{
				throw new ImageCommonException("ImageCommon::DeviceCompDB!ValidateAndLoad: " +
					"Unable to parse Device CompDB XML file '" + xmlFile + "'", innerException);
			}
			finally
			{
				textReader.Close();
			...
			using (Stream manifestResourceStream = executingAssembly.GetManifestResourceStream(text))
			{
				...
				catch (XsdValidatorException innerException2)
				{
					...
					{
						throw new ImageCommonException("ImageCommon::DeviceCompDB!ValidateAndLoad: " +
							"Unable to validate Device CompDB XSD for file '" + xmlFile + "'.", innerException2);
					}
					logger.LogWarning("ImageCommon::DeviceCompDB!ValidateAndLoad: " +
						"Unable to validate Device CompDB XSD for file '" + xmlFile + "'.", new object[0]);
					if (string.IsNullOrEmpty(deviceCompDB.SchemaVersion))
					{
						logger.LogWarning("Warning: ImageCommon::DeviceCompDB!ValidateAndLoad: " +
							"Schema Version was not given in Device CompDB. " +
							"Most up to date Schema Version is {1}.", 
							new object[]
						{
							"1.2"
						});
					...
					{
						logger.LogWarning("Warning: ImageCommon::DeviceCompDB!ValidateAndLoad: " +
							"Schema Version given in Device CompDB ({0}) does not match most " +
							"up to date Schema Version ({1}).", new object[]
						{
			...
			logger.LogInfo("DeviceCompDB: " +
				"Successfully validated the Device CompDB XML: {0}", new object[]
			{
				xmlFile
			});
			...
			{
				if (deviceCompDB.Packages.Any((CompDBPackageInfo pkg) => 
				pkg.ReleaseType == ReleaseType.Test))
				{
		...
		public override void WriteToFile(string xmlFile)
		{
			...
			{
				throw new ImageCommonException("ImageCommon::DeviceCompDB!WriteToFile: " +
					"Unable to write Device CompDB XML file '" + xmlFile + "'", innerException);
			}
			finally
			{
				textWriter.Close();
        ...
        public class DeviceDBType
        {
            internal Func<CompDBPackageInfo, bool> f9__11;
            internal List<string> featurePackageIDs;
            internal List<string> langModelPackageIDs;
            internal Predicate<CompDBFeaturePackage> f9__22;
            internal List<string> removeLanguagePackageIDs;
            internal string fmDevicePath;
            ...
            internal Func<IFileEntry, bool> f9__26;
        }



        // Token: 0x060000B9 RID: 185 RVA: 0x00009900 File Offset: 0x00007B00
        public static DeviceCompDB CreateDBFromPackages(List<IPkgInfo> packages, 
			List<Hashtable> registryTable, string targetDBFile, IULogger logger)
		{

            DeviceCompDB.DeviceDBType f8__locals1 = new DeviceCompDB.DeviceDBType();// RnD
            Dictionary<string, List<string>> dictionary =
				new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
			DeviceCompDB deviceCompDB = new DeviceCompDB();
			deviceCompDB.BuildArch = (from pkg0 in packages
			group pkg0 by pkg0.CpuType into g
			...
			foreach (IPkgInfo pkgInfo in packages)
			{
				...
				string mumDevicePath = "\\" + PkgConstants.c_strMumDeviceFolder + 
					"\\" + PkgConstants.c_strMumFile;
				bool flag = false;
				try
				{
				...
				if (!flag)
				{
					IFileEntry fileEntry = pkgInfo.Files.FirstOrDefault((IFileEntry file) => 
					file.DevicePath.Equals(mumDevicePath, StringComparison.OrdinalIgnoreCase));
				...
				if (!string.IsNullOrEmpty(featureInfoXML))
				{
					...
					PrepCBSFeature.ParseFeatureInfoXML(featureInfoXML, out fmID, 
						out featureID, out group, out text4, out list2);
					CompDBFeature compDBFeature = new CompDBFeature(featureID, fmID, 
						CompDBFeature.CompDBFeatureTypes.MobileFeature, group);
					compDBFeature.Packages = new List<CompDBFeaturePackage>();
					foreach (FeatureManifest.FMPkgInfo fmpkgInfo in list2)
					{
						CompDBFeaturePackage item = new CompDBFeaturePackage(fmpkgInfo.ID, 
							pkgInfo.Name.Equals(fmpkgInfo.ID, StringComparison.OrdinalIgnoreCase));
						if (!string.IsNullOrEmpty(fmpkgInfo.Language))
						{
							...
							&& !source.Contains(fmpkgInfo.Resolution, StringComparer.OrdinalIgnoreCase))
						{
							continue;
						}
				...
				CompDBPackageInfo item2 = new CompDBPackageInfo(
					pkgInfo, null, null, null, deviceCompDB, false, false);
				list.Add(item2);
			}

			f8__locals1.langModelPackageIDs = (from pkg2 in (from feat in deviceCompDB.Features
			where feat.FeatureID.StartsWith(FeatureManifest.PackageGroups.KEYBOARD.ToString() + "_", 
				StringComparison.OrdinalIgnoreCase) 
			   || feat.FeatureID.StartsWith(FeatureManifest.PackageGroups.SPEECH.ToString() + "_", 
			StringComparison.OrdinalIgnoreCase)
			select feat).SelectMany((CompDBFeature feat) => feat.Packages)
			select pkg2.ID).ToList<string>();
			IEnumerable<CompDBPackageInfo> source2 = list;
			Func<CompDBPackageInfo, bool> predicate;
			if ((predicate = f8__locals1.f9__11) == null)
			{
                DeviceCompDB.DeviceDBType f8__locals3 = f8__locals1;
				
				predicate = (f8__locals3.f9__11 = ((CompDBPackageInfo pkg) => 
				f8__locals3.langModelPackageIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase)) 
				);
			}
			...
			f8__locals1.featurePackageIDs = ( from pkg in 
				deviceCompDB.Features.SelectMany( (CompDBFeature feat) => feat.Packages)
			select pkg.ID ).ToList<string>();
			deviceCompDB.Packages = (from pkg in list
			where f8__locals1.featurePackageIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase)
			...
			select new CompDBLanguage(lang)).ToList<CompDBLanguage>();
			f8__locals1.languages = (from lang in deviceCompDB.Languages
			select lang.Id).ToList<string>();
			f8__locals1.removeLanguagePackageIDs = (from pair in dictionary
			where !f8__locals1.languages.Contains(pair.Key, StringComparer.OrdinalIgnoreCase)
			select pair).SelectMany((KeyValuePair<string, List<string>> pair) 
			=> from id in pair.Value select id).ToList<string>();
			foreach (CompDBFeature compDBFeature2 in deviceCompDB.Features)
			{
				...
				if ((match = f8__locals1.f9__22) == null)
				{
                    DeviceDBType f8__locals4 = f8__locals1;
					match = (f8__locals4.f9__22 = ((CompDBFeaturePackage pkg) => 
					f8__locals4.removeLanguagePackageIDs.Contains(pkg.ID, 
			...
			if (!string.IsNullOrEmpty(targetDBFile) && LongPathFile.Exists(targetDBFile))
			{
				...
				if (buildCompDB.MSConditionalFeatures.SelectMany((FMConditionalFeature feat) 
					=> feat.GetAllConditions()).ToList<Condition>().Any(
					(Condition cond) => cond.Type != Condition.ConditionType.Feature))
				{
					deviceCompDB.ConditionAnswers = new DeviceConditionAnswers(logger);
					deviceCompDB.ConditionAnswers.PopulateConditionAnswers(
						buildCompDB.MSConditionalFeatures, registryTable);
				}
			}
			f8__locals1.fmDevicePath = "\\" + DevicePaths.MSFMPath;
			IPkgInfo pkgInfo2 = packages.Where(delegate(IPkgInfo pkg)
			{
				...
				if ((predicate2 = f8__locals1.f9__26) == null)
				{
					predicate2 = (f8__locals1.f9__26 = 
					((IFileEntry fi) => fi.DevicePath.StartsWith(
						f8__locals1.fmDevicePath, StringComparison.OrdinalIgnoreCase)));
				}
				return files.Any(predicate2);
			}).FirstOrDefault<IPkgInfo>();
			if (pkgInfo2 != null)
			{
				IFileEntry fileEntry2 = pkgInfo2.Files.FirstOrDefault((IFileEntry fi) => 
				fi.DevicePath.StartsWith(f8__locals1.fmDevicePath, StringComparison.OrdinalIgnoreCase));
				if (fileEntry2 != null)
				{
					string text5 = FileUtils.GetTempFile() + "." 
						+ Path.GetFileName(fileEntry2.DevicePath);
					...
					if (!string.IsNullOrEmpty(featureManifest.BuildID))
					{
                        ...
                        deviceCompDB2.BuildID = (deviceCompDB2.BSPBuildID = 
							new Guid(featureManifest.BuildID));
					...
					if (!string.IsNullOrEmpty(featureManifest.BuildInfo))
					{
						...
						deviceCompDB3.BuildInfo = 
							(deviceCompDB3.BSPBuildInfo = featureManifest.BuildInfo);
					}
				}
			}
