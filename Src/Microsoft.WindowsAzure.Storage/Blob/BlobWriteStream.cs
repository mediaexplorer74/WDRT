...
namespace Microsoft.WindowsAzure.Storage.Blob
{
	...
	internal sealed class BlobWriteStream : BlobWriteStreamBase
	{
		...
		public override IAsyncResult BeginWrite(byte[] buffer, int offset, int count, AsyncCallback callback, object state)
		{
			...
			StorageAsyncResult<NullType> storageAsyncResult2 = storageAsyncResult;
			...
			if (this.lastException == null)
			{
				while (count > 0)
				{
					...
					if (num2 == num)
					{
						this.DispatchWrite(storageAsyncResult2);
						flag = true;
						storageAsyncResult2 = null;
					}
				}
			}
			if (!flag)
		...
		{
			StorageAsyncResult<NullType> storageAsyncResult = new StorageAsyncResult<NullType>(callback, state);
			ICancellableAsyncResult cancellableAsyncResult = this.BeginFlush(new AsyncCallback(this.CommitFlushCallback), storageAsyncResult);
			...
			lock (storageAsyncResult.CancellationLockerObject)
			{
				...
				try
				{
					...
					if (this.blockBlob != null)
					{
						...
						ICancellableAsyncResult cancellableAsyncResult = this.blockBlob.BeginPutBlockList(this.blockList, this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.PutBlockListCallback), storageAsyncResult);
						storageAsyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
					}
					else if (this.blobMD5 != null)
					{
						base.Blob.Properties.ContentMD5 = this.blobMD5.ComputeHash();
						ICancellableAsyncResult cancellableAsyncResult2 = base.Blob.BeginSetProperties(this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.SetPropertiesCallback), storageAsyncResult);
		...
		private void WriteBlock(Stream blockData, string blockId, string blockMD5, StorageAsyncResult<NullType> asyncResult)
		{
			...
			this.parallelOperationSemaphore.WaitAsync(delegate(bool calledSynchronously)
			{
				try
				{
					ICancellableAsyncResult cancellableAsyncResult = this.blockBlob.BeginPutBlock(blockId, blockData, blockMD5, this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.PutBlockCallback), null);
					if (asyncResult != null)
					{
						asyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
					}
				}
				catch (Exception ex)
				{
		...
		private void WritePages(Stream pageData, long offset, string contentMD5, StorageAsyncResult<NullType> asyncResult)
		{
			...
			this.parallelOperationSemaphore.WaitAsync(delegate(bool calledSynchronously)
			{
				try
				{
					ICancellableAsyncResult cancellableAsyncResult = this.pageBlob.BeginWritePages(pageData, offset, contentMD5, this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.WritePagesCallback), null);
					if (asyncResult != null)
					{
						asyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
					}
		...
		private void WriteAppendBlock(Stream blockData, long offset, string blockMD5, StorageAsyncResult<NullType> asyncResult)
		{
			...
			this.parallelOperationSemaphore.WaitAsync(delegate(bool calledSynchronously)
			{
				try
				{
					...
					ICancellableAsyncResult cancellableAsyncResult = this.appendBlob.BeginAppendBlock(blockData, blockMD5, this.accessCondition, this.options, this.operationContext, new AsyncCallback(this.AppendBlockCallback), count);
					if (asyncResult != null)
					{
						asyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
					}
