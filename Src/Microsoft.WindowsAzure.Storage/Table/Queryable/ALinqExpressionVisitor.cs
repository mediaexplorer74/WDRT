...
namespace Microsoft.WindowsAzure.Storage.Table.Queryable
{
	...
	internal abstract class ALinqExpressionVisitor
	{
		...
		internal virtual Expression Visit(Expression exp)
		{
			...
			default:
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression type {0} is not supported.", new object[] { exp.NodeType.ToString() }));
			}
		}
		...
		internal virtual MemberBinding VisitBinding(MemberBinding binding)
		{
			...
			default:
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression type {0} is not supported.", new object[] { binding.BindingType.ToString() }));
			}
		}
