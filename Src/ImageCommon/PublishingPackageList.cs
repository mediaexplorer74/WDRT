...
namespace Microsoft.WindowsPhone.Imaging
{
	...
	public class PublishingPackageList
	{
		...
		public PublishingPackageList(string sourceListPath, string destListPath, IULogger logger)
		{
			...
			enumerable = enumerable.Select((PublishingPackageInfo pkg) => pkg.SetUpdateType(PublishingPackageInfo.UpdateTypes.Diff)).ToList<PublishingPackageInfo>();
		...
		public Dictionary<string, string> GetFeatureIDWithFMIDPackages(OwnerType forOwner = 0)
		{
			...
			foreach (string text in featureIDWithFMIDs)
			{
				...
				List<PublishingPackageInfo> list = allPackagesForFeatureIDWithFMID.Where((PublishingPackageInfo pkg) => pkg.IsFeatureIdentifierPackage).ToList<PublishingPackageInfo>();
		...
		{
			IEnumerable<PublishingPackageInfo> enumerable = this.Packages.Where((PublishingPackageInfo pkg) => pkg.OwnerType == 1 || pkg.OwnerType == 2);
			if (forOwner != null)
			{
				enumerable = this.Packages.Where((PublishingPackageInfo pkg) => pkg.OwnerType == forOwner);
			}
			return enumerable.Select((PublishingPackageInfo pkg) => pkg.FeatureID).Distinct(this.IgnoreCase).ToList<string>();
		}
		...
		public List<string> GetFeatureIDWithFMIDs(OwnerType forOwner = 0)
		{
			...
			return enumerable.Select((PublishingPackageInfo pkg) => pkg.FeatureIDWithFMID).Distinct(this.IgnoreCase).ToList<string>();
		}
		...
		{
			return this.GetAllPackagesForFeatures(new List<string> { FeatureID }, forOwner);
		}
		...
		{
			List<PublishingPackageInfo> list = this.Packages.Where((PublishingPackageInfo pkg) => FeatureIDWithFMID.Equals(pkg.FeatureIDWithFMID, StringComparison.OrdinalIgnoreCase)).ToList<PublishingPackageInfo>();
			if (forOwner != null)
			{
				list = list.Where((PublishingPackageInfo pkg) => pkg.OwnerType == forOwner).ToList<PublishingPackageInfo>();
			}
			return list;
		}
		...
		private List<PublishingPackageInfo> GetAllPackagesForFeaturesAndFMs(List<string> FeatureIDs, List<string> fmFilter, OwnerType forOwner = 0)
		{
			...
			List<string> newFMFilter = fmFilter.Select((string fm) => fm.ToUpper().Replace("SKU", "FM")).ToList<string>();
			List<string> newSKUFilter = fmFilter.Select((string fm) => fm.ToUpper().Replace("FM", "SKU")).ToList<string>();
			IEnumerable<PublishingPackageInfo> enumerable = allPackagesForFeatures.Where((PublishingPackageInfo pkg) => newFMFilter.Contains(pkg.SourceFMFile, this.IgnoreCase) || newSKUFilter.Contains(pkg.SourceFMFile, this.IgnoreCase));
			return enumerable.ToList<PublishingPackageInfo>();
		}
		...
		{
			IEnumerable<PublishingPackageInfo> enumerable = this.Packages.Where((PublishingPackageInfo pkg) => pkg.OwnerType == forOwner && FeatureIDs.Contains(pkg.FeatureID, this.IgnoreCase));
			if (forOwner == null)
			{
				enumerable = this.Packages.Where((PublishingPackageInfo pkg) => FeatureIDs.Contains(pkg.FeatureID, this.IgnoreCase));
			}
			return enumerable.ToList<PublishingPackageInfo>();
		}
		...
		private List<PublishingPackageInfo> GetUpdatePackageList(List<string> orgFeatures, List<string> newFeatures, List<string> orgLangs, List<string> newLangs, List<string> resolutions, List<string> orgFMs, List<string> newFMs, OwnerType forOwner = 0, bool DoOnlyChanges = false)
		{
			...
			List<string> orgFMsNormalized = orgFMs.Select((string fm) => this.NormalizeFM(fm)).ToList<string>();
			...
			List<PublishingPackageInfo> list2 = this.Packages.Where((PublishingPackageInfo pkg) => addFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && pkg.UpdateType != PublishingPackageInfo.UpdateTypes.PKR && newFMs.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && resolutions.Contains(pkg.SatelliteValue, this.IgnoreCase)) || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && newLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)))).ToList<PublishingPackageInfo>();
			List<PublishingPackageInfo> list3 = this.Packages.Where((PublishingPackageInfo pkg) => removeFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && orgFMsNormalized.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && resolutions.Contains(pkg.SatelliteValue, this.IgnoreCase)) || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && orgLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)))).ToList<PublishingPackageInfo>();
			list2.AddRange(this.Packages.Where((PublishingPackageInfo pkg) => commonFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && addFMs.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && pkg.UpdateType != PublishingPackageInfo.UpdateTypes.PKR && (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && resolutions.Contains(pkg.SatelliteValue, this.IgnoreCase)) || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && newLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)))).ToList<PublishingPackageInfo>());
			list3.AddRange(this.Packages.Where((PublishingPackageInfo pkg) => commonFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && removeFMs.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && (!this.IsUpdateList || pkg.UpdateType != PublishingPackageInfo.UpdateTypes.Canonical) && (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && resolutions.Contains(pkg.SatelliteValue, this.IgnoreCase)) || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && orgLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)))).ToList<PublishingPackageInfo>());
			list2.AddRange(this.Packages.Where((PublishingPackageInfo pkg) => commonFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && newFMs.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && pkg.UpdateType != PublishingPackageInfo.UpdateTypes.PKR && pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && addLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			list3.AddRange(this.Packages.Where((PublishingPackageInfo pkg) => commonFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && orgFMsNormalized.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && removeLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			if (this.IsUpdateList && !DoOnlyChanges)
			{
				list.AddRange(this.Packages.Where((PublishingPackageInfo pkg) => commonFeatures.Contains(pkg.FeatureID, this.IgnoreCase) && commonFMs.Contains(this.NormalizeFM(pkg.SourceFMFile), this.IgnoreCase) && (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && resolutions.Contains(pkg.SatelliteValue, this.IgnoreCase)) || (pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && commonLangs.Contains(pkg.SatelliteValue, this.IgnoreCase)))).ToList<PublishingPackageInfo>());
				list3 = list3.Except(list3.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.Canonical && pkg.OwnerType != 1)).ToList<PublishingPackageInfo>();
			}
		...
		public UpdateOSInput GetUpdateInput(string build1OEMInput, string build2OEMInput, string msPackageRoot, CpuId cpuType, IULogger logger)
		{
			...
			char[] array = new char[] { '\\' };
			msPackageRoot = msPackageRoot.Trim(array);
			OEMInput oeminput = new OEMInput();
			OEMInput oeminput2 = new OEMInput();
			OEMInput.ValidateInput(ref oeminput, build1OEMInput, logger, msPackageRoot, cpuType.ToString());
			OEMInput.ValidateInput(ref oeminput2, build2OEMInput, logger, msPackageRoot, cpuType.ToString());
			List<PublishingPackageInfo> updatePackageList = this.GetUpdatePackageList(oeminput, oeminput2, 268435455, 0);
			list = updatePackageList.Select((PublishingPackageInfo pkg) => pkg.Path).ToList<string>();
			...
			updateOSInput.Description = "(Updating to)" + oeminput2.Description;
		...
		private List<string> GetDepricatedLangs(List<string> Langs)
		{
			...
			using (List<string>.Enumerator enumerator = Langs.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					if (this.Packages.Where((PublishingPackageInfo pkg) => pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && pkg.SatelliteValue.Equals(lang, StringComparison.OrdinalIgnoreCase) && pkg.UpdateType != PublishingPackageInfo.UpdateTypes.PKR).Count<PublishingPackageInfo>() == 0)
					{
						list.Add(lang);
					}
				}
		...
		public List<PublishingPackageInfo> GetMSPackageListForPOP(OEMInput orgOemInput, OEMInput newOemInput)
		{
			...
			bool flag = true;
			List<PublishingPackageInfo> list3 = this.GetUpdatePackageList(list, featureList, list2, userInterface, newOemInput.Resolutions, fms, fms2, 1, flag);
			if (this.IsUpdateList)
			{
				list3 = list3.Except(this.GetSourceOnlyPkgs(list3.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.PKR).ToList<PublishingPackageInfo>())).ToList<PublishingPackageInfo>();
			}
			return list3;
		}
		...
		{
			List<string> list2 = (from pkg in this.Packages
			...
			List<string> list3 = this.Packages.Select((PublishingPackageInfo pkg) => pkg.ID).Distinct(StringComparer.OrdinalIgnoreCase).ToList<string>();
			List<string> inSourceOnlyPackageIDs = list3.Except(list2, this.IgnoreCase).Distinct(this.IgnoreCase).ToList<string>();
			return list.Where((PublishingPackageInfo pkg) => inSourceOnlyPackageIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase)).ToList<PublishingPackageInfo>();
		}
		...
		private List<PublishingPackageInfo> RemoveDuplicatesPkgs(List<PublishingPackageInfo> pkgList)
		{
			...
			List<PublishingPackageInfo> list2 = pkgList.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.PKR).ToList<PublishingPackageInfo>();
			List<PublishingPackageInfo> list3 = pkgList.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.Canonical).ToList<PublishingPackageInfo>();
			List<PublishingPackageInfo> list4 = pkgList.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.Diff).ToList<PublishingPackageInfo>();
			excludePackages = list2.Select((PublishingPackageInfo pkgID) => pkgID.ID).Intersect(list3.Select((PublishingPackageInfo dupPkg) => dupPkg.ID), this.IgnoreCase).ToList<string>();
			list.AddRange(list2.Where((PublishingPackageInfo pkg) => !excludePackages.Contains(pkg.ID, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			list.AddRange(list3.Where((PublishingPackageInfo pkg) => !excludePackages.Contains(pkg.ID, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			list.AddRange(list4);
		...
		{
			List<PublishingPackageInfo> list = new List<PublishingPackageInfo>();
			foreach (PublishingPackageInfo publishingPackageInfo in pkgs)
			{
				PublishingPackageInfo publishingPackageInfo2 = this.ChangeToCanonical(publishingPackageInfo);
				list.Add(publishingPackageInfo2);
			}
			return list.Distinct<PublishingPackageInfo>().ToList<PublishingPackageInfo>();
		}
		...
		private List<PublishingPackageInfo> ChangeToPKRs(List<PublishingPackageInfo> pkgs)
		{
			...
			foreach (PublishingPackageInfo publishingPackageInfo in pkgs)
			{
				PublishingPackageInfo publishingPackageInfo2 = this.ChangeToPKR(publishingPackageInfo);
				list.Add(publishingPackageInfo2);
			}
			return list.Distinct<PublishingPackageInfo>().ToList<PublishingPackageInfo>();
		}
		...
		public List<PublishingPackageInfo> GetOEMPackageListForPOP(OEMInput oemInput1, OEMInput oemInput2)
		{
			...
			if (this.IsUpdateList)
			{
				list = list.Except(this.GetTargetOnlyPkgs(list.Where((PublishingPackageInfo pkg) => pkg.UpdateType == PublishingPackageInfo.UpdateTypes.PKR).ToList<PublishingPackageInfo>())).ToList<PublishingPackageInfo>();
			}
			return list;
		}
		...
		private List<PublishingPackageInfo> GetTargetOnlyPkgs(List<PublishingPackageInfo> list)
		{
			List<string> sourcePackageIDs = (from pkg in this.Packages
				...
				select pkg.ID).Distinct<string>().ToList<string>();
			return list.Where((PublishingPackageInfo pkg) => !sourcePackageIDs.Contains(pkg.ID, StringComparer.OrdinalIgnoreCase)).ToList<PublishingPackageInfo>();
		}
		...
		public List<PublishingPackageInfo> GetAllPackagesForImage(OEMInput oemInput, OwnerType forOwnerType = 0)
		{
			...
			list.AddRange(allPackagesForFeaturesAndFMs.Where((PublishingPackageInfo pkg) => pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Base).ToList<PublishingPackageInfo>());
			list.AddRange(allPackagesForFeaturesAndFMs.Where((PublishingPackageInfo pkg) => pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Language && langs.Contains(pkg.SatelliteValue, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			list.AddRange(allPackagesForFeaturesAndFMs.Where((PublishingPackageInfo pkg) => pkg.SatelliteType == PublishingPackageInfo.SatelliteTypes.Resolution && res.Contains(pkg.SatelliteValue, this.IgnoreCase)).ToList<PublishingPackageInfo>());
			return list.Distinct<PublishingPackageInfo>().ToList<PublishingPackageInfo>();
		}
		...
		public void ValidateConstraints()
		{
			...
			IEnumerable<IGrouping<string, PublishingPackageInfo>> enumerable = from pkg in this.Packages
				...
				group pkg by pkg.ID;
			foreach (IGrouping<string, PublishingPackageInfo> grouping in enumerable)
			{
				if (grouping.Count<PublishingPackageInfo>() > 1)
				{
					IEnumerable<string> enumerable2 = grouping.Select((PublishingPackageInfo pkg) => pkg.FeatureID).Distinct(this.IgnoreCase);
					if (grouping.Count<PublishingPackageInfo>() != enumerable2.Count<string>())
					{
						IEnumerable<IGrouping<string, PublishingPackageInfo>> enumerable3 = from pkg in grouping
							group pkg by pkg.FeatureID;
						foreach (IGrouping<string, PublishingPackageInfo> grouping2 in enumerable3.Where((IGrouping<string, PublishingPackageInfo> gp) => gp.Count<PublishingPackageInfo>() > 1))
						{
							stringBuilder.AppendLine(string.Concat(new object[]
							{
								"\t",
								grouping2.First<PublishingPackageInfo>().FeatureID,
								": (",
								grouping2.First<PublishingPackageInfo>().ID,
								" Count=",
								grouping2.Count<PublishingPackageInfo>(),
								")\n"
							}));
						}
					}
					if (this.IsTargetFeatureEnabled)
					{
						...
						{
							if (enumerable2.Except(list2).Count<string>() == 0)
							{
								flag = true;
								break;
							}
						...
						if (!flag)
						{
							...
							{
								"\t",
								grouping.First<PublishingPackageInfo>().ID,
								": (",
		...
		public static void ValidateAndLoad(ref PublishingPackageList xmlInput, string xmlFile, IULogger logger)
		{
			...
			using (Stream manifestResourceStream = executingAssembly.GetManifestResourceStream(text))
			{
				...
				{
					throw new ImageCommonException("ImageCommon!ValidateAndLoad: Unable to validate Publishing Package Info XSD for file '" + xmlFile + "'.", ex);
				}
			}
			logger.LogInfo("ImageCommon: Successfully validated the Publishing Package Info XML: {0}", new object[] { xmlFile });
			...
			{
				throw new ImageCommonException("ImageCommon!ValidateAndLoad: Unable to parse Publishing Package Info XML file '" + xmlFile + "'", ex2);
			}
			finally
			{
				textReader.Close();
		...
		public void WriteToFile(string xmlFile)
		{
			...
			{
				throw new ImageCommonException("ImageCommon!WriteToFile: Unable to write Publishing Package List XML file '" + xmlFile + "'", ex);
			}
			finally
			{
				textWriter.Close();
