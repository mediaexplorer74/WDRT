		...
		public FullFlashUpdateImage CreateFFU()
		{
			...
			try
			{
				foreach (InputStore inputStore in this._parameters.Stores)
				{
					...
					uint num = this._parameters.MinSectorCount;
					if (!inputStore.IsMainOSStore())
					{
						num = inputStore.SizeInSectors;
					}
					fullFlashUpdateStore.Initialize(fullFlashUpdateImage, inputStore.Id, inputStore.IsMainOSStore(), inputStore.DevicePath, inputStore.OnlyAllocateDefinedGptEntries, num, this._parameters.SectorSize);
			...
			{
				throw new ImageCommonException("ImageCommon!ImageGenerator::CreateFFU: Failed to add partitions to FFU: ", ex2);
			}
