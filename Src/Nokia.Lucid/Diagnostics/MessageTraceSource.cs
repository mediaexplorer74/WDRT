...
namespace Nokia.Lucid.Diagnostics
{
	...
	internal sealed class MessageTraceSource : TraceSource
	{
		...
		public void MessageDispatch_Start(IntPtr windowHandle, int message, IntPtr wParam, IntPtr lParam)
		{
			...
			string text = string.Format(CultureInfo.InvariantCulture, "Dispatching message to window.\r\nHWND: 0x{0:x4}\r\nMessage: 0x{1:x4} ({2})\r\nWPARAM: 0x{3:x4}\r\nLPARAM: 0x{4:x4}", new object[]
			{
			...
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.MessageDispatch, text);
		}
		...
		{
			string messageName = KnownNames.GetMessageName(message);
			string text = string.Format(CultureInfo.InvariantCulture, "Dispatched message to window.\r\nHWND: 0x{0:x4}\r\nMessage: 0x{1:x4} ({2})\r\nWPARAM: 0x{3:x4}\r\nLPARAM: 0x{4:x4}", new object[]
			{
				windowHandle.ToInt64(),
				message,
				messageName,
			...
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.MessageDispatch, text);
		}
		...
		{
			string messageName = KnownNames.GetMessageName(message);
			string text = string.Format(CultureInfo.InvariantCulture, "Error while dispatching message to window, suppressing exception.\r\nHWND: 0x{0:x4}\r\nMessage: 0x{1:x4} ({2})\r\nWPARAM: 0x{3:x4}\r\nLPARAM: 0x{4:x4}\r\nException:\r\n{5}", new object[]
			{
				windowHandle.ToInt64(),
				message,
				messageName,
			...
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.MessageDispatch, text);
		}
		...
		{
			string messageName = KnownNames.GetMessageName(message);
			string text = string.Format(CultureInfo.InvariantCulture, "Received thread message, discarding.\r\nMessage: 0x{0:x4} ({1})\r\nWPARAM: 0x{2:x4}\r\nLPARAM: 0x{3:x4}", new object[]
			{
				message,
				messageName,
				wParam.ToInt64(),
			...
			base.TraceEvent(TraceEventType.Warning, 3, text);
		}
		...
		{
			string messageName = KnownNames.GetMessageName(message);
			string text = string.Format(CultureInfo.InvariantCulture, "Received window message.\r\nHWND: 0x{0:x4}\r\nMessage: 0x{1:x4} ({2})\r\nWPARAM: 0x{3:x4}\r\nLPARAM: 0x{4:x4}", new object[]
			{
				windowHandle.ToInt64(),
				message,
				messageName,
			...
			base.TraceEvent(TraceEventType.Verbose, 1, text);
		}
		...
		{
			string text2;
			string text = (KnownNames.TryGetEventTypeName(eventType, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text4;
			string text3 = (KnownNames.TryGetDeviceTypeName(deviceType, out text4) ? (" (" + text4 + ")") : string.Empty);
			string text5 = string.Format(CultureInfo.InvariantCulture, "Received device notification.\r\nHWND: 0x{0:x4}\r\nEvent: 0x{1:x4}{2}\r\nHeader: 0x{3:x4}{4}", new object[]
			{
				windowHandle.ToInt64(),
				eventType,
				text,
			...
			this.TraceEvent(TraceEventType.Verbose, MessageTraceEventId.DeviceNotification, text5);
		}

		// Token: 0x0600010D RID: 269 RVA: 0x0000A734 File Offset: 0x00008934
		public void MessageWindowCreation_Start(string windowClass)
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Creating message window.\r\nClass: {0}", new object[] { windowClass });
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.MessageWindowCreation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Created message window and acquired HWND handle.\r\nClass: {0}\r\nHWND: 0x{1:x4}", new object[]
			{
				windowClass,
				windowHandle.ToInt64()
			});
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.MessageWindowCreation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Could not create message window.\r\nClass: {0}\r\nError: 0x{1:x4} ({2})", new object[] { windowClass, errorCode, errorMessage });
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.MessageWindowCreation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Attaching window proc to message window.\r\nHWND: 0x{0:x4}", new object[] { windowHandle.ToInt64() });
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.MessageWindowProcAttach, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Attached window proc to message window.\r\nHWND: 0x{0:x4}", new object[] { windowHandle.ToInt64() });
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.MessageWindowProcAttach, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Could not attach window proc to message window.\r\nHWND: {0}\r\nError: 0x{1:x4} ({2})", new object[] { windowHandle, errorCode, errorMessage });
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.MessageWindowProcAttach, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Posting WM_CLOSE to message queue.\r\nHWND: 0x{0:x4}", new object[] { windowHandle.ToInt64() });
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.MessageWindowCloseRequest, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Posted WM_CLOSE to message queue.\r\nHWND: 0x{0:x4}", new object[] { windowHandle.ToInt64() });
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.MessageWindowCloseRequest, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Could not post WM_CLOSE to message queue.\r\nHWND: 0x{0:x4}\r\nError: 0x{1:x4} ({2})", new object[]
			{
				windowHandle.ToInt64(),
				errorCode,
				errorText
			});
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.MessageWindowCloseRequest, text);
		}
		...
		{
			string text2;
			string text = (KnownNames.TryGetInterfaceClassName(interfaceClass, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text3 = string.Format(CultureInfo.InvariantCulture, "Registering device notification.\r\nHWND: 0x{0:x4}\r\nClass: {1}{2}", new object[]
			{
				windowHandle.ToInt64(),
				interfaceClass,
				text
			});
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.DeviceNotificationRegistration, text3);
		}
		...
		public void DeviceNotificationRegistration_Stop(IntPtr windowHandle, Guid interfaceClass, IntPtr devNotifyHandle)
		{
			string text2;
			string text = (KnownNames.TryGetInterfaceClassName(interfaceClass, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text3 = string.Format(CultureInfo.InvariantCulture, "Registered device notification and acquired HDEVNOTIFY handle.\r\nHWND: 0x{0:x4}\r\nClass: {1}{2}\r\nHDEVNOTIFY: 0x{3:x4}", new object[]
			{
				windowHandle.ToInt64(),
				interfaceClass,
				text,
			...
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.DeviceNotificationRegistration, text3);
		}
		...
		public void DeviceNotificationRegistration_Error(IntPtr windowHandle, Guid interfaceClass, int errorCode, string errorMessage)
		{
			string text2;
			string text = (KnownNames.TryGetInterfaceClassName(interfaceClass, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text3 = string.Format(CultureInfo.InvariantCulture, "Could not register device notification, deferring exception.\r\nHWND: 0x{0:x4}\r\nClass: {1}{2}\r\nError: 0x{3:x4} ({4})", new object[]
			{
				windowHandle.ToInt64(),
				interfaceClass,
				text,
			...
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.DeviceNotificationRegistration, text3);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Unregistering device notification.\r\nHWND: 0x{0:x4}\r\nHDEVNOTIFY: 0x{1:x4}", new object[]
			{
				windowHandle.ToInt64(),
				devNotifyHandle.ToInt64()
			});
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.DeviceNotificationUnregistration, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Unregistered device notification.\r\nHWND: 0x{0:x4}\r\nHDEVNOTIFY: 0x{1:x4}", new object[]
			{
				windowHandle.ToInt64(),
				devNotifyHandle.ToInt64()
			});
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.DeviceNotificationUnregistration, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Could not unregister device notification, deferring exception.\r\nHWND: 0x{0:x4}\r\nHDEVNOTIFY: 0x{1:x4}\r\nError: 0x{2:x4} ({3})", new object[]
			{
				windowHandle.ToInt64(),
				devNotifyHandle.ToInt64(),
				errorCode,
			...
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.DeviceNotificationUnregistration, text);
		}
		...
		public void DeviceNotificationProcessing_Start(IntPtr windowHandle, string devicePath, Guid interfaceClass, int eventType)
		{
			...
			string text = (KnownNames.TryGetInterfaceClassName(interfaceClass, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text4;
			string text3 = (KnownNames.TryGetEventTypeName(eventType, out text4) ? (" (" + text4 + ")") : string.Empty);
			string text5 = string.Format(CultureInfo.InvariantCulture, "Processing device notification.\r\nHWND: 0x{0:x4}\r\nPath: {1}\r\nClass: {2}{3}\r\nEvent: 0x{4:x4}{5}", new object[]
			{
				windowHandle.ToInt64(),
				devicePath,
				interfaceClass,
			...
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.DeviceNotificationProcessing, text5);
		}
		...
		public void DeviceNotificationProcessing_Stop(IntPtr windowHandle, string devicePath, Guid interfaceClass, int eventType)
		{
			...
			string text4;
			string text3 = (KnownNames.TryGetEventTypeName(eventType, out text4) ? (" (" + text4 + ")") : string.Empty);
			string text5 = string.Format(CultureInfo.InvariantCulture, "Processed device notification.\r\nHWND: 0x{0:x4}\r\nPath: {1}\r\nClass: {2}{3}\r\nEvent: 0x{4:x4}{5}", new object[]
			{
				windowHandle.ToInt64(),
				devicePath,
				interfaceClass,
			...
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.DeviceNotificationProcessing, text5);
		}
		...
		public void DeviceNotificationProcessing_Error(IntPtr windowHandle, string devicePath, Guid interfaceClass, int eventType, Exception exception)
		{
			...
			string text4;
			string text3 = (KnownNames.TryGetEventTypeName(eventType, out text4) ? (" (" + text4 + ")") : string.Empty);
			string text5 = string.Format(CultureInfo.InvariantCulture, "Error while processing device notification.\r\nHWND: 0x{0:x4}\r\nPath: {1}\r\nClass: {2}{3}\r\nEvent: 0x{4:x4}{5}\r\nException:\r\n{6}", new object[]
			{
				windowHandle.ToInt64(),
				devicePath,
				interfaceClass,
			...
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.DeviceNotificationProcessing, text5);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Delegating thread exception to thread exception handler.\r\nHWND: 0x{0:x4}\r\nThread exception:\r\n{1}", new object[]
			{
				windowHandle.ToInt64(),
				threadException
			});
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.ThreadExceptionDelegation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Error while delegating thread exception to thread exception handler, deferring thread exception.\r\nHWND: 0x{0:x4}\r\nThread exception:\r\n{1}\r\nException:\r\n{2}", new object[]
			{
				windowHandle.ToInt64(),
				threadException,
				exception
			});
			this.TraceEvent(TraceEventType.Error, MessageTraceEventId.ThreadExceptionDelegation, text);
		}
		...
		public void ThreadExceptionDelegation_Stop(IntPtr windowHandle, bool handled, Exception threadException)
		{
			...
			if (handled)
			{
				text = string.Format(CultureInfo.InvariantCulture, "Thread exception handled by thread exception handler, suppressing thread exception.\r\nHWND: 0x{0:x4}\r\nThread exception:\r\n{1}", new object[]
				{
					windowHandle.ToInt64(),
					threadException
				});
			...
			{
				text = string.Format(CultureInfo.InvariantCulture, "Thread exception not handled by thread exception handler, deferring thread exception.\r\nHWND: 0x{0:x4}\r\nThread exception:\r\n{1}", new object[]
				{
					windowHandle.ToInt64(),
					threadException
				});
			}
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.ThreadExceptionDelegation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Changing message window status.\r\nHWND: 0x{0:x4}\r\nOld status: {1}\r\nNew status: {2}", new object[]
			{
				windowHandle.ToInt64(),
				oldStatus,
				newStatus
			});
			this.TraceEvent(TraceEventType.Start, MessageTraceEventId.MessageWindowStatusChange, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Changed message window status.\r\nHWND: 0x{0:x4}\r\nOld status: {1}\r\nNew status: {2}", new object[]
			{
				windowHandle.ToInt64(),
				oldStatus,
				newStatus
			});
			this.TraceEvent(TraceEventType.Stop, MessageTraceEventId.MessageWindowStatusChange, text);
		}
