	...
	internal static class EntityUtilities
	{
		...
		private static Func<object[], object> GenerateActivator(Type type, Type[] ctorParamTypes)
		{
			...
			for (int i = 0; i < parameters.Length; i++)
			{
				...
				Expression expression2 = Expression.ArrayIndex(parameterExpression, expression);
				Expression expression3 = Expression.Convert(expression2, parameterType);
				array[i] = expression3;
			}
			NewExpression newExpression = Expression.New(constructor, array);
			LambdaExpression lambdaExpression = Expression.Lambda(typeof(Func<object[], object>), newExpression, new ParameterExpression[] { parameterExpression });
