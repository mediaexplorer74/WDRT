...
namespace Microsoft.WindowsAzure.Storage.Core.Util
{
	...
	internal class AsyncStreamCopier<T> : IDisposable
	{
		...
		private static void MaximumCopyTimeCallback(object copier, bool timedOut)
		{
			if (timedOut)
			{
				AsyncStreamCopier<T> asyncStreamCopier = (AsyncStreamCopier<T>)copier;
				AsyncStreamCopier<T>.ForceAbort(asyncStreamCopier, true);
			}
		}
		...
		private int NextReadLength()
		{
			...
			{
				long num = this.copyLen.Value - Interlocked.Read(ref this.currentBytesReadFromSource);
				return (int)Math.Min(num, (long)this.currentReadBuff.Length);
			}
