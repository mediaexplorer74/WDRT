...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public sealed class CloudTable
	{
		...
		{
			TableRequestOptions tableRequestOptions = TableRequestOptions.ApplyDefaults(requestOptions, this.ServiceClient);
			operationContext = operationContext ?? new OperationContext();
			if (this.Exists(true, tableRequestOptions, operationContext))
			{
				return false;
			}
			bool flag;
			try
			{
				this.Create(tableRequestOptions, operationContext);
				flag = true;
			}
		...
		public ICancellableAsyncResult BeginCreateIfNotExists(TableRequestOptions requestOptions, OperationContext operationContext, AsyncCallback callback, object state)
		{
			...
			ICancellableAsyncResult cancellableAsyncResult = this.BeginExists(true, tableRequestOptions, operationContext, new AsyncCallback(this.CreateIfNotExistHandler), storageAsyncResult);
		...
		private void CreateIfNotExistHandler(IAsyncResult asyncResult)
		{
			...
			lock (storageAsyncResult.CancellationLockerObject)
			{
				...
				try
				{
					...
					{
						ICancellableAsyncResult cancellableAsyncResult = this.BeginCreate((TableRequestOptions)storageAsyncResult.RequestOptions, storageAsyncResult.OperationContext, delegate(IAsyncResult createRes)
						{
							storageAsyncResult.CancelDelegate = null;
							...
							catch (StorageException ex2)
							{
								...
								{
									if (ex2.RequestInformation.ExtendedErrorInformation == null || ex2.RequestInformation.ExtendedErrorInformation.ErrorCode == TableErrorCodeStrings.TableAlreadyExists)
									{
										storageAsyncResult.Result = false;
										storageAsyncResult.OnComplete();
									}
		...
		public bool DeleteIfExists(TableRequestOptions requestOptions = null, OperationContext operationContext = null)
		{
			...
			try
			{
				this.Delete(tableRequestOptions, operationContext);
				flag = true;
			}
			catch (StorageException ex)
			{
		...
		public ICancellableAsyncResult BeginDeleteIfExists(TableRequestOptions requestOptions, OperationContext operationContext, AsyncCallback callback, object state)
		{
			...
			ICancellableAsyncResult cancellableAsyncResult = this.BeginExists(true, tableRequestOptions, operationContext, new AsyncCallback(this.DeleteIfExistsHandler), storageAsyncResult);
			storageAsyncResult.CancelDelegate = new Action(cancellableAsyncResult.Cancel);
			return storageAsyncResult;
		}
		...
		private void DeleteIfExistsHandler(IAsyncResult asyncResult)
		{
			...
			lock (storageAsyncResult.CancellationLockerObject)
			{
				...
				try
				{
					...
					{
						ICancellableAsyncResult cancellableAsyncResult = this.BeginDelete((TableRequestOptions)storageAsyncResult.RequestOptions, storageAsyncResult.OperationContext, delegate(IAsyncResult deleteRes)
						{
							storageAsyncResult.CancelDelegate = null;
		...
		public string GetSharedAccessSignature(SharedAccessTablePolicy policy, string accessPolicyIdentifier, string startPartitionKey, string startRowKey, string endPartitionKey, string endRowKey, string sasVersion)
		{
			...
			StorageAccountKey key = this.ServiceClient.Credentials.Key;
			string hash = SharedAccessSignatureHelper.GetHash(policy, accessPolicyIdentifier, startPartitionKey, startRowKey, endPartitionKey, endRowKey, canonicalName, text2, key.KeyValue);
			UriQueryBuilder signature = SharedAccessSignatureHelper.GetSignature(policy, this.Name, accessPolicyIdentifier, startPartitionKey, startRowKey, endPartitionKey, endRowKey, hash, key.KeyName, text2);
			return signature.ToString();
		}
		...
		private string GetCanonicalName(string sasVersion)
		{
			...
			return string.Format(CultureInfo.InvariantCulture, text2, new object[] { "table", accountName, text });
		}
	}
}
