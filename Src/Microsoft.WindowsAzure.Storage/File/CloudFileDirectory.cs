...
namespace Microsoft.WindowsAzure.Storage.File
{
	...
	public sealed class CloudFileDirectory : IListFileItem
	{
		...
		private void CreateIfNotExistsHandler(FileRequestOptions options, OperationContext operationContext, StorageAsyncResult<bool> storageAsyncResult)
		{
			ICancellableAsyncResult cancellableAsyncResult = this.BeginExists(options, operationContext, delegate(IAsyncResult existsResult)
			{
				...
				lock (storageAsyncResult.CancellationLockerObject)
				{
					...
					ICancellableAsyncResult cancellableAsyncResult2 = this.BeginCreate(options, operationContext, delegate(IAsyncResult createResult)
					{
						...
						{
							if (ex2.RequestInformation.ExtendedErrorInformation != null && ex2.RequestInformation.ExtendedErrorInformation.ErrorCode == FileErrorCodeStrings.ResourceAlreadyExists)
							{
								storageAsyncResult.Result = false;
								storageAsyncResult.OnComplete();
							}
		...
		{
			FileRequestOptions fileRequestOptions = FileRequestOptions.ApplyDefaults(options, this.ServiceClient, true);
			ResultSegment<IListFileItem> resultSegment = this.ListFilesAndDirectoriesSegmentedCore(maxResults, currentToken, fileRequestOptions, operationContext);
		...
		private RESTCommand<ResultSegment<IListFileItem>> ListFilesAndDirectoriesImpl(int? maxResults, FileRequestOptions options, FileContinuationToken currentToken)
		{
			...
			restcommand.PostProcessResponse = delegate(RESTCommand<ResultSegment<IListFileItem>> cmd, HttpWebResponse resp, OperationContext ctx)
			{
				...
				List<IListFileItem> list = listFilesAndDirectoriesResponse.Files.Select((IListFileEntry item) => this.SelectListFileItem(item)).ToList<IListFileItem>();
				FileContinuationToken fileContinuationToken = null;
				if (listFilesAndDirectoriesResponse.NextMarker != null)
				{
					fileContinuationToken = new FileContinuationToken
					{
		...
		public CloudFileDirectory(Uri directoryAbsoluteUri)
			: this(new StorageUri(directoryAbsoluteUri), null)
		{
		}
		...
		public CloudFile GetFileReference(string fileName)
		{
			...
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, fileName);
			return new CloudFile(storageUri, fileName, this.Share);
		}
		...
		public CloudFileDirectory GetDirectoryReference(string itemName)
		{
			...
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, itemName);
			return new CloudFileDirectory(storageUri, itemName, this.Share);
		}
