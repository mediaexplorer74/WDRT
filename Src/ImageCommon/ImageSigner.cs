...
namespace Microsoft.WindowsPhone.Imaging
{
	...
	public class ImageSigner
	{
		...
		public void SignFFUImage()
		{
			...
			{
				this._logger.LogError("ImageCommon!ImageSigner::SignFFUImage: Error while signing FFU image: {0}", new object[] { ex.Message });
		...
		private bool VerifyCatalogData(byte[] catalogData, byte[] hashTableData)
		{
			...
			bool flag = ImageSigner.VerifyCatalogData(tempFileName, hashTableData);
		...
		internal bool VerifyHashTable()
		{
			...
			this._logger.LogInfo("ImageCommon: Using Chunksize: {0}KB", new object[] { this._ffuImage.ChunkSize });
			try
			{
			...
			{
				throw new ImageCommonException("ImageCommon!ImageSigner::VerifyHashTable: Error while retrieving Hash Table from FFU", ex);
			}
			return true;
		}
			...
			try
			{
				...
				if (EnsureMicrosoftIssuer)
				{
					if (!ImageSigner.certPublicKeys.TryGetValue(x509Certificate.Thumbprint, out flag))
					{
						...
						{
							if (string.Compare("3B1EFD3A66EA28B16697394703A72CA340A05BD5", x509ChainElement.Certificate.Thumbprint, flag2) == 0 || string.Compare("9E594333273339A97051B0F82E86F266B917EDB3", x509ChainElement.Certificate.Thumbprint, flag2) == 0 || string.Compare("5f444a6740b7ca2434c7a5925222c2339ee0f1b7", x509ChainElement.Certificate.Thumbprint, flag2) == 0 || string.Compare("8A334AA8052DD244A647306A76B8178FA215F344", x509ChainElement.Certificate.Thumbprint, flag2) == 0)
							{
								flag = true;
								break;
							}
		...
		internal static byte[] GetCatalogHash(string catalogFile)
		{
			...
			try
			{
				...
				array = new byte[20];
				int num = (int)(cryptcatmember.sEncodedIndirectData.cbData - (uint)array.Length);
				Marshal.Copy(IntPtr.Add(cryptcatmember.sEncodedIndirectData.pbData, num), array, 0, array.Length);
			}
			catch (Exception ex)
			{
