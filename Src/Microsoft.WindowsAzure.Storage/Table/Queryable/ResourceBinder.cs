	...
	internal class ResourceBinder : DataServiceALinqExpressionVisitor
	{
		...
		internal static void VerifyNotSelectManyProjection(Expression expression)
		{
			...
			if (resourceSetExpression != null)
			{
				...
				if (projection != null)
				{
					...
					{
						throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression type {0} is not supported.", new object[] { methodCallExpression }));
					}
				}
				else if (resourceSetExpression.HasTransparentScope)
				{
					throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The expression type {0} is not supported.", new object[] { resourceSetExpression }));
				}
			}
		}
		...
		private static Expression AnalyzePredicate(MethodCallExpression mce)
		{
			...
			if (list4 != null && list4.Count > 0)
			{
				...
				{
					Expression expression3 = ResourceBinder.BuildKeyPredicateFilter(resourceSetExpression.CreateReference(), resourceSetExpression.KeyPredicate);
					list4.Add(expression3);
				...
				{
					num = 0;
					expression4 = resourceSetExpression.Filter.Predicate;
				}
				else
				{
					num = 1;
				...
				{
					expression4 = Expression.And(expression4, list4[i]);
				}
				ResourceBinder.AddSequenceQueryOption(resourceSetExpression, new FilterQueryOptionExpression(mce.Method.ReturnType, expression4));
			}
		...
		internal bool AnalyzeProjection(MethodCallExpression mce, SequenceMethod sequenceMethod, out Expression e)
		{
			...
			if (sequenceMethod == SequenceMethod.SelectManyResultSelector)
			{
				...
				try
				{
					flag2 = ProjectionAnalyzer.Analyze(lambdaExpression, resourceExpression3, false);
				}
			...
			else
			{
				...
				lambdaExpression3 = ProjectionRewriter.TryToRewrite(lambdaExpression3, resourceExpression.ResourceType);
				ResourceExpression resourceExpression4 = resourceExpression.CreateCloneWithNewType(mce.Type);
				if (!ProjectionAnalyzer.Analyze(lambdaExpression3, resourceExpression4, flag))
				{
					return false;
				}
		...
		private static void AddSequenceQueryOption(ResourceExpression target, QueryOptionExpression qoe)
		{
			...
			if (qoe.NodeType == (ExpressionType)10006)
			{
				...
				{
					throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The {0} query option cannot be specified after the {1} query option.", new object[] { "filter", "top" }));
				}
				if (resourceSetExpression.Projection != null)
				{
					throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The {0} query option cannot be specified after the {1} query option.", new object[] { "filter", "select" }));
				}
			}
		...
		internal override Expression VisitMemberAccess(MemberExpression m)
		{
			...
			if (memberExpression != null && ResourceBinder.PatternRules.MatchNonPrivateReadableProperty(memberExpression, out propertyInfo) && TypeSystem.TryGetPropertyAsMethod(propertyInfo, out methodInfo))
			{
		...
		internal override Expression VisitMethodCall(MethodCallExpression mce)
		{
			...
			if (ReflectionUtil.TryIdentifySequenceMethod(mce.Method, out sequenceMethod))
			{
				...
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The method '{0}' is not supported", new object[] { mce.Method.Name }));
			}
			...
			if (mce.Method == TableQueryableExtensions.WithOptionsMethodInfo.MakeGenericMethod(new Type[] { type }))
			{
			...
			if (mce.Method == TableQueryableExtensions.WithContextMethodInfo.MakeGenericMethod(new Type[] { type }))
			{
		...
		internal static class PatternRules
		{
			...
			internal static bool MatchTransparentScopeSelector(ResourceSetExpression input, LambdaExpression resultSelector, out ResourceSetExpression.TransparentAccessors transparentScope)
			{
				...
				for (int i = 0; i < newExpression.Arguments.Count; i++)
				{
					...
					{
						member = properties.Where((PropertyInfo property) => property.GetGetMethod() == member).FirstOrDefault<PropertyInfo>();
						if (member == null)
						{
							return false;
						}
