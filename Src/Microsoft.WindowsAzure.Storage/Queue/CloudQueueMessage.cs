		...
		internal string GetMessageContentForTransfer(bool shouldEncodeMessage, QueueRequestOptions options = null)
		{
			...
			if (options != null)
			{
				...
				if (options.EncryptionPolicy != null)
				{
					...
					{
						throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Encrypted Messages cannot be larger than {0} bytes. Please note that encrypting queue messages can increase their size.", new object[] { CloudQueueMessage.MaxMessageSize }));
					}
					return text;
				}
			}
			...
			if (this.MessageType != QueueMessageType.Base64Encoded)
			{
				if (shouldEncodeMessage)
				{
					...
					{
						throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Messages cannot be larger than {0} bytes.", new object[] { CloudQueueMessage.MaxMessageSize }));
					}
				}
				else
				{
