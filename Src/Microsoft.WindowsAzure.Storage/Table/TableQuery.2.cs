...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public sealed class TableQuery
	{
		...
		internal TableQuerySegment<DynamicTableEntity> ExecuteQuerySegmented(TableContinuationToken token, CloudTableClient client, CloudTable table, TableRequestOptions requestOptions, OperationContext operationContext)
		{
			...
			operationContext = operationContext ?? new OperationContext();
			RESTCommand<TableQuerySegment<DynamicTableEntity>> restcommand = TableQuery.QueryImpl(this, token, client, table, tableRequestOptions);
			return Executor.ExecuteSync<TableQuerySegment<DynamicTableEntity>>(restcommand, tableRequestOptions.RetryPolicy, operationContext);
		}
		...
		internal TableQuerySegment<TResult> ExecuteQuerySegmented<TResult>(TableContinuationToken token, CloudTableClient client, CloudTable table, EntityResolver<TResult> resolver, TableRequestOptions requestOptions, OperationContext operationContext)
		{
			...
			TableRequestOptions tableRequestOptions = TableRequestOptions.ApplyDefaults(requestOptions, client);
			operationContext = operationContext ?? new OperationContext();
			RESTCommand<TableQuerySegment<TResult>> restcommand = TableQuery.QueryImpl<TResult>(this, token, client, table, resolver, tableRequestOptions);
		...
		private static string GenerateFilterCondition(string propertyName, string operation, string givenValue, EdmType edmType)
		{
			...
			{
				text = string.Format(CultureInfo.InvariantCulture, "{0}L", new object[] { givenValue });
			}
			else if (edmType == EdmType.DateTime)
			{
				text = string.Format(CultureInfo.InvariantCulture, "datetime'{0}'", new object[] { givenValue });
			}
			else if (edmType == EdmType.Guid)
			{
				text = string.Format(CultureInfo.InvariantCulture, "guid'{0}'", new object[] { givenValue });
			}
			else if (edmType == EdmType.Binary)
			{
				text = string.Format(CultureInfo.InvariantCulture, "X'{0}'", new object[] { givenValue });
			}
			else
			{
				text = string.Format(CultureInfo.InvariantCulture, "'{0}'", new object[] { givenValue.Replace("'", "''") });
			}
			return string.Format(CultureInfo.InvariantCulture, "{0} {1} {2}", new object[] { propertyName, operation, text });
		}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "({0}) {1} ({2})", new object[] { filterA, operatorString, filterB });
		}
