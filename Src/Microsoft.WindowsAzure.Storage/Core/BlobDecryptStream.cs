...
namespace Microsoft.WindowsAzure.Storage.Core
{
	...
	internal class BlobDecryptStream : Stream
	{
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			{
				LengthLimitingStream lengthLimitingStream = new LengthLimitingStream(this.userStream, (long)this.discardFirst, this.userProvidedLength);
				this.cryptoStream = this.encryptionPolicy.DecryptBlob(lengthLimitingStream, this.metadata, out this.transform, this.requireEncryption, (!this.bufferIV) ? null : this.iv, this.noPadding);
			}
			if (count > 0)
			{
