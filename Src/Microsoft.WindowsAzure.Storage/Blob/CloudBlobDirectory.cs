...
namespace Microsoft.WindowsAzure.Storage.Blob
{
	...
	public sealed class CloudBlobDirectory : IListBlobItem
	{
		...
		public CloudPageBlob GetPageBlobReference(string blobName, DateTimeOffset? snapshotTime)
		{
			...
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, blobName, this.ServiceClient.DefaultDelimiter);
			return new CloudPageBlob(storageUri, snapshotTime, this.ServiceClient.Credentials);
		}
		...
		public CloudBlockBlob GetBlockBlobReference(string blobName, DateTimeOffset? snapshotTime)
		{
			CommonUtility.AssertNotNullOrEmpty("blobName", blobName);
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, blobName, this.ServiceClient.DefaultDelimiter);
			return new CloudBlockBlob(storageUri, snapshotTime, this.ServiceClient.Credentials);
		}
		...
		public CloudAppendBlob GetAppendBlobReference(string blobName, DateTimeOffset? snapshotTime)
		{
			CommonUtility.AssertNotNullOrEmpty("blobName", blobName);
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, blobName, this.ServiceClient.DefaultDelimiter);
			return new CloudAppendBlob(storageUri, snapshotTime, this.ServiceClient.Credentials);
		}
		...
		public CloudBlob GetBlobReference(string blobName, DateTimeOffset? snapshotTime)
		{
			CommonUtility.AssertNotNullOrEmpty("blobName", blobName);
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, blobName, this.ServiceClient.DefaultDelimiter);
			return new CloudBlob(storageUri, snapshotTime, this.ServiceClient.Credentials);
		}
		...
		public CloudBlobDirectory GetDirectoryReference(string itemName)
		{
			...
			StorageUri storageUri = NavigationHelper.AppendPathToUri(this.StorageUri, itemName, this.ServiceClient.DefaultDelimiter);
			return new CloudBlobDirectory(storageUri, this.Prefix + itemName, this.Container);
		}
