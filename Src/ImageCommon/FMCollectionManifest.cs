	...
	public class FMCollectionManifest
	{
		...
		public static void ValidateAndLoad(ref FMCollectionManifest xmlInput, string xmlFile, IULogger logger)
		{
			...
			using (Stream manifestResourceStream = executingAssembly.GetManifestResourceStream(text))
			{
				...
				{
					throw new ImageCommonException("ImageCommon!ValidateAndLoad: Unable to validate FM Collection Manifest XSD for file '" + xmlFile + "'.", ex);
				}
			}
			logger.LogInfo("ImageCommon: Successfully validated the Feature Manifest XML: {0}", new object[] { xmlFile });
			...
			{
				throw new ImageCommonException("ImageCommon!ValidateAndLoad: Unable to parse FM Collection XML file '" + xmlFile + "'.", ex2);
			}
			finally
			{
				textReader.Close();
		...
		public void ValidateFeatureIdentiferPackages(List<PublishingPackageInfo> packages)
		{
			...
			using (List<FeatureIdentifierPackage>.Enumerator enumerator = this.FeatureIdentifierPackages.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					{
						List<PublishingPackageInfo> list = packages.Where((PublishingPackageInfo pkg) => string.Equals(pkg.FeatureID, fip.FeatureID, StringComparison.OrdinalIgnoreCase) && !string.IsNullOrEmpty(fip.ID) && string.Equals(pkg.ID, fip.ID, StringComparison.OrdinalIgnoreCase) && string.Equals(pkg.Partition, fip.Partition, StringComparison.OrdinalIgnoreCase)).ToList<PublishingPackageInfo>();
						if (list.Count<PublishingPackageInfo>() == 0)
						{
							flag = true;
