	...
	public class EventSource : IDisposable
	{
		...
		protected unsafe void WriteEvent(int eventId, string arg1)
		{
			checked
			{
				if (this.m_eventSourceEnabled)
				{
					...
					fixed (char* ptr = arg1)
					{
						EventSource.EventData* ptr2 = stackalloc EventSource.EventData[unchecked((UIntPtr)1) * (UIntPtr)sizeof(EventSource.EventData)];
			...
			if (this.m_eventSourceEnabled)
			{
				...
				fixed (char* ptr = arg1)
				{
					fixed (char* ptr2 = arg2)
					{
						...
						ptr3[1].Size = checked((arg2.Length + 1) * 2);
				...
				fixed (char* ptr = arg1)
				{
					fixed (char* ptr2 = arg2)
					{
						fixed (char* ptr3 = arg3)
						{
							...
							ptr4[1].Size = checked((arg2.Length + 1) * 2);
							ptr4[2].DataPointer = (IntPtr)((void*)ptr3);
							ptr4[2].Size = checked((arg3.Length + 1) * 2);
		...
		protected unsafe void WriteEvent(int eventId, long arg1, string arg2)
		{
			if (this.m_eventSourceEnabled)
			{
				...
				fixed (char* ptr = arg2)
				{
					...
					ptr2[1].Size = checked((arg2.Length + 1) * 2);
					this.WriteEventCore(eventId, 2, ptr2);
				}
			}
		}
		...
		protected unsafe void WriteEvent(int eventId, byte[] arg1)
		{
			if (this.m_eventSourceEnabled)
			{
				...
				fixed (byte* ptr = &arg1[0])
				{
					...
					ptr2->DataPointer = (IntPtr)((void*)(&num));
					ptr2->Size = 4;
					ptr2[1].DataPointer = (IntPtr)((void*)ptr);
					ptr2[1].Size = num;
					this.WriteEventCore(eventId, 2, ptr2);
		...
		protected unsafe void WriteEvent(int eventId, long arg1, byte[] arg2)
		{
			if (this.m_eventSourceEnabled)
			{
				...
				fixed (byte* ptr = &arg2[0])
				{
					...
					ptr2->DataPointer = (IntPtr)((void*)(&arg1));
					ptr2->Size = 8;
					ptr2[1].DataPointer = (IntPtr)((void*)(&num));
					ptr2[1].Size = 4;
					...
					if (commandArgs.Command == EventCommand.Update)
					{
						...
						if (!commandArgs.enable)
						{
							for (int j = 0; j < this.m_eventData.Length; j++)
							{
								...
								this.m_eventData[j].EnabledForAnyListener = flag2;
							}
		...
		private static void DebugCheckEvent(ref Dictionary<string, string> eventsByName, EventSource.EventMetadata[] eventData, MethodInfo method, EventAttribute eventAttribute, ManifestBuilder manifest, EventManifestOptions options)
		{
			...
			checked
			{
				...
				{
					manifest.ManifestError(Microsoft.Diagnostics.Tracing.Internal.Environment.GetResourceString("EventSource_EventNameReused", new object[] { name }), true);
				}
		...
		private unsafe void WriteMultiMergeInner(string eventName, ref EventSourceOptions options, TraceLoggingEventTypes eventTypes, Guid* activityID, Guid* childActivityID, params object[] values)
		{
			...
			checked
			{
				EventSource.EventData* ptr2 = stackalloc EventSource.EventData[unchecked((UIntPtr)(checked(eventTypes.dataCount + 3))) * (UIntPtr)sizeof(EventSource.EventData)];
				GCHandle* ptr3 = stackalloc GCHandle[unchecked((UIntPtr)pinCount) * (UIntPtr)sizeof(GCHandle)];
				fixed (byte* ptr4 = this.providerMetadata, nameMetadata = nameInfo.nameMetadata, typeMetadata = eventTypes.typeMetadata)
				{
					...
					try
					{
						...
						this.WriteEventRaw(ref eventDescriptor, activityID, childActivityID, (int)(unchecked((long)(DataCollector.ThreadInstance.Finish() - ptr2))), (IntPtr)((void*)ptr2));
					}
					finally
					{
		...
		internal unsafe void WriteMultiMerge(string eventName, ref EventSourceOptions options, TraceLoggingEventTypes eventTypes, Guid* activityID, Guid* childActivityID, EventSource.EventData* data)
		{
			...
			checked
			{
				if (nameInfo != null)
				{
					EventSource.EventData* ptr = stackalloc EventSource.EventData[unchecked((UIntPtr)(checked(eventTypes.dataCount + eventTypes.typeInfos.Length * 2 + 3))) * (UIntPtr)sizeof(EventSource.EventData)];
					fixed (byte* ptr2 = this.providerMetadata, nameMetadata = nameInfo.nameMetadata, typeMetadata = eventTypes.typeMetadata)
					{
						...
						for (int i = 0; i < eventTypes.typeInfos.Length; i++)
						{
							if (eventTypes.typeInfos[i].DataType == typeof(string))
							{
								...
								unchecked(ptr[num]).m_Size = unchecked(data[i]).m_Size - 2;
								num++;
							}
							else
							{
		...
		private unsafe void WriteImpl<T>(string eventName, ref EventSourceOptions options, ref T data, Guid* pActivityId, Guid* pRelatedActivityId)
		{
			try
			{
				...
				if (nameInfo != null)
				{
					...
					byte* ptr = stackalloc byte[(UIntPtr)instance.scratchSize];
					checked
					{
						EventSource.EventData* ptr2 = stackalloc EventSource.EventData[unchecked((UIntPtr)(checked(instance.dataCount + 3))) * (UIntPtr)sizeof(EventSource.EventData)];
						GCHandle* ptr3 = stackalloc GCHandle[unchecked((UIntPtr)pinCount) * (UIntPtr)sizeof(GCHandle)];
						try
						{
							fixed (byte* ptr4 = this.providerMetadata)
							{
								fixed (byte* ptr5 = nameInfo.nameMetadata)
								{
									fixed (byte* ptr6 = instance.typeMetadata)
									{
										...
										try
										{
											DataCollector.ThreadInstance.Enable(ptr, instance.scratchSize, ptr2 + 3, instance.dataCount, ptr3, pinCount);
											instance.typeInfo.WriteData(TraceLoggingDataCollector.Instance, ref data);
											this.WriteEventRaw(ref eventDescriptor, pActivityId, pRelatedActivityId, (int)(unchecked((long)(DataCollector.ThreadInstance.Finish() - ptr2))), (IntPtr)((void*)ptr2));
		...
		private struct Sha1ForNonSecretPurposes
		{
			...
			public void Finish(byte[] output)
			{
				...
				checked
				{
					num = this.length + unchecked((long)(checked(8 * this.pos)));
					this.Append(128);
					while (this.pos != 56)
					{
						this.Append(0);
