...
namespace Microsoft.WindowsAzure.Storage.File.Protocol
{
	...
	public sealed class ListFilesAndDirectoriesResponse : ResponseParsingBase<IListFileEntry>
	{
		...
		private IListFileEntry ParseFileEntry(Uri baseUri)
		{
			...
			cloudFileAttributes.StorageUri = new StorageUri(uri);
							...
							while (this.reader.IsStartElement())
							{
								...
								else
								{
									...
									if ((name2 = this.reader.Name) != null)
									{
										...
										if (name2 == "Etag")
										{
											fileDirectoryProperties.ETag = string.Format(CultureInfo.InvariantCulture, "\"{0}\"", new object[] { this.reader.ReadElementContentAsString() });
											continue;
										}
									}
									this.reader.Skip();
