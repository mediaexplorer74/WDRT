...
namespace Microsoft.WindowsAzure.Storage.File
{
	...
	public sealed class CloudFileShare
	{
		...
		public bool CreateIfNotExists(FileRequestOptions requestOptions = null, OperationContext operationContext = null)
		{
			...
			bool flag = this.Exists(fileRequestOptions, operationContext);
			if (flag)
			{
				return false;
			}
			...
			try
			{
				this.Create(fileRequestOptions, operationContext);
				flag2 = true;
			}
				...
				lock (storageAsyncResult.CancellationLockerObject)
				{
					...
					ICancellableAsyncResult cancellableAsyncResult2 = this.BeginCreate(options, operationContext, delegate(IAsyncResult createResult)
					{
						...
						catch (StorageException ex2)
						{
							...
							{
								if (ex2.RequestInformation.ExtendedErrorInformation == null || ex2.RequestInformation.ExtendedErrorInformation.ErrorCode == FileErrorCodeStrings.ShareAlreadyExists)
								{
									storageAsyncResult.Result = false;
									storageAsyncResult.OnComplete();
								}
				...
				lock (storageAsyncResult.CancellationLockerObject)
				{
					...
					ICancellableAsyncResult cancellableAsyncResult2 = this.BeginDelete(accessCondition, options, operationContext, delegate(IAsyncResult deleteResult)
					{
						...
						catch (StorageException ex2)
						{
							...
							{
								if (ex2.RequestInformation.ExtendedErrorInformation == null || ex2.RequestInformation.ExtendedErrorInformation.ErrorCode == FileErrorCodeStrings.ShareNotFound)
								{
									storageAsyncResult.Result = false;
									storageAsyncResult.OnComplete();
								}
		...
		private string GetSharedAccessCanonicalName()
		{
			...
			return string.Format(CultureInfo.InvariantCulture, "/{0}/{1}/{2}", new object[] { "file", accountName, name });
		}
