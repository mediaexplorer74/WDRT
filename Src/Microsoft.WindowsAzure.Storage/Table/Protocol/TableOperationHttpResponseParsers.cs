...
namespace Microsoft.WindowsAzure.Storage.Table.Protocol
{
	...
	internal static class TableOperationHttpResponseParsers
	{
			...
			using (ODataMessageReader odataMessageReader = new ODataMessageReader(new HttpResponseAdapterMessage(resp, cmd.ResponseStream), odataMessageReaderSettings))
			{
				...
				while (odataBatchReader.State == ODataBatchReaderState.Operation)
				{
					...
					if (flag2)
					{
						...
						{
							string text2 = TableRequest.ExtractEntityIndexFromExtendedErrorInformation(cmd.CurrentResult);
							if (!string.IsNullOrEmpty(text2))
							{
								text = text2;
							}
						}
						throw new StorageException(cmd.CurrentResult, "Unexpected response code for operation : " + text, null)
						{
							IsRetryable = true
						};
					}
			...
			else
			{
				...
				using (ODataMessageReader odataMessageReader = new ODataMessageReader(new HttpResponseAdapterMessage(resp, responseStream), odataMessageReaderSettings, new TableStorageModel(accountName)))
				{
					...
					while (odataReader.State == ODataReaderState.EntryStart)
					{
						...
						resultSegment.Results.Add(TableOperationHttpResponseParsers.ReadAndResolve<TElement>(odataEntry, resolver, options));
						odataReader.Read();
					}
		...
		private static void DrainODataReader(ODataReader reader)
		{
			...
			{
				throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "OData Reader state expected to be Completed state. Actual state: {0}.", new object[] { reader.State }));
			}
		}
		...
		private static void ReadQueryResponseUsingJsonParser<TElement>(ResultSegment<TElement> retSeg, Stream responseStream, string etag, Func<string, string, DateTimeOffset, IDictionary<string, EntityProperty>, string, TElement> resolver, Func<string, string, string, string, EdmType> propertyResolver, Type type, OperationContext ctx, TableRequestOptions options)
		{
			...
			{
				flag = TableEntity.DisablePropertyResolverCache;
			...
			using (JsonReader jsonReader = new JsonTextReader(streamReader))
			{
				...
				{
					Dictionary<string, string> dictionary = jtoken2.ToObject<Dictionary<string, string>>();
					retSeg.Results.Add(TableOperationHttpResponseParsers.ReadAndResolveWithEdmTypeResolver<TElement>(dictionary, resolver, propertyResolver, etag, type, ctx, flag, options));
				}
				if (jsonReader.Read())
				{
		...
		private static void ReadEntityUsingJsonParser(TableResult result, TableOperation operation, Stream stream, OperationContext ctx, TableRequestOptions options)
		{
			...
			using (JsonReader jsonReader = new JsonTextReader(streamReader))
			{
				...
				Dictionary<string, string> dictionary = jsonSerializer.Deserialize<Dictionary<string, string>>(jsonReader);
				if (operation.OperationType == TableOperationType.Retrieve)
				{
					result.Result = TableOperationHttpResponseParsers.ReadAndResolveWithEdmTypeResolver<object>(dictionary, operation.RetrieveResolver, options.PropertyResolver, result.Etag, operation.PropertyResolverType, ctx, TableEntity.DisablePropertyResolverCache, options);
				}
				else
				{
					TableOperationHttpResponseParsers.ReadAndUpdateTableEntityWithEdmTypeResolver(operation.Entity, dictionary, EntityReadFlags.Timestamp | EntityReadFlags.Etag, options.PropertyResolver, ctx);
				}
				if (jsonReader.Read())
				{
					throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "The JSON reader has not yet reached the completed state.", new object[0]));
		...
		private static T ReadAndResolve<T>(ODataEntry entry, Func<string, string, DateTimeOffset, IDictionary<string, EntityProperty>, string, T> resolver, TableRequestOptions options)
		{
			...
			foreach (ODataProperty odataProperty in entry.Properties)
			{
				...
				{
					dateTimeOffset = new DateTimeOffset((DateTime)odataProperty.Value);
				}
				else
				{
			...
			if (options.EncryptionPolicy != null)
			{
				...
				if (dictionary.TryGetValue("_ClientEncryptionMetadata2", out entityProperty) && dictionary.TryGetValue("_ClientEncryptionMetadata1", out entityProperty2))
				{
					EncryptionData encryptionData = null;
					byte[] array = options.EncryptionPolicy.DecryptMetadataAndReturnCEK(text, text2, entityProperty2, entityProperty, out encryptionData);
					byte[] binaryValue = entityProperty.BinaryValue;
					HashSet<string> hashSet = JsonConvert.DeserializeObject<HashSet<string>>(Encoding.UTF8.GetString(binaryValue, 0, binaryValue.Length));
					dictionary = options.EncryptionPolicy.DecryptEntity(dictionary, hashSet, text, text2, array, encryptionData);
				}
			...
			return resolver(text, text2, dateTimeOffset, dictionary, entry.ETag);
		}
		...
		private static T ReadAndResolveWithEdmTypeResolver<T>(Dictionary<string, string> entityAttributes, Func<string, string, DateTimeOffset, IDictionary<string, EntityProperty>, string, T> resolver, Func<string, string, string, string, EdmType> propertyResolver, string etag, Type type, OperationContext ctx, bool disablePropertyResolverCache, TableRequestOptions options)
		{
			...
			if (options.EncryptionPolicy != null)
			{
				string text3 = null;
				string text4 = null;
				if (entityAttributes.TryGetValue("_ClientEncryptionMetadata2", out text3) && entityAttributes.TryGetValue("_ClientEncryptionMetadata1", out text4))
				{
					EntityProperty entityProperty = EntityProperty.CreateEntityPropertyFromObject(text3, EdmType.Binary);
					EntityProperty entityProperty2 = EntityProperty.CreateEntityPropertyFromObject(text4, EdmType.String);
					entityAttributes.TryGetValue("PartitionKey", out text);
					entityAttributes.TryGetValue("RowKey", out text2);
					array = options.EncryptionPolicy.DecryptMetadataAndReturnCEK(text, text2, entityProperty2, entityProperty, out encryptionData);
					dictionary.Add("_ClientEncryptionMetadata2", entityProperty);
					byte[] binaryValue = entityProperty.BinaryValue;
					hashSet = JsonConvert.DeserializeObject<HashSet<string>>(Encoding.UTF8.GetString(binaryValue, 0, binaryValue.Length));
				}
				else if (options.RequireEncryption != null && options.RequireEncryption.Value)
				{
					throw new StorageException("Encryption data does not exist. If you do not want to decrypt the data, please do not set the RequireEncryption flag on request options.", null)
			...
			foreach (KeyValuePair<string, string> keyValuePair in entityAttributes)
			{
				...
				{
					dateTimeOffset = DateTimeOffset.Parse(keyValuePair.Value, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal);
					if (etag == null)
					{
						etag = TableOperationHttpResponseParsers.GetETagFromTimestamp(keyValuePair.Value);
					}
				}
				else if (keyValuePair.Key == "_ClientEncryptionMetadata1")
				{
					dictionary.Add(keyValuePair.Key, EntityProperty.CreateEntityPropertyFromObject(keyValuePair.Value, EdmType.String));
				}
				else if (keyValuePair.Key == "_ClientEncryptionMetadata2")
				{
					if (!dictionary.ContainsKey("_ClientEncryptionMetadata2"))
					{
						dictionary.Add(keyValuePair.Key, EntityProperty.CreateEntityPropertyFromObject(keyValuePair.Value, EdmType.Binary));
					}
				}
				else
				{
					if (propertyResolver != null)
					{
						...
						try
						{
							EdmType edmType = propertyResolver(text, text2, keyValuePair.Key, keyValuePair.Value);
							Logger.LogVerbose(ctx, "Attempting to deserialize '{0}' as type '{1}'", new object[] { keyValuePair.Key, edmType });
							try
							{
								TableOperationHttpResponseParsers.CreateEntityPropertyFromObject(dictionary, hashSet, keyValuePair, edmType);
							}
							catch (FormatException ex)
							{
								throw new StorageException(string.Format(CultureInfo.InvariantCulture, "Failed to parse property '{0}' with value '{1}' as type '{2}'", new object[] { keyValuePair.Key, keyValuePair.Value, edmType }), ex)
								{
									IsRetryable = false
								};
							}
						...
						{
							throw new StorageException("The custom property resolver delegate threw an exception. Check the inner exception for more details", ex2)
							{
								IsRetryable = false
							};
						}
					...
					if (type != null)
					{
						...
						if (dictionary2 != null)
						{
							...
							Logger.LogVerbose(ctx, "Attempting to deserialize '{0}' as type '{1}'", new object[] { keyValuePair.Key, edmType2 });
							TableOperationHttpResponseParsers.CreateEntityPropertyFromObject(dictionary, hashSet, keyValuePair, edmType2);
						}
					}
					else
					{
						Logger.LogVerbose(ctx, "No property resolver available. Deserializing the entity properties as strings.", new object[0]);
						TableOperationHttpResponseParsers.CreateEntityPropertyFromObject(dictionary, hashSet, keyValuePair, EdmType.String);
					}
				}
			}
			...
			return resolver(text, text2, dateTimeOffset, dictionary, etag);
		}
