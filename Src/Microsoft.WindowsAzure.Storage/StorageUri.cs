	...
	public sealed class StorageUri : IEquatable<StorageUri>
	{
		...
		public StorageUri(Uri primaryUri, Uri secondaryUri)
		{
			if (primaryUri != null && secondaryUri != null)
			{
				...
				{
					IEnumerable<string> enumerable = primaryUri.Segments.Skip(flag ? 2 : 0);
					IEnumerable<string> enumerable2 = secondaryUri.Segments.Skip(flag2 ? 2 : 0);
					if (!enumerable.SequenceEqual(enumerable2) || primaryUri.Query != secondaryUri.Query)
					{
						throw new ArgumentException("Primary and secondary location URIs in a StorageUri must point to the same resource.", "secondaryUri");
					}
				}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "Primary = '{0}'; Secondary = '{1}'", new object[] { this.PrimaryUri, this.SecondaryUri });
		}
		...
		private static void AssertAbsoluteUri(Uri uri)
		{
			...
			{
				string text = string.Format(CultureInfo.InvariantCulture, "Address '{0}' is a relative address. Only absolute addresses are permitted.", new object[] { uri.ToString() });
				throw new ArgumentException(text, "uri");
			}
		}
