...
namespace Nokia.Lucid.Diagnostics
{
	...
	internal sealed class DeviceDetectionTraceSource : TraceSource
	{
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Compiling filter expression.\r\nExpression:\r\n{0}", new object[] { filter });
			this.TraceEvent(TraceEventType.Start, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Compiled filter expression.\r\nExpression:\r\n{0}", new object[] { filter });
			this.TraceEvent(TraceEventType.Stop, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Error while compiling filter expression.\r\nExpression:\r\n{0}\r\nException:\r\n{1}", new object[] { filter, exception });
			this.TraceEvent(TraceEventType.Error, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Evaluating filter expression for device path.\r\nPath: {0}", new object[] { devicePath });
			this.TraceEvent(TraceEventType.Start, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		public void FilterExpressionEvaluation_Stop(string devicePath, bool matched)
		{
			...
			if (matched)
			{
				text = string.Format(CultureInfo.InvariantCulture, "Device path matches filter expression.\r\nPath: {0}", new object[] { devicePath });
			}
			else
			{
				text = string.Format(CultureInfo.InvariantCulture, "Device path does not match filter expression, rejecting device notification.\r\nPath: {0}", new object[] { devicePath });
			}
			this.TraceEvent(TraceEventType.Stop, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Error while evaluating filter expression for device path.\r\nPath: {0}\r\nException:\r\n{1}", new object[] { devicePath, exception });
			this.TraceEvent(TraceEventType.Error, DeviceDetectionTraceEventId.FilterExpressionEvaluation, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Raising device change event.\r\nAction: {0}\r\nPath: {1}\r\nType: {2}", new object[] { action, devicePath, deviceType });
			this.TraceEvent(TraceEventType.Start, DeviceDetectionTraceEventId.DeviceChangeEvent, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Raised device change event.\r\nAction: {0}\r\nPath: {1}\r\nType: {2}", new object[] { action, devicePath, deviceType });
			this.TraceEvent(TraceEventType.Stop, DeviceDetectionTraceEventId.DeviceChangeEvent, text);
		}
		...
		{
			string text = string.Format(CultureInfo.InvariantCulture, "Error while raising device change event.\r\nAction: {0}\r\nPath: {1}\r\nType: {2}\r\nException:\r\n{3}", new object[] { action, devicePath, deviceType, exception });
			this.TraceEvent(TraceEventType.Error, DeviceDetectionTraceEventId.DeviceChangeEvent, text);
		}
		...
		{
			string text2;
			string text = (KnownNames.TryGetInterfaceClassName(classGuid, out text2) ? (" (" + text2 + ")") : string.Empty);
			string text3 = string.Format(CultureInfo.InvariantCulture, "Invalid device interface class mapping.\r\nClass: {0}{1}\r\nType: {2}", new object[] { classGuid, text, deviceType });
			this.TraceEvent(TraceEventType.Warning, DeviceDetectionTraceEventId.InvalidDeviceMapping, text3);
		}
