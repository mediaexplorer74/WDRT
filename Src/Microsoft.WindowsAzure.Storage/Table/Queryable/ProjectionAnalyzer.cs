	...
	internal static class ProjectionAnalyzer
	{
		...
		internal static bool Analyze(LambdaExpression le, ResourceExpression re, bool matchMembers)
		{
			...
			else
			{
				if (le.Body.NodeType == ExpressionType.Call)
				{
					...
					if (methodCallExpression.Method == ReflectionUtil.ProjectMethodInfo.MakeGenericMethod(new Type[] { le.Body.Type }))
					{
		...
		private class EntityProjectionAnalyzer : ALinqExpressionVisitor
		{
			...
			internal static void Analyze(MemberInitExpression mie, PathBox pb)
			{
				...
				foreach (MemberBinding memberBinding in mie.Bindings)
				{
					...
					if (memberAssignment != null)
					{
						...
						{
							throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Initializing instances of the entity type {0} with the expression {1} is not supported.", new object[] { memberType, memberAssignment.Expression }));
						}
						...
						if (flag2 && !flag)
						{
							throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Constructing or initializing instances of the type {0} with the expression {1} is not supported.", new object[] { memberType, memberAssignment.Expression }));
						}
					}
				}
			}
			...
			internal override Expression VisitMemberAccess(MemberExpression m)
			{
				...
				if (ResourceBinder.PatternRules.MatchNonPrivateReadableProperty(m, out propertyInfo))
				{
					Expression expression = base.VisitMemberAccess(m);
					this.box.AppendToPath(propertyInfo);
					return expression;
				}
				throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Initializing instances of the entity type {0} with the expression {1} is not supported.", new object[]
				{
