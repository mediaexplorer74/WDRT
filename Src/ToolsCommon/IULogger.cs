	...
	public class IULogger : IDeploymentLogger
	{
		...
		public void LogException(Exception exp, LoggingLevel level)
		{
			...
			if (stackTrace.FrameCount > 0)
			{
				...
				if (frame != null)
				{
					string text = string.Format("{0}({1},{2}):", frame.GetFileName(), frame.GetFileLineNumber(), frame.GetFileColumnNumber());
					stringBuilder.Append(string.Format("{0}{1}", text, Environment.NewLine));
				}
			}
