...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public sealed class TableOperation
	{
		...
		internal Uri GenerateRequestURI(Uri uri, string tableName)
		{
			...
			if (this.isTableEntity)
			{
				text = string.Format(CultureInfo.InvariantCulture, "'{0}'", new object[] { this.Entity.WriteEntity(null)["TableName"].StringValue });
			}
			...
			return NavigationHelper.AppendPathToSingleUri(uri, string.Format(CultureInfo.InvariantCulture, "{0}({1})", new object[] { tableName, text }));
		}
