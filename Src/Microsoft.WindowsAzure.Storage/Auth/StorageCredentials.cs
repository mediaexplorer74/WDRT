...
namespace Microsoft.WindowsAzure.Storage.Auth
{
	...
	public sealed class StorageCredentials
	{
		...
		public void UpdateKey(string keyValue, string keyName)
		{
			if (!this.IsSharedKey)
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Cannot update key unless Account Key credentials are used.", new object[0]);
		...
		public void UpdateSASToken(string sasToken)
		{
			if (!this.IsSAS)
			{
				string text = string.Format(CultureInfo.CurrentCulture, "Cannot update Shared Access Signature unless Sas credentials are used.", new object[0]);
				throw new InvalidOperationException(text);
			}
			CommonUtility.AssertNotNullOrEmpty("sasToken", sasToken);
			this.SASToken = sasToken;
