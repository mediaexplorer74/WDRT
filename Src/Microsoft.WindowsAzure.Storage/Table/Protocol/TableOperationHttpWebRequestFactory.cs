...
namespace Microsoft.WindowsAzure.Storage.Table.Protocol
{
	...
	internal static class TableOperationHttpWebRequestFactory
	{
		...
		internal static HttpWebRequest BuildRequestForTableQuery(Uri uri, UriQueryBuilder builder, int? timeout, bool useVersionHeader, OperationContext ctx, TablePayloadFormat payloadFormat)
		{
			...
			Logger.LogInformational(ctx, "Setting payload format for the request to '{0}'.", new object[] { payloadFormat });
			return httpWebRequest;
		}
		...
		internal static Tuple<HttpWebRequest, Stream> BuildRequestForTableOperation(Uri uri, UriQueryBuilder builder, IBufferManager bufferManager, int? timeout, TableOperation operation, bool useVersionHeader, OperationContext ctx, TableRequestOptions options, string accountName)
		{
			...
			Logger.LogInformational(ctx, "Setting payload format for the request to '{0}'.", new object[] { value });
			...
			if (operation.OperationType == TableOperationType.Insert || operation.OperationType == TableOperationType.Merge || operation.OperationType == TableOperationType.InsertOrMerge || operation.OperationType == TableOperationType.InsertOrReplace || operation.OperationType == TableOperationType.Replace)
			{
				...
				ODataMessageWriter odataMessageWriter = new ODataMessageWriter(httpWebRequestAdapterMessage, odataMessageWriterSettings, new TableStorageModel(accountName));
				ODataWriter odataWriter = odataMessageWriter.CreateODataEntryWriter();
				TableOperationHttpWebRequestFactory.WriteOdataEntity(operation.Entity, operation.OperationType, ctx, odataWriter, options);
		...
		{
			HttpWebRequest httpWebRequest = TableOperationHttpWebRequestFactory.BuildRequestCore(NavigationHelper.AppendPathToSingleUri(uri, "$batch"), builder, "POST", timeout, useVersionHeader, ctx);
			TablePayloadFormat value = options.PayloadFormat.Value;
			Logger.LogInformational(ctx, "Setting payload format for the request to '{0}'.", new object[] { value });
			ODataMessageWriterSettings odataMessageWriterSettings = new ODataMessageWriterSettings
			{
			...
			HttpWebRequestAdapterMessage httpWebRequestAdapterMessage = new HttpWebRequestAdapterMessage(httpWebRequest, bufferManager);
			ODataMessageWriter odataMessageWriter = new ODataMessageWriter(httpWebRequestAdapterMessage, odataMessageWriterSettings);
			...
			{
				string text = tableOperation.HttpMethod;
				if (tableOperation.OperationType == TableOperationType.Merge || tableOperation.OperationType == TableOperationType.InsertOrMerge)
				{
					options.AssertNoEncryptionPolicyOrStrictMode();
					text = "MERGE";
				}
				ODataBatchOperationRequestMessage odataBatchOperationRequestMessage = odataBatchWriter.CreateOperationRequestMessage(text, tableOperation.GenerateRequestURI(uri, tableName));
				...
				{
					using (ODataMessageWriter odataMessageWriter2 = new ODataMessageWriter(odataBatchOperationRequestMessage, odataMessageWriterSettings, new TableStorageModel(accountName)))
					{
						ODataWriter odataWriter = odataMessageWriter2.CreateODataEntryWriter();
						TableOperationHttpWebRequestFactory.WriteOdataEntity(tableOperation.Entity, tableOperation.OperationType, ctx, odataWriter, options);
					}
				}
			}
			if (!flag)
		...
		internal static IEnumerable<ODataProperty> GetPropertiesFromDictionary(IDictionary<string, EntityProperty> properties, TableRequestOptions options, string partitionKey, string rowKey)
		{
			...
			return properties.Select((KeyValuePair<string, EntityProperty> kvp) => new ODataProperty
			{
				Name = kvp.Key,
