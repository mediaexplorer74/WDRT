...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	public sealed class CloudTableClient
	{
		...
		public TableResultSegment ListTablesSegmented(string prefix, int? maxResults, TableContinuationToken currentToken, TableRequestOptions requestOptions = null, OperationContext operationContext = null)
		{
			...
			List<CloudTable> list = tableQuerySegment.Results.Select((DynamicTableEntity tbl) => new CloudTable(tbl.Properties["TableName"].StringValue, this)).ToList<CloudTable>();
			return new TableResultSegment(list)
			{
		...
		private static TableQuery<DynamicTableEntity> GenerateListTablesQuery(string prefix, int? maxResults)
		{
			...
			{
				string text = prefix + '{';
				tableQuery = tableQuery.Where(TableQuery.CombineFilters(TableQuery.GenerateFilterCondition("TableName", "ge", prefix), "and", TableQuery.GenerateFilterCondition("TableName", "lt", text)));
			}
			if (maxResults != null)
			{
