	...
	public sealed class QueueEncryptionPolicy
	{
		...
		internal string EncryptMessage(byte[] inputMessage)
		{
			...
			using (AesCryptoServiceProvider aesCryptoServiceProvider = new AesCryptoServiceProvider())
			{
				...
				text = JsonConvert.SerializeObject(cloudQueueEncryptedMessage);
			}
			return text;
		}
		...
		internal byte[] DecryptMessage(string inputMessage, bool? requireEncryption)
		{
			...
			try
			{
				...
				if (cloudQueueEncryptedMessage.EncryptionData != null)
				{
					...
					if (encryptionAlgorithm == EncryptionAlgorithm.AES_CBC_256)
					{
						using (AesCryptoServiceProvider aesCryptoServiceProvider = new AesCryptoServiceProvider())
						{
							...
							byte[] array2 = Convert.FromBase64String(cloudQueueEncryptedMessage.EncryptedMessageContents);
				...
				{
					array3 = Convert.FromBase64String(cloudQueueEncryptedMessage.EncryptedMessageContents);
				}
			}
			catch (JsonException ex)
			{
				throw new StorageException("Error while de-serializing the encrypted queue message string from the wire. Please check inner exception for more details.", ex)
				{
					IsRetryable = false
				};
			}
