...
namespace Microsoft.WindowsAzure.Storage.Table
{
	...
	internal class TableQueryProvider : IQueryProvider
	{
		...
		public IQueryable CreateQuery(Expression expression)
		{
			...
			Type type = typeof(TableQuery<>).MakeGenericType(new Type[] { elementType });
			...
			return (IQueryable)TableQueryProvider.ConstructorInvoke(constructor, array);
		}
		...
		{
			CommonUtility.AssertNotNull("expression", expression);
			return ReflectionUtil.TableQueryProviderReturnSingletonMethodInfo.MakeGenericMethod(new Type[] { expression.Type }).Invoke(this, new object[] { expression });
		}
		...
		{
			CommonUtility.AssertNotNull("expression", expression);
			return (TResult)((object)ReflectionUtil.TableQueryProviderReturnSingletonMethodInfo.MakeGenericMethod(new Type[] { typeof(TResult) }).Invoke(this, new object[] { expression }));
		}
		...
		{
			IQueryable<TElement> queryable = new TableQuery<TElement>(expression, this);
			...
			throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, "The method '{0}' is not supported.", new object[] { methodCallExpression.Method.Name }));
		}
