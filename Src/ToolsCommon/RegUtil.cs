...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	...
	public static class RegUtil
	{
		...
		public static byte[] HexStringToByteArray(string hexString)
		{
			...
			{
				foreach (string text in hexString.Split(new char[] { ',' }))
				{
					byte b = 0;
					if (!byte.TryParse(text, NumberStyles.AllowHexSpecifier, CultureInfo.InvariantCulture.NumberFormat, out b))
					{
						throw new IUException("Invalid hex string: {0}", new object[] { hexString });
					}
					list.Add(b);
				}
			}
		...
		{
			string text = RegUtil.NormalizeValueName(name);
			output.AppendFormat(";Value:{0}", string.Join(";", values.Select((string x) => x.Replace(";", "\\;"))));
			output.AppendLine();
			output.AppendFormat("{0}=hex(7):", text);
		...
		public static void RegOutput(StringBuilder output, string name, ulong value)
		{
			...
			output.AppendLine();
			output.AppendFormat("{0}=hex(b):", text);
		...
		{
			string text = RegUtil.NormalizeValueName(name);
			output.AppendFormat("{0}=dword:{1:X8}", text, value);
			output.AppendLine();
		}
		...
		{
			string text = RegUtil.NormalizeValueName(name);
			output.AppendFormat("{0}=hex:", text);
		...
		{
			string text = RegUtil.NormalizeValueName(name);
			output.AppendFormat("{0}=hex({1:x}):", text, type);
			RegUtil.ByteArrayToRegString(output, value.ToArray<byte>(), RegUtil.BinaryLineLength / 3);
			output.AppendLine();
		}
